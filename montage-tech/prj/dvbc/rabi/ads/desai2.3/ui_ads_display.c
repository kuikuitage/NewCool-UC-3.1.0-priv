...
BOOL is_unauth_ad_on_showing = FALSE;
...
RET_CODE nvram_ad_logo_read(u32 offset, u8 *p_buf, u32 size)
{
	...
	nvm_read = dm_direct_read(p_dm_handle,DEC_RW_ADS_BLOCK_ID, 0, size, p_buf);
	...
	{
		DEBUG(ADS,INFO,"adv_logo:read logo data length too long or data type not right \n",__FUNCTION__);
		return ERR_FAILURE;
	}
	...
	DEBUG(ADS,INFO,"read crc =0x%x,check crc =0x%x\n",crc_32_read,crc_32_check);
...
static RET_CODE ui_pic_fullscr_play(void)
{
	...
	{   
		DEBUG(ADS,INFO,"@@DS_AD: Full scr status not ready!\n");
		return ret;
	}
	if((pic_full_scr_info->uiDataLen == 0) || (pic_full_scr_info->pucPicData == NULL))
	{
		DEBUG(ADS,INFO,"@@DS_AD: Full scr ad data error!\n");
		return ret;
	}
	//fullscreen ad
	DEBUG(ADS,INFO,"[UI_ADS_DIS] fw_get_focus_id()= %d \n",fw_get_focus_id());
	DEBUG(ADS,INFO,"[show full screen ad] pic_full_scr_info = 0x%x, pic_show_info->uiDataLen = %d\n",pic_full_scr_info,pic_full_scr_info->uiDataLen);
	if((ROOT_ID_BACKGROUND == fw_get_focus_id()))
	{
		pic_adv_stop();
		DEBUG(ADS,INFO,"[ui_pic_fullscr_play] show fullscreen ad!!!!\n");
...
static RET_CODE ui_unauth_ad_play(void)
{
	...
	{
		DEBUG(ADS,INFO,"pic_unauth_ad_info is null, will show full screen ad \n");
	...
	if(ROOT_ID_BACKGROUND == fw_get_focus_id())
	{
		...
		DEBUG(ADS,INFO,"ui_unauth_ad_play show pic_unauth_ad_info!! \n");
...
RET_CODE ui_check_fullscr_ad_play(void)
{
  ...
  if(TRUE == is_unauth_ad_enable)
  {
    if(ROOT_ID_BACKGROUND == fw_get_focus_id())
    {
      ...
      UI_PRINTF("show ADS_AD_TYPE_UNAUTH ad ret value = %d \n",ret);
    }
  }  
  if(!ret)
  {
    ret = ui_pic_fullscr_play();
	UI_PRINTF("show ADS_AD_TYPE_FULLSCREEN ad ret = %d \n",ret);
  }
  return ret;
}
...
static RET_CODE ui_show_desai_osd_ad(u8 *p_data)
{
  ...
  OS_PRINTF("@@@@@pst_osd_ads_roll->ucOsdNum = %d \n",pst_osd_ads_roll->ucOsdNum);
...
static RET_CODE ui_show_desai_pic_ad(u8 *p_data)
{
	...
	DEBUG(ADS,INFO,"[UI]: pic_show_info-->enPicType = %d \n",pic_show_info->enPicType);
	...
	switch(pic_show_info->enPosType)
	{
	    ...
	    {
			DEBUG(ADS,INFO,"[DSAD_STB_UNAUTHORIZED] p_data = 0x%x, pic_show_info->uiDataLen = %d\n",
			        p_data, pic_show_info->uiDataLen);
			DEBUG(ADS,INFO,"[DSAD_STB_UNAUTHORIZED] x= %d, y=%d, w= %d, h = %d \n",
		...
		{
			DEBUG(ADS,INFO,"[show full screen ad] get_full_scr_ad_status= %d \n",get_full_scr_ad_status());
			DEBUG(ADS,INFO,"[show full screen ad] p_data = 0x%x, pic_show_info->uiDataLen = %d\n",
			            p_data, pic_show_info->uiDataLen);
			DEBUG(ADS,INFO,"[full screen ad rect] x= %d, y=%d, w= %d, h = %d \n",
			          pic_show_info->usStartX, pic_show_info->usStartY,
			         pic_show_info->usWidth, pic_show_info->usHeigth);
...
static RET_CODE ui_save_logo_data_to_nvm(ST_DSAD_AV_SHOW_INFO * av_show_info)
{
	...
	{
		DEBUG(ADS,INFO,"av_show_info->uiDataLen = %d  \n",av_show_info->uiDataLen);
		return ret;
	}
...
RET_CODE ui_adv_desai_display(ads_info_t *p_ads_info)
{
	...
	if(p_ads_info == NULL)
	{
		DEBUG(ADS,INFO,"ADS: ads desai p_ads_info null \n");
		return ret;
	}
	...
	switch(p_ads_info->ads_media_type)
	{
		case ADS_AD_TYPE_OSD:
		{
			DEBUG(ADS,INFO,"ADS: Desai ad type [OSD_TYPE] \n");
		...
		{
			DEBUG(ADS,INFO,"ADS: Desai ad type [PIC_TYPE] \n");
		...
		case ADS_AD_TYPE_AV:
		{
			DEBUG(ADS,INFO,"ADS: Desai ad type [AV_TYPE] \n");
...
RET_CODE ui_adv_desai_hide(ads_ad_type_t ad_type, u8 *p_data)
{
	...
	{
		DEBUG(ADS,INFO,"[UI_ADS_DIS] hide fullscreen ad \n");
		pic_adv_stop();
		is_full_screen_ad_on_showing = FALSE;
	}
...
RET_CODE on_ads_process(control_t *p_ctrl, u16 msg, u32 para1, u32 para2)
{
	...
	if(NULL != p_data)
	{
		...
		{
			case ADS_AD_TYPE_PIC:
				DEBUG(ADS,INFO,"ADS: on_ads_process Desai ad type [PIC_TYPE] \n");
				ret = ui_show_desai_pic_ad(p_data);
				break;
			case ADS_AD_TYPE_AV:
				DEBUG(ADS,INFO,"ADS:on_ads_process  Desai ad type [AV_TYPE] \n");
	...
	else
	{
		...
		{
			DEBUG(ADS,INFO,"ADS:on_ads_process  Desai ad type [OSD_UPDATE] \n");
