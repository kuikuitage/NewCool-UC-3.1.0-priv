...
static RET_CODE ui_adv_pic_region_init(const rect_t *rect)
{
  ...
  //p_region_buffer = (void*)SUB_BUFFER_ADDR;
#ifdef WIN32
  //p_region_buffer = (void *)((u32)p_region_buffer &  (~0xF0000000));
#endif
  //MT_ASSERT(p_region_buffer != NULL);
...
BOOL ui_adv_set_pic_by_type(ads_ad_type_t type)
{
  ...
  switch(type)
  {
    case ADS_AD_TYPE_MENU_UP:
      g_rect.left = (SCREEN_WIDTH-DESAI_PIC_W)/2;//160 + 90;
    ...
    case ADS_AD_TYPE_VOLBAR:
      g_rect.left = 100/*VOLUME_L + VOLUME_W*/;
      g_rect.right = g_rect.left+DESAI_PIC_W;
      g_rect.top =100 /*VOLUME_T - (DESAI_PIC_H-VOLUME_H)/2*/;
      g_rect.bottom = g_rect.top+DESAI_PIC_H;
      break;
...
BOOL ads_get_flag[DSAD_STB_POS_UNKNOWN] = {0,};
void desai_ads_check_all_pic(void)
{
		int  cnt = 0;
		u16 pg_id = 0;
		ads_info_t ads_info = {0};
