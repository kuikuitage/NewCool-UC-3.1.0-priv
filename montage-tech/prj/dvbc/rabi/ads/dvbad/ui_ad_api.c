...
static BOOL ui_adv_set_pic_by_type(ads_ad_type_t type)
{
	...
	switch(type)
	{
		...
		case ADS_AD_TYPE_MENU_DOWN:
			g_rect.left = (SCREEN_WIDTH-400)/2;
			g_rect.right = g_rect.left + 400 ;
			g_rect.top = (SCREEN_HEIGHT-250)/2;
			g_rect.bottom = g_rect.top + 250;
			break;
		case ADS_AD_TYPE_VOLBAR:
			g_rect.left = VOLUME_L + VOLUME_W;
			g_rect.right = g_rect.left+300;
			g_rect.top =VOLUME_T - (100-VOLUME_H)/2;
			g_rect.bottom = g_rect.top+100;
			break;
		case ADS_AD_TYPE_CHLIST:
			g_rect.left = 20;
			g_rect.right = g_rect.left+412-52;
			g_rect.top = 520;
			g_rect.bottom = g_rect.top+200; 
			break;
		case ADS_AD_TYPE_CHBAR:
			g_rect.left = SCREEN_WIDTH-200;
			g_rect.right = g_rect.left+200;
			g_rect.top = SCREEN_HEIGHT-200;
			g_rect.bottom = g_rect.top+200; 
			break;
		case ADS_AD_TYPE_BANNER:
			g_rect.left = SCREEN_WIDTH-200+60;
			g_rect.right = g_rect.left+120;
			g_rect.top = SCREEN_HEIGHT-200+60;
	...
	ads_dvbad_set_type(type,&cmd,&ads_info);
	ret = ads_io_ctrl(g_adm_id,cmd,&ads_info);
	DEBUG(ADS,INFO,"start show picture tickets2=%d\n", mtos_ticks_get());
	
	if(ads_info.ads_data_len)
	{
		if(SUCCESS == ui_adv_set_pic(ads_info.ads_data_len, (u8 *)ads_info.p_priv))
		{
			DEBUG(ADS,INFO," TRUE\n");
			return TRUE;
		}
