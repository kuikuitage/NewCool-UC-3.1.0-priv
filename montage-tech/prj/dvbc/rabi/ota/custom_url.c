 ...
 //static custom_url_t customs_url_tbl[MAX_CUSTOM_URL];
  ...
  if(rd>0)
  {
    ...
    while(k < MAX_CUSTOM_URL && (p_line = strtok(p_line?NULL:buf, "\r\n")))
    {
      ...
      while(((customs_url_tbl[k].url[j++] = *p_line++))&&(j<CUS_URL_LEN));
      if(CUS_URL_LEN == j)
      {
        while(*p_line)p_line++;
        customs_url_tbl[k].url[CUS_URL_LEN-1] = '\0';
      }
     // mtos_printk("name:%s", customs_url_tbl[k].name);
