...
void ui_ota_api_ota_param_init(void)
{
   ...
   ota_param.ota_tp.lock_mode = ota_info.sys_mode;
   ota_param.ota_tp.data_pid = ota_info.download_data_pid;
   memcpy(&ota_param.ota_tp.locks, &ota_info.locks,sizeof(dvbs_lock_info_t));
   #else
   memset(&ota_param,0,sizeof(ota_param_t));
#ifdef DTMB_PROJECT
   ota_param.ota_tp.lock_mode = SYS_DTMB;
   ota_param.ota_tp.lockt.tp_freq = 474000;
#else
...
void ui_ota_api_set_ota_tp(void)
{
   ...
   memcpy((u8 *)&ota_info, (u8 *)sys_status_get_ota_info(), sizeof(ota_info_t));
   set_tp.lock_mode  = ota_info.sys_mode;
   set_tp.data_pid = ota_info.download_data_pid;
#ifdef DTMB_PROJECT
    memcpy(&set_tp.lockt, &ota_info.lockt,sizeof(dvbc_lock_info_t));
    OS_PRINTF("[debug] ota_info.lockt.tp_freq = %x  ota_info.sys_mode = %d  ota_info.download_data_pid = %x\n",ota_info.lockt.tp_freq,
		ota_info.sys_mode,ota_info.download_data_pid);
#else
...
void ui_ota_api_auto_mode_start(ota_work_t  mode)
{
  ...
  g_auto_mode = TRUE;
   ...
   OS_PRINTF("#####ui_ota_api_init:lock mode:%d\n",policy.tp_info.do_ota_tp.lock_mode);
    OS_PRINTF("#####ui_ota_api_init:lockc freq:%d\n",policy.tp_info.do_ota_tp.lockc.tp_freq);
    OS_PRINTF("#####ui_ota_api_init:lockt freq:%d\n",policy.tp_info.do_ota_tp.lockt.tp_freq);
    OS_PRINTF("#####ui_ota_api_init:lock sym:%d\n",policy.tp_info.do_ota_tp.lockc.tp_sym);
