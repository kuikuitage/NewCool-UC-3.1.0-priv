...
static RET_CODE mmi_init(void)
{
  ...
  gdi_info.vsurf_buf_addr = mem_mgr_require_block(BLOCK_OSD1_32BIT_BUFFER, SYS_MODULE_GDI);
  MT_ASSERT(gdi_info.vsurf_buf_addr != 0);

  gdi_info.vsurf_buf_size = mem_mgr_get_block_size(BLOCK_OSD1_32BIT_BUFFER);
