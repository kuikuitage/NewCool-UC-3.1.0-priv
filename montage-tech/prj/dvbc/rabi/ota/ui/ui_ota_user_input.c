  ...
  for(i = 0; i < OTA_USER_INPUT_ITEM_CNT; i++)
  {
    switch(i + 1)
    {
      ...
      case IDC_OTA_USER_INPUT_DPID:
        ...
        switch(i + 1)
        {
          case IDC_OTA_USER_INPUT_TPFREQ:
            ...
            if(ota_param.ota_tp.lock_mode == SYS_DVBC)
              nbox_set_num_by_dec(p_item[i], ota_param.ota_tp.lockc.tp_freq);
            else if(ota_param.ota_tp.lock_mode == SYS_DTMB)
              nbox_set_num_by_dec(p_item[i], ota_param.ota_tp.lockt.tp_freq);
            break;
#ifndef DTMB_PROJECT
          case IDC_OTA_USER_INPUT_SYM:
      ...
      case IDC_OTA_USER_INPUT_QAM:
        ...
        cbox_static_set_focus(p_item[i], (NIM_MODULA_QAM64 - NIM_MODULA_QAM16));
        y += (OTA_USER_INPUT_ITEMH + OTA_USER_INPUT_ITEM_VGAP);
        break;    
#endif
...
#ifdef DTMB_PROJECT
  ctrl_set_related_id(p_item[1],  0,  5, 0, 4);//freq
  ctrl_set_related_id(p_item[3],  0,  2, 0, 5);//pid
  ctrl_set_related_id(p_item[4],  0,  4, 0, 2);//start
#else
	...
	control_t *p_ctrl, u16 msg, u32 para1, u32 para2)
{
  ...
  struct control *p_tpfreq;
#ifndef DTMB_PROJECT
  struct control *p_symb, *p_modulation;
#endif
  ...
  upgrade.lockt.tp_freq = nbox_get_num(p_tpfreq); //KHz
#else
...
#ifdef DTMB_PROJECT
  memcpy(&(tp_info.lockt), &(upgrade.lockt), sizeof(upgrade.lockt));
#else
