...
#define DES_ROUND_DATA_LEN 8
#define DES_KEY_SIZE 24
#define MAX_BUFFER_LEN 1024//increase if not enough

static void HexToString(char * s,int srclen,char *outstr,int max_out_len)
{
	int i;
	int j=0;
	unsigned char tmpch=' ';
	...
	{
		tmpch = s[i];
		*(tmpret+j) = hexes[ tmpch >> 4];
		j++;
		*(tmpret+j) = hexes[ tmpch & 0xf];
		j++;
	}
#if defined WIN32 || defined WIN_NT
	_snprintf(outstr,max_out_len-1,"%s",tmpret);
#else
	snprintf(outstr,max_out_len-1,"%s",tmpret);
#endif
}
static void StringToHex(char *s,char *outstr,int *outputlen)
{
	unsigned int i;
	int tmpseq=0;
	char tmpret[1024];
	...
	*outputlen = strlen(s)/2;
	memset(tmphexstr,0,sizeof(tmphexstr));
	memset(tmpret,0,sizeof(tmpret));
	for(i=0; i<strlen(s); i+=2)
	{
		strncpy(tmphexstr,s+i,2);
		sxx = strtol(tmphexstr,NULL,16);
		tmpret[tmpseq] = (unsigned char )sxx;
		tmpseq++;
	}
	
	memcpy(outstr,tmpret,*outputlen);
}

int IPTV_3DesEncrypt(char *key,char *data,char *out_data,int max_out_len)
{
	unsigned char key_temp[DES_KEY_SIZE];
	unsigned char buf_temp[MAX_BUFFER_LEN];//test
	int data_len = strlen(data);
	int data_padding_len = DES_ROUND_DATA_LEN-(data_len%DES_ROUND_DATA_LEN);
	int round = 0;
	int i = 0;
	int out_len = strlen(data)+data_padding_len;
	if(out_len > sizeof(buf_temp))//error should increase the buf_temp size
	{
		return 0;
	}
	memset(buf_temp,0,sizeof(buf_temp));
	memset(key_temp,'0',sizeof(key_temp));
	memcpy((char *)key_temp,(char *)key,(strlen(key)>sizeof(key_temp))?sizeof(key_temp):strlen(key));
	memcpy((char *)buf_temp,data,data_len);

	if(data_padding_len == 0)
	{
		data_padding_len = 8;
	}
	memset(&buf_temp[data_len],data_padding_len,data_padding_len);//pad data
	round = out_len/DES_ROUND_DATA_LEN;
	doPC(key_temp);
	doPC(key_temp+8);
	doPC(key_temp+16);
	for(i=0;i<round;i++)
	{
		des(key_temp,DES_ECS2_CRYPT,buf_temp+DES_ROUND_DATA_LEN*i);
		des(key_temp+8,DES_ECS2_DECRYPT,buf_temp+DES_ROUND_DATA_LEN*i);
		des(key_temp+16,DES_ECS2_CRYPT,buf_temp+DES_ROUND_DATA_LEN*i);
	}
	HexToString(buf_temp,out_len,out_data,max_out_len);
	data_len = strlen(out_data);
	mtos_printk("encrypt out:%s\n",out_data);
	return data_len;
}
...
int IPTV_3DesDecrypt(char *key,char *data,char *out_data,int max_out_len)
{
	...
	memset(key_temp,'0',sizeof(key_temp));
	memcpy((char *)key_temp,(char *)key,(strlen(key)>sizeof(key_temp))?sizeof(key_temp):strlen(key));
	StringToHex(data,buf_temp,&max_out_len);
	round = max_out_len/DES_ROUND_DATA_LEN;
	doPC(key_temp);
	doPC(key_temp+8);
	doPC(key_temp+16);
	for(i=0;i<round;i++)
	{
		des(key_temp+16,DES_ECS2_DECRYPT,buf_temp+DES_ROUND_DATA_LEN*i);
		des(key_temp+8,DES_ECS2_CRYPT,buf_temp+DES_ROUND_DATA_LEN*i);
		des(key_temp,DES_ECS2_DECRYPT,buf_temp+DES_ROUND_DATA_LEN*i);
	}
	for(i=max_out_len-1;i>=0;i--)
	{
		if(buf_temp[i] <= 8)
		{
			buf_temp[i] = 0;
	...
	data_len = strlen(buf_temp);
#if defined WIN32 || defined WIN_NT
	_snprintf(out_data,max_out_len-1,"%s",buf_temp);
#else
	snprintf(out_data,max_out_len-1,"%s",buf_temp);
#endif
	mtos_printk("decrypt out:  %d %s\n",data_len,out_data);
	return data_len;
}
