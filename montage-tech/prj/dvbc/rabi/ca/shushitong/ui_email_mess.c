...
static RET_CODE email_plist_update(control_t* ctrl, u16 start, u16 size, u32 context)
{
  ...
  for (i = 0; i < size; i++)
  {
    if (i + start < cnt)
    {
      ...
      sprintf((char *)asc_str,"%02x", p_mail_data_buf->p_mail_head[i + start].send_date[6]);
      asc_to_dec(asc_str, &tmp_value, 2);
      ...
      #if 1
      sprintf((char *)asc_str, "%04d/%02d/%02d %02d:%02d:%02d",
              p_mail_data_buf->p_mail_head[i + start].send_date[0] * 100 
                + p_mail_data_buf->p_mail_head[i + start].send_date[1],
              p_mail_data_buf->p_mail_head[i + start].send_date[2],
              p_mail_data_buf->p_mail_head[i + start].send_date[3],
              p_mail_data_buf->p_mail_head[i + start].send_date[4],
              p_mail_data_buf->p_mail_head[i + start].send_date[5],
              p_mail_data_buf->p_mail_head[i + start].send_date[6]);
      #endif
...
RET_CODE open_email_mess(u32 para1, u32 para2)
{
  ...
  list_field_attr_t email_plist_attr[EMAIL_LIST_FIELD] =
  {
    ...
    { LISTFIELD_TYPE_UNISTR | STL_CENTER| STL_VCENTER,
        400, 50, 0, &email_plist_field_rstyle,  &emial_plist_field_fstyle},

    { LISTFIELD_TYPE_STRID | STL_CENTER| STL_VCENTER,
        40, 460, 0, &email_plist_field_rstyle,  &emial_plist_field_fstyle },

    { LISTFIELD_TYPE_UNISTR | STL_CENTER| STL_VCENTER ,
        240, 550, 0, &email_plist_field_rstyle,  &emial_plist_field_fstyle},
  };
...
static RET_CODE on_email_list_change_focus(control_t *p_list, u16 msg, u32 para1, u32 para2)  
{
  ...
  {
	ctrl_paint_ctrl(ctrl_get_child_by_id(p_list->p_parent, IDC_EMAIL_RECEIVED_HEAD),TRUE);
	ctrl_paint_ctrl(ctrl_get_child_by_id(p_list->p_parent, IDC_EMAIL_RECEIVED),TRUE);
	ctrl_paint_ctrl(ctrl_get_child_by_id(p_list->p_parent, IDC_EMAIL_RESET_HEAD),TRUE);
	ctrl_paint_ctrl(ctrl_get_child_by_id(p_list->p_parent, IDC_EMAIL_RESET),TRUE);
