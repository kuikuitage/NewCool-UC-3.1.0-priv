...
//#define dvb_cas_manager
#ifdef  dvb_cas_manager
	#define DVB_MANAGER_PRINTF mtos_printk
#else
	#define DVB_MANAGER_PRINTF(...)     do{}while(0)
#endif
static void ca_do_cmd(u32 cmd_id, u32 para1)
{
   DVB_MANAGER_PRINTF("[DVBCA] +++++++++++%s %s %d \n",__FILE__,__FUNCTION__,__LINE__);
   ui_ca_do_cmd(cmd_id,para1,0);
}
void dvbca_set_sid(u16 pg_id)
{
	DVB_MANAGER_PRINTF("[DVBCA] +++++++++++%s %s %d \n",__FILE__,__FUNCTION__,__LINE__);
    cas_sid_t ca_sid;
...
void dvbca_dvb_monitor_start(u16 pg_id)
{
	DVB_MANAGER_PRINTF("[DVBCA] +++++++++++%s %s %d \n",__FILE__,__FUNCTION__,__LINE__);
    play_param_t g_pb_info = {0};
...
void dvbca_stop_play(void)
{
	DVB_MANAGER_PRINTF("[DVBCA] +++++++++++%s %s %d \n",__FILE__,__FUNCTION__,__LINE__);
	ca_do_cmd(CAS_CMD_STOP,0);
}

void dvbca_reset_data(void)
{
	DVB_MANAGER_PRINTF("[DVBCA] +++++++++++%s %s %d \n",__FILE__,__FUNCTION__,__LINE__);
	ca_do_cmd(CAS_CMD_FACTORY_SET,0);
}

void dvbca_area_limit_check_start(void)
{
	DVB_MANAGER_PRINTF("[DVBCA] +++++++++++%s %s %d \n",__FILE__,__FUNCTION__,__LINE__);
	on_ca_zone_check_start(CAS_CMD_ZONE_CHECK,0, 0);
}

void dvbca_get_card_info(void)
{
	DVB_MANAGER_PRINTF("[DVBCA] +++++++++++%s %s %d \n",__FILE__,__FUNCTION__,__LINE__);
	ca_do_cmd(CAS_CMD_CARD_INFO_GET,0);
}

void dvbca_auto_receive_email_start(void)
{
	DVB_MANAGER_PRINTF("[DVBCA] +++++++++++%s %s %d \n",__FILE__,__FUNCTION__,__LINE__);
	ca_do_cmd(CAS_CMD_MAIL_HEADER_GET, 0);
}

void dvbca_redraw_after_close_all_menu()
{
	DEBUG(CAS, INFO, "\n");
 // redraw_finger();
}

//destop CA module manage
RET_CODE dvbca_manage_process_menu(ca_input_menu_type type,control_t *p_ctrl, u32 para1, u32 para2)
{
    ...
    switch(type)
    {
      ...
      ret = manage_open_menu(ROOT_ID_CA_ENTITLE_EXP_DLG, 0, 0);
      break;
...
static menu_attr_t dvbca_menu_attr[] =//register ca message
{
  //  root_id,            play_state,   auto_close, signal_msg,   open
     //{ROOT_ID_CONDITIONAL_ACCEPT, PS_KEEP, OFF, SM_OFF, open_conditional_accept},
     {ROOT_ID_EMAIL_MESS, PS_STOP, OFF, SM_OFF, open_email_mess},
     {ROOT_ID_CA_CARD_INFO, PS_KEEP, OFF, SM_OFF, open_ca_card_info},
     {ROOT_ID_CONDITIONAL_ACCEPT_LEVEL, PS_KEEP, OFF, SM_OFF, open_cas_accept_level},
     {ROOT_ID_CONDITIONAL_ACCEPT_PIN, PS_KEEP, OFF, SM_OFF, open_cas_accept_pin},
     {ROOT_ID_CONDITIONAL_ACCEPT_WORKTIME, PS_KEEP, OFF, SM_OFF, open_cas_accept_worktime},
...
cas_manager_policy_t *construct_cas_manager_policy(void)
{
  ...
  p_policy->ap_cas_id  = CAS_ID_DVBCA;
  p_policy->cas_cfg_id = CONFIG_CAS_ID_DVBCA;
  ...
  p_policy->cas_set_sid = dvbca_set_sid;
  p_policy->cas_dvb_monitor_start = dvbca_dvb_monitor_start;
  p_policy->cas_auto_receive_email_start = dvbca_auto_receive_email_start;
  p_policy->cas_play_stop = dvbca_stop_play;
  p_policy->cas_sys_ready = NULL;
  p_policy->cas_reset_data = dvbca_reset_data;
  p_policy->cas_request_card_info = dvbca_get_card_info;
  p_policy->cas_ui_desktop_process = NULL;
  p_policy->cas_manage_process_menu = dvbca_manage_process_menu;
  p_policy->cas_manage_process_msg = ui_desktop_proc_cas;
    p_policy->cas_manage_process_keymap = ui_desktop_keymap_cas;
  p_policy->ca_menu_attr =(menu_attr_t *) dvbca_menu_attr;
  p_policy->attr_number  = (sizeof(dvbca_menu_attr) /sizeof(menu_attr_t));
  ...
  p_policy->cas_manage_reroll_after_close_all_menu = dvbca_redraw_after_close_all_menu;
