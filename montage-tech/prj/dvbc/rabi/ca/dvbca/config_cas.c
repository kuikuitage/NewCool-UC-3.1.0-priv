...
static RET_CODE nvram_read(u32 offset, u8 *p_buf, u32 *size)
{
  ...
  read_len = dm_direct_read(p_dm_handle, CADATA_BLOCK_ID, offset, *size, p_buf);
  *size = read_len;
...
static RET_CODE nvram_erase(u32 offset, u32 size)
{
  ...
  base_addr =  dm_get_block_addr(p_dm_handle, CADATA_BLOCK_ID) - get_flash_addr();
   ...
   OS_PRINTF("[debug] nvram_erase offset[%x] base_addr[%x] read_len[%d] \n", offset , base_addr,size);
...
static RET_CODE nvram_write(u32 offset, u8 *p_buf, u32 size)
{
    ...
    base_addr =  dm_get_block_addr(p_dm_handle, CADATA_BLOCK_ID) - get_flash_addr();
      ...
      OS_PRINTF("[debug] nvram_read base_addr[%x]  offset[%x]  size[%d] \n",base_addr, offset , size);
    if(dm_direct_write(p_dm_handle, CADATA_BLOCK_ID, offset, size, p_buf) == DM_SUC)
    {
      return SUCCESS;
    }
    else
...
extern RET_CODE cas_adt_dvbca_attach(cas_module_cfg_t *p_cfg, u32 *p_cam_id);
...
#ifndef WIN32
  DEBUG(DBG,INFO,"\n");
  cas_adt_dvbca_attach(&cas_module_cfg, &cas_id);
  DEBUG(DBG,INFO,"\n");
  cas_module_init(CAS_ID_DVBCA);
  DEBUG(DBG,INFO,"\n");
#endif
