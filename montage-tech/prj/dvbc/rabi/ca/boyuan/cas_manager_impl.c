...
//#define boyuan_cas_manager
#ifdef  boyuan_cas_manager
	#define BOYUAN_MANAGER_PRINTF mtos_printk
#else
	#define BOYUAN_MANAGER_PRINTF(...)     do{}while(0)
#endif
static void ca_do_cmd(u32 cmd_id, u32 para1)
{
   BOYUAN_MANAGER_PRINTF("[BOYUAN] +++++++++++%s %s %d \n",__FILE__,__FUNCTION__,__LINE__);
   ui_ca_do_cmd(cmd_id,para1,0);
}

void boyuan_ca_set_sid(u16 pg_id)
{
	BOYUAN_MANAGER_PRINTF("[BOYUAN] +++++++++++%s %s %d \n",__FILE__,__FUNCTION__,__LINE__);
    cas_sid_t ca_sid;
...
void boyuan_ca_dvb_monitor_start(u16 pg_id)
{
	BOYUAN_MANAGER_PRINTF("[BOYUAN] +++++++++++%s %s %d \n",__FILE__,__FUNCTION__,__LINE__);
    play_param_t g_pb_info = {0};
...
void boyuan_ca_reset_data(void)
{
	BOYUAN_MANAGER_PRINTF("[BOYUAN] +++++++++++%s %s %d \n",__FILE__,__FUNCTION__,__LINE__);
...
void boyuan_ca_area_limit_check_start(void)
{
	BOYUAN_MANAGER_PRINTF("[BOYUAN] +++++++++++%s %s %d \n",__FILE__,__FUNCTION__,__LINE__);
...
void boyuan_ca_get_card_info(void)
{
	BOYUAN_MANAGER_PRINTF("[BOYUAN] +++++++++++%s %s %d \n",__FILE__,__FUNCTION__,__LINE__);
	ca_do_cmd(CAS_CMD_CARD_INFO_GET,0);
}

void boyuan_ca_auto_receive_email_start(void)
{
	BOYUAN_MANAGER_PRINTF("[BOYUAN] +++++++++++%s %s %d \n",__FILE__,__FUNCTION__,__LINE__);
...
void boyuan_ca_redraw_after_close_all_menu()
{
	DEBUG(CAS, INFO, "\n");
	redraw_finger();
}
...
RET_CODE boyuan_ca_manage_process_menu(ca_input_menu_type type,control_t *p_ctrl, u32 para1, u32 para2)
{
...
static menu_attr_t boyuan_ca_menu_attr[] =
{
  ...
  #if 0
static preview_attr_t boyuan_preview_attr[] =
{
  {
...
cas_manager_policy_t *construct_cas_manager_policy(void)
{
  ...
  DEBUG(CAS, INFO, "\n");
  p_policy->ap_cas_id  =   CAS_ID_BY;
  p_policy->cas_cfg_id = CONFIG_CAS_ID_BY;
  ...
  #endif
  p_policy->cas_area_limit_check_start = boyuan_ca_area_limit_check_start;
  p_policy->cas_set_sid = boyuan_ca_set_sid;
  p_policy->cas_dvb_monitor_start = boyuan_ca_dvb_monitor_start;
  p_policy->cas_auto_receive_email_start = boyuan_ca_auto_receive_email_start;
  p_policy->cas_play_stop = boyuan_ca_stop_play;
  p_policy->cas_sys_ready = NULL;
  p_policy->cas_reset_data = boyuan_ca_reset_data;
  p_policy->cas_request_card_info = boyuan_ca_get_card_info;
  p_policy->cas_ui_desktop_process = NULL;
  p_policy->cas_manage_process_menu = boyuan_ca_manage_process_menu;
  p_policy->cas_manage_process_msg = ui_desktop_proc_cas;
    p_policy->cas_manage_process_keymap = ui_desktop_keymap_cas;
  p_policy->ca_menu_attr =(menu_attr_t *) boyuan_ca_menu_attr;
  p_policy->attr_number  = (sizeof(boyuan_ca_menu_attr) /sizeof(menu_attr_t));
  ...
  p_policy->cas_manage_reroll_after_close_all_menu = boyuan_ca_redraw_after_close_all_menu;
