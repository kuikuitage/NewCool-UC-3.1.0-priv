...
static RET_CODE ca_info_list_entitle_update(control_t *p_list, u16 start, u16 size, u32 context)
{
  ...
  for (i = 0; i < size; i++)
  {
    if (i + start < cnt)
    {
      ...
      sprintf((char *)asc_str,"%s",p_entitle->p_entitle_info[i + start].product_name);
      ...
      sprintf((char *)asc_str,"%04d/%02d/%02d %02d:%02d:%02d ~ %04d/%02d/%02d %02d:%02d:%02d",
                p_entitle->p_entitle_info[i + start].start_time[0]*100 + 
                ...
                p_entitle->p_entitle_info[i + start].expired_time[0]*100 + p_entitle->p_entitle_info[i + start].expired_time[1],
...
RET_CODE open_ca_entitle_info(u32 para1, u32 para2)
{
  ...
  {
    IDS_CA_PRODUCT_ID, IDS_CA_SP_NAME, IDS_CA_AUTHORIZE_DEADLINE
  };
  ...
  static list_field_attr_t ca_info_list_entitle_attr[CA_INFO_ENTITLE_FIELD] =
  {
	...
	{ LISTFIELD_TYPE_UNISTR| STL_LEFT| STL_VCENTER,
      CA_INFO_PRODUCT_NAME_W, CA_INFO_PRODUCT_NAME_VGAP, 0, &ca_info_field_rstyle,  &ca_info_field_fstyle },
    { LISTFIELD_TYPE_UNISTR| STL_LEFT| STL_VCENTER,
      CA_INFO_ENTITLE_TIME_W, (CA_INFO_PRODUCT_NAME_W + CA_INFO_PRODUCT_NAME_VGAP), 0, &ca_info_field_rstyle,  &ca_info_field_fstyle },
  };
  ...
  {
    mbox_set_content_by_strid(p_mbox, 0, itemmenu_btn_str[0]);
	mbox_set_content_by_strid(p_mbox, 1, itemmenu_btn_str[1]);
	mbox_set_content_by_strid(p_mbox, 3, itemmenu_btn_str[2]);
  }
