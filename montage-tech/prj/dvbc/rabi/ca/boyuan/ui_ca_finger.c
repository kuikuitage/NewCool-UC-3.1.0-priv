...
p_ca_finger->show_color[2], 
  ...
  if(p_ca_finger->disp_bg)
  {
   ...
   gdi_set_palette(0, (u16)C_BLUE, 1, &color);
 	}
  ...
  g_rsc_config.p_rstyle_tab[RSI_BLUE].bg.value = p_colors;
#endif

  finger_rect->w = static_w * ratio;
  finger_rect->h = static_h * ratio; 
  //x & y
  ...
  {
    finger_rect->x = p_ca_finger->show_postion_x;
    finger_rect->y = p_ca_finger->show_postion_y;
  } 
...
void open_ca_finger_menu(finger_msg_t *p_ca_finger)
{
  ...
  p_cont = fw_create_mainwin(ROOT_ID_FINGER_PRINT,
                             finger_rect.x, finger_rect.y,
                             finger_rect.w, finger_rect.h,
                             ROOT_ID_INVALID, 0,
                             OBJ_ATTR_INACTIVE, 0);
  if(p_cont == NULL)
  {
  ...
  //ctrl_set_rstyle(p_cont, RSI_ROLL_OSD, RSI_ROLL_OSD, RSI_ROLL_OSD);
  ctrl_set_rstyle(p_cont, RSI_BLUE, RSI_BLUE, RSI_BLUE);
  ...
  DEBUG(CAS, INFO, "show finger:%s\n", p_ca_finger->data);
  DEBUG(CAS, INFO, "x:%d, y:%d, w:%d, h:%d\n", finger_rect.x, finger_rect.y, 
  										finger_rect.w, finger_rect.h);
#ifdef SPT_DUMP_DATA
