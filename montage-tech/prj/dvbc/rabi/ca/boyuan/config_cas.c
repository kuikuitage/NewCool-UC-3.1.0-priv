...
//extern RET_CODE cas_boyuan_attach(cas_module_cfg_t *p_cfg, u32 *p_cam_id);
...
static RET_CODE nvram_write(u32 offset, u8 *p_buf, u32 size)
{
    ...
    MT_ASSERT(p_dm_handle != NULL);
    base_addr =  dm_get_block_addr(p_dm_handle, CADATA_BLOCK_ID) - get_flash_addr();
    dm_direct_erase(p_dm_handle, CADATA_BLOCK_ID, offset -base_addr, size);
...
void on_cas_init(void)
{
   ...
   u32 cas_id = 0;
    ...
    cas_by_attach(&cas_module_cfg, &cas_id);
    cas_module_init(CAS_ID_BY);
