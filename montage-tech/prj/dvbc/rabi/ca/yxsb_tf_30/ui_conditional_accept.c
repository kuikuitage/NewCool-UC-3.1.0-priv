...
RET_CODE open_conditional_accept(u32 para1, u32 para2)
{
  ...
  u16 str_id[CONDITIONAL_ACCEPT_ITEM_CNT] = 
  {
    ...
    IDS_CA_CARD_MANAGE, IDS_CA_STB_PAIR2, IDS_CA_EMAIL
  };
  ...
  {
    p_ctrl[i] = ctrl_create_ctrl(CTRL_TEXT, (u8)(IDC_BTN_CA_INFO + i),
                                              CONDITIONAL_ACCEPT_ITEM_X, (u16)y,
...
void for_test()
{
  ...
  for(i = 0; i < 4; i++)
  {
    p_card_info.cas_ver[i] = 0xF - i;
  }
  p_oper_info.max_num = 4;
  for(i = 0; i < 4; i++)
  {
    p_oper_info.p_operator_info[i].operator_id = i + 10;
  }
}
#endif
static RET_CODE on_ca_frm_btn_select(control_t *p_ctrl, u16 msg, u32 para1, u32 para2)
{
  ...
  switch(ctrlID)
  {
    case IDC_BTN_CA_INFO:
      ...
      #else
      manage_open_menu(ROOT_ID_CA_CARD_INFO, &p_card_info, 0);
      #endif
      break;
    ...
    case IDC_BTN_CA_OPE_INFO:
      #ifndef WIN32
      manage_open_menu(ROOT_ID_CA_OPE, (u32)para1, 0);
      #else 
      manage_open_menu(ROOT_ID_CA_OPE, &p_entitle_info, 0);
      #endif
      break;
      
    case IDC_BTN_CA_FEED:
      manage_open_menu(ROOT_ID_CONDITIONAL_ACCEPT_MOTHER, (u32)para1, 0);
      break;
      
    case IDC_BTN_CA_PAIR:
