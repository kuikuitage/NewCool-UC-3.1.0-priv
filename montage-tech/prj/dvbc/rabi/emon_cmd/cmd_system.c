...
static int TE_Register_check(char *param)
{
  ...
  RegisterFlag = 1;
  APPLOGA("AV Register check\n");
  //SDK_Boxinfo_t tBoxinfo = {0};
  //SDK_Boxinfo_GetBoxinfo(&tBoxinfo);
  sys_status_get_av_set(&av_set);
  APPLOGA("av_set.tv_mode = %d,av_set.tv_resolution = %d\n",av_set.tv_mode,av_set.tv_resolution);

  APPLOGA(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>disp 部分校验>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>\r\n");
  for(i = 0; i < DISP_NUM; i++)
  {
    //uReadValue =  MT_SYS_ReadRegister(st_RegisterArr[i].m_RegisterAddr);
    uReadValue =  hal_get_u32((volatile u32 *)st_RegisterArr[i].m_RegisterAddr);
    
    if((uReadValue & st_RegisterArr[i].m_MaskValue) != (st_RegisterArr[i].m_RegisterValue & st_RegisterArr[i].m_MaskValue))
    {
      RegisterFlag = 0;
      APPLOGA("Check Fail ResgiterAddr[0x%08x],RegisterValue[0x%08x],MaskValue[0x%08x],uReadValue[0x%08x]\r\n",st_RegisterArr[i].m_RegisterAddr,st_RegisterArr[i].m_RegisterValue,st_RegisterArr[i].m_MaskValue,uReadValue);
    }
  }
  ...
  for(i = DISP_NUM; i < REGISTER_NUM; i++)
  {
    ...
    switch(st_RegisterArr[i].m_RegisterAddr)
    {
      case 0xbf460008:
      {
        if((av_set.tv_mode == 0) && (av_set.tv_resolution == 3) && ((uReadValue & st_RegisterArr[i].m_MaskValue) != (0x13362621 & st_RegisterArr[i].m_MaskValue)))
          APPLOGA("Check Fail  1080I_PAL ResgiterAddr[0x%08x],RegisterValue[0x%08x],MaskValue[0x%08x],uReadValue[0x%08x]\r\n",st_RegisterArr[i].m_RegisterAddr,st_RegisterArr[i].m_RegisterValue,st_RegisterArr[i].m_MaskValue,uReadValue);
        else if((av_set.tv_mode == 0) && (av_set.tv_resolution == 2) && ((uReadValue & st_RegisterArr[i].m_MaskValue) != (0x13362621 & st_RegisterArr[i].m_MaskValue)))
          APPLOGA("Check Fail  720P_PAL ResgiterAddr[0x%08x],RegisterValue[0x%08x],MaskValue[0x%08x],uReadValue[0x%08x]\r\n",st_RegisterArr[i].m_RegisterAddr,st_RegisterArr[i].m_RegisterValue,st_RegisterArr[i].m_MaskValue,uReadValue);
        else if((av_set.tv_mode == 0) && (av_set.tv_resolution == 1) && ((uReadValue & st_RegisterArr[i].m_MaskValue) != (st_RegisterArr[i].m_RegisterValue  & st_RegisterArr[i].m_MaskValue)))
          APPLOGA("Check Fail 576P_PAL ResgiterAddr[0x%08x],RegisterValue[0x%08x],MaskValue[0x%08x],uReadValue[0x%08x]\r\n",st_RegisterArr[i].m_RegisterAddr,st_RegisterArr[i].m_RegisterValue,st_RegisterArr[i].m_MaskValue,uReadValue);
      }
      break;
      default:
        if((uReadValue & st_RegisterArr[i].m_MaskValue) != (st_RegisterArr[i].m_RegisterValue & st_RegisterArr[i].m_MaskValue))
  ...
  if(RegisterFlag == 1)
    APPLOGA("---------------Register Check SuccessFul-----------------------------\r\n"); 	
  else
  {
    APPLOGA("---------------Register Check Failed-----------------------------\r\n");
  }
...
static int  disp_av_check(char *param)
{
  void *p_disp = NULL;
  ...
  if(p_disp == NULL)
  {
    APPLOGA("Check Fail  p_disp is NULL\n");
  }
  else
  {
...
static EMON_COMMAND_LIST cmd_sys[] = 
{
  ...
  {disp_av_check, "av.check", "disp av check",FALSE},
  {0, 0, 0, 0}
};
