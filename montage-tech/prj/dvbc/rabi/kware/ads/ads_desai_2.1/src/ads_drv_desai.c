...
extern EN_DSAD_POS_TYPE desai_ad_type_get(u32 ads_type);


#if 0 //for test, will be delete
extern u8 *g_temp_nvram;
#endif

extern BOOL desai_ads_check_all_pic(void);
...
BOOL ds_ad_set_filter_nit_table(void)
{
	...
	ADS_DRV_PRINTF("[drv]Start to set NIT filter %s %d ! \n",__func__,__LINE__);
...
#if 0
u16 ds_ads_find_pat(u8 *p_data, u32 data_len)
{
    u32 index =0;
    u32 read_data_len = 0;
    u16 ts_pid = 0xffff;
    u8   pat_table_id = 0xff;
    u8   pmt_table_id = 0xff;
    u8   payload_start_flag = 0;
    ...
    {
        index = read_data_len;
        if(p_data[index] == 0x47 && p_data[index + 188] == 0x47)
        {
            //find PAT table
            index++;
            payload_start_flag = (p_data[index]  & 0x40) >> 7;
            ts_pid = ((p_data[index] & 0x1f) << 8) | p_data[index + 1];
            index += 2;
            if(ts_pid == PAT_PID && payload_start_flag == 1)
            {
                //the first section in pat table
                adaptation_field_control = (p_data[index] & 0x60) >> 4;
                index++;
                if(adaptation_field_control == 1)
                {
                    //parser pat table
                    pat_table_id = p_data[index];
                    if(pat_table_id == PAT_TABLE_ID)
                    {
                        index++;
                        pat_table_len = (p_data[index] & 0x0f) | p_data[index + 1];
                        index += 7;
                        pmt_data_len = pat_table_len - 9;
                        while(pmt_data_len > 0)
                        {
                            program_number = (p_data[index] << 8) | (p_data[index + 1]);
                            if(program_number == 0)
                            {
                                index += 4;
                            }
                            ...
                            {
                                pmt_table_id = ((p_data[index] & 0x1f) << 8) | (p_data[index + 1]); 
                                return pmt_table_id;
                            }                            
                        }
                    }
...
void ds_ads_find_pmt(u8 *p_data, u32 data_len, u8 pmt_pid, 
                                         u16 *v_pid, u16 *a_pid, u16 *pcr_pid)
{
    u32 index =0;
    u16 ts_pid = 0xffff;
    u32 read_data_len = 0;
    ...
    while(read_data_len > data_len)
    {
        ...
        if(p_data[index] == 0x47 && p_data[index + 188] == 0x47)
        {
            ...
            ts_pid = ((p_data[index] & 0x1f) << 8) | p_data[index + 1];
            index += 2;
            if(ts_pid == pmt_pid && payload_start_flag == 1)
            {
                //the first section in pmt table
                adaptation_field_control = (p_data[index] & 0x60) >> 4;
                index++;
                if(adaptation_field_control == 1)
                {
                    //parser pmt table
                    pmt_table_id = p_data[index];
                    if(pmt_table_id == PMT_TABLE_ID)
                    {
                        index++;
                        section_len = ((p_data[index] & 0x0f) << 8) | p_data[index + 1];
                        index += 7;
                        *pcr_pid = ((p_data[index] & 0x1f) << 8) | p_data[index + 1];
                        index += 2;
                        program_info_len = ((p_data[index] & 0x0f) << 8) | p_data[index + 1];
                        index += 2;
                        index += program_info_len;

                        av_info_len = section_len - 9 - 4 - program_info_len;
                        while(av_info_len > 0)
                        {
                            stream_type = p_data[index];
                            index++;
                            av_info_len--;
                            if(stream_type == 0x1 || stream_type == 0x2)
                            {
                                //video
                                *v_pid = ((p_data[index] & 0x1f) << 8) | p_data[index + 1];
                                index += 2;
                            }
                            else if(stream_type == 0x3 || stream_type == 0x4)
                            {
                                //audio
                                *a_pid = ((p_data[index] & 0x1f) << 8) | p_data[index + 1];
                                index += 2;
                            }

                            av_info_len -= 2;
                            
                            es_info_len = ((p_data[index] & 0x0f) << 8) | p_data[index + 1];
                            av_info_len--;

                            av_info_len -= es_info_len;                            
                        }
                    }
...
#if 0    
    //find pat table
    pmt_pid = ds_ads_find_pat(p_data, data_len);
    if(pmt_pid == 0xff)
    {
        ADS_DRV_PRINTF("[drv] %s %d ERR! \n",__func__,__LINE__);
        MT_ASSERT(0);
    }
    ADS_DRV_PRINTF("[drv] pmt_pid=0x%x \n",pmt_pid);

    //find pmt table
    ds_ads_find_pmt(p_data,data_len,pmt_pid,&v_pid,&a_pid);
#endif
...
void DSAD_ShowMessage(UINT8 ucMessageType, IN UINT8 * pucMessage)
{
    ...
    switch(ucMessageType)
    {
        case DSAD_MESSAGE_UPDATE_LOG_TYPE:
            ADS_DRV_PRINTF("[drv] MESSAGE_UPDATE_LOG_TYPE GOT \n");
...
#if 1
 //EXCEPT logo , OSD ,this func just for check the ads exit
 BOOL ui_adv_check_pic_ready(ads_info_t* p_ads_info ,ads_ad_type_t ads_ad_type)
 {
	 ST_DSAD_PROGRAM_PARA stProgramPara;
	 ST_DSAD_PROGRAM_SHOW_INFO pstProgramShow;
	 EN_DSAD_POS_TYPE ad_type = DSAD_STB_POS_UNKNOWN;
	 EN_DSAD_FUNCTION_TYPE ret = DSAD_FUN_OK;
 
	 BOOL b_ret = FALSE;
	 ad_type = desai_ad_type_get((u32)ads_ad_type);
	 if(NULL != p_ads_info)
	 {
		 stProgramPara.usNetworkId = p_ads_info->network_id;
		 stProgramPara.usTsId = p_ads_info->ts_id;
		 stProgramPara.usServiceId = p_ads_info->service_id;
	 }
	 switch(ad_type)
	 {
		 case DSAD_STB_LOG:
			 ret = DSAD_NoProgramGetElement(&pstProgramShow,ad_type);
			 if(DSAD_FUN_OK == ret )
				 b_ret = TRUE;
			 break;
		 case DSAD_STB_EPG:
			 ret = DSAD_ProgramGetElement(stProgramPara, &pstProgramShow,ad_type);
			 if(DSAD_FUN_OK == ret )
				 b_ret = TRUE;
			 break;
	...
	for(;;)
	{
	    ...
	    for(index = 0; index < DS_ADS_FILTER_MAX_NUM; index++)
	    {
	        ...
	        else
	        {
	            ...
	            {
	                OS_PRINTF("[drv] %s %d NIT data filtered out ,Parsing...\n",__func__,__LINE__);
	            ...
	            {
	                ADS_DRV_PRINTF("index=%d, channel_pid = 0x%x, data_len =%d ,callback_func=0x%x \n",index,ds_ads_filter_info[index].channel_pid,data_len,ds_ads_filter_info[index].callback_func);
	    			//OS_PRINTF("[drv] %s %d lock begin \n",__func__,__LINE__);
	    		   //OS_PRINTF("[drv] %s %d lock begin \n",__func__,__LINE__);
	    		   ds_ads_unlock();
	 		   		//OS_PRINTF("[drv] %s %d lock end  \n",__func__,__LINE__);
	 		   	   //ds_ad_data_debug(p_data,data_len);
	               ds_ads_filter_info[index].callback_func(ds_ads_filter_info[index].channel_pid,p_data, (u16)data_len);
	    		   //OS_PRINTF("[drv] %s %d lock begin \n",__func__,__LINE__);
	    		   ds_ads_lock();
	 		   		//OS_PRINTF("[drv] %s %d lock end  \n",__func__,__LINE__);
	                ds_ads_filter_info[index].start_time = mtos_ticks_get();
	 ...
	 desai_ads_check_all_pic();
			 ...
			 if(is_ap_playback_active())
			 {
				 ...
				 {
				   //OS_PRINTF("ADV CHECK ADS EXIT INFO: SID[%d] TS_ID[%d] NET_ID[%d]\n",pg.s_id, pg.ts_id,pg.orig_net_id);
				   ads_info.service_id = (u16)pg.s_id;
				   ads_info.ts_id = (u16)pg.ts_id;
				   ads_info.network_id= (u16)pg.orig_net_id;
				 }	
			 }
			 ui_adv_check_pic_ready(&ads_info,ADS_AD_TYPE_CHBAR);
			 for(cnt = 0;cnt < 7 ; cnt++)
			 {
				 OS_PRINTF("++++++++++ %d\t",ads_get_flag[cnt]);
			 }
			 OS_PRINTF(" ++++++++++\n");
#endif
