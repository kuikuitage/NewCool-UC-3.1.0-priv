...
EN_DSAD_POS_TYPE desai_ad_type_get(u32 ads_type)
{
...
RET_CODE ads_adt_desai_init(void *param)
{
	...
	ADS_ADT_PRINTF("[adt] Before DSAD_Init %s,%d \n",__func__,__LINE__);
	...
	ADS_ADT_PRINTF("[adt] After DSAD_Init %s,%d \n",__func__,__LINE__);
    if(ret == FALSE)
    {
...
RET_CODE ads_adt_desai_open(void)
{
    ...
    //parser NIT table
    OS_PRINTF("[drv]---Start to lock FRE!!!--- %s %d  \n",__func__,__LINE__);
	...
	ADS_ADT_PRINTF("[adt] Set filter nit table to get DSADS PID--- %s,%d \n",__func__,__LINE__);
    ...
    for(index = 0; index < GET_AD_PID_MAX_TIMES; index++)
    {
        ...
        {
            //set PID
            ADS_ADT_PRINTF("[adt] Get the DSADS PID and set to DSADS MODULE!!!---%s,%d \n",__func__,__LINE__);
...
extern u8 uc_current_osd_index;
RET_CODE ads_adt_desai_io_ctrl(u32 cmd, void *param)
{
    ...
    switch(cmd)
    {
        case ADS_IOCMD_AD_NO_PROGRAM:
			...
			ADS_ADT_PRINTF("[adt]Get noprogram desai ad type %d success!!! %s,%d \n",desai_ad_type,__func__,__LINE__);
            break;
        ...
        case ADS_IOCMD_AD_PROGRAM:
			...
			ADS_ADT_PRINTF("[adt]Get program desai ad type %d success!!! %s,%d \n",desai_ad_type,__func__,__LINE__);
            break;

        case ADS_IOCMD_AD_OSD:
            ADS_ADT_PRINTF("[adt]Trying to get OSD data!!! %s,%d \n",__func__,__LINE__);
            program_para.usNetworkId = p_ads_info->network_id;
            program_para.usTsId = p_ads_info->ts_id;
            program_para.usServiceId = p_ads_info->service_id;
            ret = DSAD_ProgramGetOSD(program_para, &osd_show,uc_current_osd_index);
            if(ret != DSAD_FUN_OK)
                break;
            memcpy((void *)p_ads_info->p_priv, (const void *)&osd_show, sizeof(ST_DSAD_OSD_PROGRAM_SHOW_INFO));
			ADS_ADT_PRINTF("[adt]Get noprogram desai ad type [OSD] %d success!!! %s,%d \n",desai_ad_type,__func__,__LINE__);
            break;
			
         case ADS_IOCMD_AD_VERSION_GET:
             ADS_ADT_PRINTF("[adt]Trying to get AD version!!! %s,%d \n",__func__,__LINE__);
