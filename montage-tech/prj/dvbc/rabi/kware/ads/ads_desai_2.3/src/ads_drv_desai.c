...
BOOL DSAD_RegisterTask (IN INT8* pucName, UINT8 ucPriority, IN void* pTaskFun )
{
    ...
    {
        ADS_DRV_PRINTF("[drv]DSAD_RegisterTask %s %d ERR! \n",__func__,__LINE__);
        MT_ASSERT(0);
    }
	else
	{
	    ...
	    ADS_DRV_PRINTF("[drv]DSAD_RegisterTask %s,%d \n",pucName,
...
void DSAD_ShowMessage(UINT8 ucMessageType, IN UINT8 * pucMessage)
{
    ...
    switch(ucMessageType)
    {
        ...
        case DSAD_MSEEAGE_UPDATEOSD_TYPE:
            OS_PRINTF("[drv] DSAD_MSEEAGE_UPDATEOSD_TYPE \n");
	...
	for(;;)
	{
	    ...
	    for(index = 0; index < DS_ADS_FILTER_MAX_NUM; index++)
	    {
	        ...
	        else
	        {
	            ...
	            {
	            		//OS_PRINTF("[drv] ds_ads_filter_info[%d].channel_pid=0x%x,callback_func = %x data_len = %d\n",index,
						//ds_ads_filter_info[index].channel_pid,
						//ds_ads_filter_info[index].callback_func,data_len);
					ds_ads_unlock();
...

					{
						...
						static EN_DSAD_POS_TYPE type =0;
						type =  type%DSAD_STB_POS_UNKNOWN +DSAD_STB_LOG;
						

						if(DSAD_FUN_OK == DSAD_ProgramGetElement(stProgramPara, &pstProgramShow,type) )
						{
						   //OS_PRINTF("[%s %d]:DSAD_ProgramGetElement  type[%d]\n",__FUNCTION__,__LINE__,type);
						}
						else if(DSAD_FUN_OK == DSAD_NoProgramGetElement(&pstProgramShow,type) )
						{
							//OS_PRINTF("[%s %d]:DSAD_NoProgramGetElement type[%d]\n",__FUNCTION__,__LINE__,type);
						}
						type++;
	            	}
