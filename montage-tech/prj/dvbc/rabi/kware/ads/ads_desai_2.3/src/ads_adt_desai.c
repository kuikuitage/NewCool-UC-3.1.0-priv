...
RET_CODE ads_adt_desai_io_ctrl(u32 cmd, void *param)
{
    ...
    switch(cmd)
    {
        case ADS_IOCMD_AD_NO_PROGRAM:
            ...
            {
            	OS_PRINTF("Current pic X,Y,W,H = %d,%d,%d,%d\n",
					program_show.stPictureShow.usStartX,
					program_show.stPictureShow.usStartY,
					program_show.stPictureShow.usWidth,
					program_show.stPictureShow.usHeigth);
        ...
        case ADS_IOCMD_AD_OSD:
            ...
            program_para.usTsId = p_ads_info->ts_id;
            program_para.usServiceId = p_ads_info->service_id;
            ret = DSAD_ProgramGetOSD(program_para, &osd_show, p_ads_info->cur_roll_index);
            if(ret != DSAD_FUN_OK)
                break;
