...
static BOOL is_net_entitlement_from_ca(SCDCAEntitle *entitle)
{
        u32 curTime, expTime;
		...
		CAS_YXSB_ConvertDate(entitle->m_tExpireDate, &yxsb_time);
        ...
        expTime = _mktime(yxsb_time.year,yxsb_time.month,yxsb_time.day,0,0,0);
		//expTime = _mktime(2016,12,12,0,0,0); //test

        DEBUG(DBG,INFO, "utc_time.year[%d]  utc_time.month[%d] utc_time.day[%d] \n",utc_time.year,utc_time.month,utc_time.day);
        DEBUG(DBG,INFO, "yxsb_time.year[%d]   yxsb_time.month[%d] yxsb_time.day[%d]\n",yxsb_time.year,yxsb_time.month,yxsb_time.day);

        if(expTime < curTime)
            return FALSE;

        DEBUG(DBG,INFO, "dvbca have net entitlement!\n");
        return TRUE;
}
...
BOOL get_vol_entitlement_from_ca(void)
{
        ...
        if(CDCA_RC_OK !=  CDCASTB_GetOperatorIds(wTVSIDs))
        {
        ...
        for(i = 0;i < CDCA_MAXNUM_OPERATOR && i<5;i++)
        {
 			//OS_PRINTF("%s ,%d wTVSIDs[i] = %d\n",__FUNCTION__,__LINE__,wTVSIDs[i]);
			if(CDCA_RC_OK == CDCASTB_GetServiceEntitles((CDCA_U16)wTVSIDs[i], &pCDEntitles))
            {
                for(j=0; j < pCDEntitles.m_wProductCount; j++)
                {
                    if(is_net_entitlement_from_ca(&pCDEntitles.m_Entitles[j]))
                        return TRUE;
                }
            }
        }
...
void send_event_to_ui_from_authorization(u32 event)
{
	...
	cas_send_event(p_priv->slot, CAS_ID_TF, event, 0);
    ...
    for(uIndex = 0; uIndex < NumberToRead; uIndex++)
    {
        ...
        switch(pCDIppv[uIndex].m_byBookEdFlag)
        {
            ...
            case CDCA_IPPVSTATUS_VIEWED:
                pIppvInfo[book_state_num].is_can_tape = pCDIppv[uIndex].m_bCanTape;
                pIppvInfo[book_state_num].book_state_flag = 1;
                
                pIppvInfo[book_state_num].product_id = pCDIppv[uIndex].m_dwProductID;
                pIppvInfo[book_state_num].price = pCDIppv[uIndex].m_wPrice;

                CAS_YXSB_ConvertDate(pCDIppv[uIndex].m_wExpiredDate, &yxsb_time);
                pIppvInfo[book_state_num].expired_time[0] = yxsb_time.year / 100;
                pIppvInfo[book_state_num].expired_time[1] = yxsb_time.year % 100;
                pIppvInfo[book_state_num].expired_time[2] = yxsb_time.month;
                pIppvInfo[book_state_num].expired_time[3] = yxsb_time.day;
                pIppvInfo[book_state_num].expired_time[4] = yxsb_time.hour;
                pIppvInfo[book_state_num].expired_time[5] = yxsb_time.minute;
                pIppvInfo[book_state_num].expired_time[6] = yxsb_time.second;

                pIppvInfo[book_state_num].burse_id = pCDIppv[uIndex].m_bySlotID;
                ...
                YXSB_ADT_DEBUG("m_wExpiredDate = %d, is_can_tape = %d, book_state_flag = %d, product_id = %d, price = %d, expired_time:%d-%d-%d %d-%d-%d, burst id = %d\n",
                                                    ...
                                                    pIppvInfo[book_state_num].is_can_tape,
                                                    pIppvInfo[book_state_num].book_state_flag,
                                                    pIppvInfo[book_state_num].product_id,
                                                    pIppvInfo[book_state_num].price,
                                                    pIppvInfo[book_state_num].expired_time[0] * 100 + pIppvInfo[book_state_num].expired_time[1],
                                                    pIppvInfo[book_state_num].expired_time[2],
                                                    pIppvInfo[book_state_num].expired_time[3],
                                                    pIppvInfo[book_state_num].expired_time[4],
                                                    pIppvInfo[book_state_num].expired_time[5],
                                                    pIppvInfo[book_state_num].expired_time[6],
                                                    pIppvInfo[book_state_num].burse_id);
                book_state_num ++;
                break;
            default:
...
static u32 YXSB_ADT_GetMailHeads(u16 *max_num, cas_mail_header_t *pMailHead)
{
    ...
    for(uIndex = 0; uIndex < uCount; uIndex++)
    {
        ...
        pMailHead[uIndex].m_id = CDHead[uIndex1].m_dwActionID;
        pMailHead[uIndex].new_email = CDHead[uIndex1].m_bNewEmail;
        memcpy(pMailHead[uIndex].subject, CDHead[uIndex1].m_szEmailHead, CDCA_MAXLEN_EMAIL_TITLE + 1);
        pMailHead[uIndex].subject[CDCA_MAXLEN_EMAIL_TITLE + 1] = 0;
        pMailHead[uIndex].priority = CDHead[uIndex1].m_wImportance;

        OS_PRINTF("[YXSB]mail_id = %d, create time = %d\n", CDHead[uIndex1].m_dwActionID, CDHead[uIndex1].m_tCreateTime);
        CAS_YXSB_ConvertU32Time(CDHead[uIndex1].m_tCreateTime, &yxsb_time);
...
static u32 YXSB_ADT_IppBuySet( ipp_buy_info_t *p_purch_info)
{
    ...
    if(NULL == p_purch_info)
        return ERR_FAILURE;
    if(p_purch_info->ipp_price[p_purch_info->index].price_type == CA_IPPVPRICETYPE_TPPNOTAP)
        m_Price.m_byPriceCode = CDCA_IPPVPRICETYPE_TPPVVIEW;
    else
        m_Price.m_byPriceCode = CDCA_IPPVPRICETYPE_TPPVVIEWTAPING;
    
    m_Price.m_wPrice = p_purch_info->ipp_price[p_purch_info->index].price;
...
static void YXSB_ADT_ParsePMT(u8 *p_buf, s_cas_yxsb_priv_t* p_priv)
{
    ...
    if(section_length > 0)
    {
          ...
          p_priv->pmt_ver_mun = version_number;
          ...
          while(section_length > 0)
          {
                ...
                //YXSB_ADT_DEBUG(" stream_type=0x%x, elementary_PID=0x%x\n", stream_type, elementary_pid);
...
static RET_CODE cas_yxsb_get_platformid(u16 *platformid)
{
    *platformid = CDCASTB_GetPlatformID();
    YXSB_ADT_DEBUG("[YXSB]CDCASTB_GetPlatformID:%d!\n", *platformid);
...
static RET_CODE cas_yxsb_io_ctrl(u32 cmd, void *param)
{
	...
	u8 pbyEmailNum = 0;
	u8 pbyEmptyNum = 0;
    s_cas_yxsb_priv_t *p_priv = g_cas_priv.cam_op[CAS_ID_TF].p_priv;
    ...
    switch(cmd)
    {
        ...
        case CAS_IOCMD_CARD_INFO_GET:
            ...
            wPlatformID = CDCASTB_GetPlatformID();
			YXSB_ADT_DEBUG("[YXSB]%s:LINE:%d,wPlatformID=  %x!\n", __FUNCTION__, __LINE__,wPlatformID);
            p_card_info->stb_sn[0] = (u8)(0xff & (wPlatformID >> 8));
            p_card_info->stb_sn[1] = (u8)(0xff & wPlatformID);
            p_card_info->stb_sn_len = 6;
            if(p_priv->machine_serial_get != NULL)
            {
                p_priv->machine_serial_get(&(p_card_info->stb_sn[2]), 4);
                OS_PRINTF("p_buf[0] = %X, p_buf[1] = %X, p_buf[2] = %X, p_buf[3] = %X, p_buf[4] = %X, p_buf[5] = %X\n",
                                                    p_card_info->stb_sn[0], p_card_info->stb_sn[1], 
                                                    p_card_info->stb_sn[2], p_card_info->stb_sn[3], 
                                                    p_card_info->stb_sn[4], p_card_info->stb_sn[5]);
            }
            break;
        ...
        case CAS_IOCMD_PATERNER_STA_GET:
            ...
            p_stb_paired_list->paired_status = CDCASTB_IsPaired(&p_stb_paired_list->paired_num,  p_stb_paired_list->STB_list); 
        ...
        case CAS_IOCMD_READ_FEED_DATA:
            ...
            YXSB_ADT_DEBUG("operator_id = %d\n", p_child_satus->operator_id);
...
static RET_CODE cas_yxsb_table_process(u32 t_id, u8 *p_buf, u32 *p_result)
{
    ...
    p_priv = (s_cas_yxsb_priv_t *)g_cas_priv.cam_op[CAS_ID_TF].p_priv;

    if (CAS_TID_PMT == t_id)
    {
        YXSB_ADT_ParsePMT(p_buf, p_priv);
        if(s_id == p_priv->program_id && pmt_ver_mun == p_priv->pmt_ver_mun)
        { 
            YXSB_ADT_DEBUG("[YXSB]pmt same\n");
            return SUCCESS;
        }
        else
        {
            s_id = p_priv->program_id;
            pmt_ver_mun = p_priv->pmt_ver_mun;
        }
        CDCASTB_SetEcmPid(CDCA_LIST_FIRST, NULL);
    ...
    {
        YXSB_ADT_DEBUG("[YXSB]nit process\n");
...
{
    s_cas_yxsb_priv_t *p_priv = NULL;
    g_cas_priv.cam_op[CAS_ID_TF].attached = 1;
    g_cas_priv.cam_op[CAS_ID_TF].card_reset_by_cam = 1;
    g_cas_priv.cam_op[CAS_ID_TF].inited = 0;

    g_cas_priv.cam_op[CAS_ID_TF].func.init = cas_yxsb_init;
    g_cas_priv.cam_op[CAS_ID_TF].func.deinit = cas_yxsb_deinit;
    g_cas_priv.cam_op[CAS_ID_TF].func.identify = cas_yxsb_identify;
    g_cas_priv.cam_op[CAS_ID_TF].func.io_ctrl = cas_yxsb_io_ctrl;
    g_cas_priv.cam_op[CAS_ID_TF].func.table_process = cas_yxsb_table_process;

    g_cas_priv.cam_op[CAS_ID_TF].func.card_remove = cas_yxsb_card_remove;
    g_cas_priv.cam_op[CAS_ID_TF].func.card_reset = cas_yxsb_card_reset;
    g_cas_priv.cam_op[CAS_ID_TF].func.get_platformid = cas_yxsb_get_platformid;
	
    p_priv = mtos_malloc(sizeof(s_cas_yxsb_priv_t));
	if(p_priv ==  NULL)
	{
		YXSB_ADT_DEBUG("[divi],p_priv malloc error ! \n");
		return ERR_FAILURE;
	}
    g_cas_priv.cam_op[CAS_ID_TF].p_priv = (s_cas_yxsb_priv_t *)p_priv;
    ...
    *p_cam_id = (u32)(&g_cas_priv.cam_op[CAS_ID_TF]);
