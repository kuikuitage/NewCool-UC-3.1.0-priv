...
{	
	...
	u16 index = 0;
	u32 uiNumber = 0;
	EN_ERR_CAS ret = CA_ERROR_UNKNOWN;
	ST_PRODUCT_ENTITLE  *p_ProductEntitles = NULL;

	u32 curTime, expTime;
	utc_time_t utc_time = {0};
	time_set_t p_set={{0}};
	
	DSCA_GetOperatorID(operatorId);
	ret = DSCA_GetProductNumber(uiOperatorID, (UINT32 *)&uiNumber);
	if(ret != CA_ERROR_OK || uiNumber == 0)
	{
		return FALSE;
	}

    	p_ProductEntitles = (ST_PRODUCT_ENTITLE  *)cas_porting_drv_base_malloc(uiNumber * sizeof(ST_PRODUCT_ENTITLE));
	if(p_ProductEntitles == NULL)
	{
		return FALSE;
	}
	if(DSCA_GetProductInfo(uiOperatorID, 0,uiNumber, p_ProductEntitles) != CA_ERROR_OK)
	{
		cas_porting_drv_base_free(p_ProductEntitles);
		return FALSE;
	}
	...
	for(index = 0; index < uiNumber; index++)
	{
		CAS_DS_PRINTF("[debug] %d-%d-%d \n",p_ProductEntitles[index].stEndTime.usYear,p_ProductEntitles[index].stEndTime.ucMonth,p_ProductEntitles[index].stEndTime.ucDay);
		CAS_DS_PRINTF("[debug] state = %d",p_ProductEntitles[index].ucEnableType);
		if(p_ProductEntitles[index].ucEnableType == 2)
		{
			expTime = _mktime(p_ProductEntitles[index].stEndTime.usYear,
							  p_ProductEntitles[index].stEndTime.ucMonth,
							  p_ProductEntitles[index].stEndTime.ucDay,0,0,0);
			if(expTime >= curTime)
			{
				cas_porting_drv_base_free(p_ProductEntitles);
				return TRUE;
...
static void ds_cas_parse_nit(u8 *p_buf, u32 uiLen)
{
    ...
    while (uiLoop > 0)
    {
        ...
        if (descriptor_tag == 0xa5 && descriptor_length == 7)
        {
			...
			CAS_DS_PRINTF("ds_cas_parse_nit:%x%x%x%x%x%x%x \n",p_data[0],p_data[1],
...
//    nim_device_t *p_nim_dev = NULL;
    ...
    switch(cmd)
    {
        ...
        case CAS_IOCMD_CARD_INFO_GET:
            ...
            //get card id
            ret = DSCA_GetCardNo((UINT8 *)p_card_info->sn, (UINT8 *)&p_card_info->card_type);
            if(ret != CA_ERROR_OK)
            {
