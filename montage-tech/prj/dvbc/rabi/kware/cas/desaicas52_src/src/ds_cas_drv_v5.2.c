...
#define DESAI_DRV_ERR OS_PRINTF
...
{   
    ...
    if(ret == SUCCESS)
    {
        ...
        {
            DESAI_DRV_ERR("[DS]DSCA_SendDataToSmartCard error! \n");
            MT_ASSERT(0);
        }
    ...
    {
        DESAI_DRV_ERR("T0 transaction failed! ret = %d\n", ret);
        ...
        if(card_update_flag == 1)
        {
            for (index = 0; index < 3; index ++)
            {
                ...
                if (SUCCESS == ret)
                {
                    ...
                    DESAI_DRV_ERR("card reset failed!\n");
            }
            card_update_flag = 0;
        }
    }
...
void DSCA_ShowMessage(UINT8 ucMessageType, EN_CAS_SHOW_MSG_TYPE  enShowType, IN UINT8 * pucMessage)
{
    ...
    DESAI_DRV_DEBUG_H("[DS]DSCA_ShowMessage,ucMessageType: 0x%x, enShowType: %d, pucMessage: %s \n", ucMessageType,enShowType,pucMessage);
    ...
    switch (ucMessageType)
    {
        ...
        case MESSAGE_CARD_UNWONTED_TYPE: /*智能卡内部异常 */
            DESAI_DRV_DEBUG_H("[DS]%s:LINE:%d, 智能卡内部异常！\n", __FUNCTION__, __LINE__);
