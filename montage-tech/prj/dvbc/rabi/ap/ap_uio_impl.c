...
static BOOL g_is_uiokey_disable = TRUE;
void set_uio_status(BOOL is_disable)
{
	g_is_uiokey_disable = is_disable;
}

BOOL get_uio_status(void)
{
...
BOOL ap_uio_translate_key(v_key_t *key)
{
  ...
  for(i = 0; i < key_num; i++)
  {
    ...
    if(temp->h_key == v_key_input)
    {
     ...
     OS_PRINTF("AP_UIO: one key translated(h_key: %x, v_key: %x)\n",
