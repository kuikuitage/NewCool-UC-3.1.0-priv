...
static void convert_perf(nim_channel_perf_t *p_signal, sig_mon_info_t *p_info)
{
 ...
 // convert_perf_hw(p_signal);
  ...
  p_info->strength = (u8)p_signal->agc;  //tmp remove
  p_info->snr = (u8)p_signal->snr;   
  p_info->ber = (u8)p_signal->ber;
}

/*
static void lock_led_set(BOOL locked,u8 tuner_id)
{
  uio_device_t *p_uio_dev = NULL;
  
  p_uio_dev = dev_find_identifier(NULL,DEV_IDT_TYPE, SYS_DEV_TYPE_UIO);
  dev_io_ctrl(p_uio_dev, UIO_FP_SET_LOCK_LBD, locked);

}
*/

extern void lock_led_set(BOOL locked,u8 tunerId);
