...
RET_CODE livetv_get_group_info(live_db_group_t *p_group, u8 group_view_id)
{
  live_db_priv_t *p_priv = get_priv();
...
RET_CODE livetv_get_pg_by_group_pos(live_db_prg_t *p_pg, u16 pos, u8 group_view_id)
{
  live_db_priv_t *p_priv = get_priv();
...
void livetv_clear_db(void)
{
 ...
 // p_priv->p_db->crc_ver = new_crc;
  ...
  memset(p_priv->p_db->grp_end_pos, 0, MAX_LIVE_GROUP * sizeof(u32));
