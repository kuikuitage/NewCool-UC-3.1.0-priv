...
typedef enum
{
  ...
  /*! A view contains all favorite TV programs */
  DB_DVBS_FAV_ALL_TV,
  /*! A view contains all favorite TV&radio programs */
  DB_DVBS_FAV_ALL,
  ...
  DB_DVBS_SAT_PG,
  DB_DVBS_SAT_ALL_TV,
  DB_DVBS_SAT_ALL_TV_FTA,
  DB_DVBS_SAT_ALL_TV_CAS,
  DB_DVBS_SAT_ALL_TV_HD,
  DB_DVBS_SAT_ALL_RADIO,
  DB_DVBS_SAT_ALL_RADIO_FTA,
  DB_DVBS_SAT_ALL_RADIO_CAS,
  DB_DVBS_SAT_ALL_RADIO_HD,
  DB_DVBS_ALL_TV_FTA,
  DB_DVBS_ALL_TV_CAS,
  DB_DVBS_ALL_TV_HD,
  DB_DVBS_FAV_TV_CAS,
  ...
  DB_DVBS_ALL_HIDE_TV,
  DB_DVBS_ALL_HIDE_RADIO,
  DB_DVBS_SAT_ALL_HIDE_TV,
  DB_DVBS_SAT_ALL_HIDE_RADIO,
  DB_DVBS_FAV_HIDE_TV,
  DB_DVBS_FAV_HIDE_RADIO,
  DB_DVBS_T2_SAT,
  DB_DVBS_CABLE_SAT,
  /*! Invaild view*/
...
/*!
 Add a new satellite into database

 \param[in,out] p_node: user satellite information, can't be NULL.
 \param[in] view id:  view id
 \return Function will return non zero values by following cases:
          1. datebase was full return DB_DVBS_FULL
          2. satellite name was duplicated return DB_DVBS_DUPLICATED
          Otherwise it'll return DB_DVBS_OK.
          Function will fill new satellite id in p_node->id.
 */
db_dvbs_ret_t db_dvbs_add_satellite(u8 view_id, sat_node_t *p_node);

/*!
  Edit a satellite information by satellite index
  \param[in] p_node An user satellite information, can't be NULL.
  \return Function will return DB_DVBS_DUPLICATED if new node information 
  is the same as old value in database. Return zero if successed. And function
  will assert failed by bad satellite index.
 */
db_dvbs_ret_t db_dvbs_edit_satellite(sat_node_t *p_node);
/*!
 Save edit to one satellite
 \param[in] p_sat_node: satellite information
 */
...
/*!
  Delete a satellite in database. This operation will cause all TPs and
  programs which belong to this satellite will be deleted.

  \param[in] view id : the view id associate certain satellite information
  \param[in] pos     : item position in view
  \return DB_DVBS_NOT_FOUND if can't find the satellite by id.
  Return DB_DVBS_OK if succeed.
 */
...
/*!
  ...
  \param[in,out] p_sat Satellite info
