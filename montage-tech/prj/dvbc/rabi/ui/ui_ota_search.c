...
static void trans_ota_nim_info(void)
{
  ...
  memcpy(&(ota_nim_lock_info.locks), &(ota_upgrade_information.locks), sizeof(ota_upgrade_information.locks));

  OS_PRINTF("%s(Line: %d): DVBC, ota_tp = %d, ota_sym = %d, ota_pid_ %d \n ", 
                    __FUNCTION__, __LINE__,ota_nim_lock_info.lockc.tp_freq, ota_nim_lock_info.lockc.tp_sym,
                      ota_nim_lock_info.data_pid);
  OS_PRINTF("%s(Line: %d): DVBT, ota_tp = %d, ota_pid_ %d \n ", 
                    __FUNCTION__, __LINE__,ota_nim_lock_info.lockt.tp_freq, ota_nim_lock_info.data_pid);
  return;
}
...
RET_CODE open_ota_search(u32 para1, u32 para2)
{
  ...
  p_cont = ui_comm_root_create(ROOT_ID_OTA_SEARCH, 0, 
                                COMM_BG_X, COMM_BG_Y, 
                                COMM_BG_W, COMM_BG_H, 
                                RSC_INVALID_ID, IDS_UPGRADE_BY_OTA);
