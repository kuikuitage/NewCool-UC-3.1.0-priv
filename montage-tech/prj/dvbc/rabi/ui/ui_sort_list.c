...
static list_field_attr_t sort_list_field_attr[SORT_LIST_FIELD_NUM] =
{
  { LISTFIELD_TYPE_STRID | STL_LEFT | STL_VCENTER,
    SORT_LIST_LIST_MIDW, 0, 0, &sort_list_item_rstyle, &sort_list_item_fstyle },
};
                               ...
                               u32 para2)
{
  ...
  //sort
  db_dvbs_pg_sort(view_id, focus);

  //close sort list and set plist state.
  is_tv = ((sys_status_get_curn_prog_mode() == CURN_MODE_TV)?TRUE:FALSE);
...
RET_CODE open_sort_list(u32 para1, u32 para2)
{
  ...
  ctrl_set_mrect(p_list, SORT_LIST_LIST_MIDL, SORT_LIST_LIST_MIDT,
                    SORT_LIST_LIST_MIDW+SORT_LIST_LIST_MIDL, SORT_LIST_LIST_MIDH+SORT_LIST_LIST_MIDT);
