                                    ...
                                    u16 rw)
{
  ...
  //u8 *class_name;
  ctrl_type_t class_type = CTRL_TCNT;
  ...
  {
    case COMM_STATIC:
      //class_name = "textfiled";//CTRL_TEXT;
      class_type = CTRL_TEXT;
      ...
      break;
    case COMM_SELECT:
      //class_name = "combobox";//CTRL_CBOX;
      class_type = CTRL_CBOX;
      ...
      break;
    case COMM_NUMSEL:
      //class_name = "container";//CTRL_CONT;
      class_type = CTRL_CONT;
      rsi_sh = RSI_COMM_SELECT_SH;
      rsi_hl = RSI_COMM_SELECT_HL;
      rsi_gray = RSI_COMM_SELECT_GRAY;
      keymap = NULL;
      ...
      break;
    case COMM_TIMEDIT:
     // class_name = "timebox";//CTRL_TBOX;
      class_type = CTRL_TBOX;
      ...
      break;
    case COMM_IPEDIT:
      //class_name = "ipbox";//CTRL_IPBOX;
      class_type = CTRL_IPBOX;
      ...
      break;
    case COMM_NUMEDIT:
      //class_name = "numbox";//CTRL_NBOX;
      class_type = CTRL_NBOX;
      ...
      break;
    case COMM_PWDEDIT:
     // class_name = "editbox";//CTRL_EBOX;
      class_type = CTRL_EBOX;
  ...
  p_ctrl = ctrl_create_ctrl(class_type, IDC_COMM_CTRL,
                            lw , 0, rw - COMM_CTRL_ICON_W, COMM_CTRL_H,
                            cont, 0);
  ...
  switch(type)
  {
    ...
    case COMM_T9EDIT:
      ...
      break;
   // case COMM_T9_V2_EDIT:
  //    ctrl_set_proc(p_ctrl, ui_comm_t9_v2_proc);
  //    break;
    case COMM_NUMSEL:
      ...
      nbox_set_align_type(p_ctrl_num, STL_CENTER | STL_VCENTER);
      ...
      p_ctrl_sel = ctrl_create_ctrl(CTRL_CBOX, IDC_COMM_EXT_SEL,
                            num_x + num_w, 0, rw - num_x - num_w- COMM_CTRL_ICON_W -18, 
      ...
      cbox_set_align_style(p_ctrl_sel, STL_LEFT | STL_VCENTER);
...
void ui_comm_ctrl_set_txt_font(control_t *cont,
                                 u32 n_fstyle,
                                 u32 h_fstyle,
                                 u32 g_fstyle)
{
  ...
  p_txt = ctrl_get_child_by_id(cont, IDC_COMM_TXT);
  text_set_font_style(p_txt, n_fstyle, h_fstyle, g_fstyle);
}
...
static void ui_comm_droplist_creat(control_t *p_ctrl_cbox, u16 page)
{
  ...
  droplist_y = (COMM_DROPLIST_ITEM_VGAP >> 1);
  while (droplist_y < COMM_DROPLIST_LIST_BORDER)
  {
    droplist_y += (page >> 1);
  }

  cbox_create_droplist(cbox, page, 6, 4);
  cbox_droplist_set_rstyle(cbox, RSI_POPUP_BG, RSI_POPUP_BG, RSI_POPUP_BG);
  cbox_droplist_set_mid_rect(cbox, 
                            COMM_DROPLIST_LIST_BORDER, droplist_y, 
                            RECTW(rc_cbox) - COMM_DROPLIST_LIST_BORDER - 16, RECTH(rc_cbox) * page - droplist_y - droplist_y, 
                            COMM_DROPLIST_ITEM_VGAP);
  cbox_droplist_set_item_rstyle(cbox, &dlist_item_rstyle);
  cbox_droplist_set_field_attr(cbox, STL_LEFT | STL_VCENTER,
                               RECTW(rc_cbox) - 16 - 16 - 16 - COMM_DROPLIST_LIST_BORDER, 16, 0);
...
void ui_comm_select_set_font(control_t *cont,
                                 u32 n_fstyle,
                                 u32 h_fstyle,
                                 u32 g_fstyle)
{
  ...
  p_ctrl = ctrl_get_child_by_id(cont, IDC_COMM_CTRL);
  cbox_set_font_style(p_ctrl, n_fstyle, h_fstyle, g_fstyle);
}
...
void ui_comm_numedit_set_font(control_t *cont,
                                 u32 n_fstyle,
                                 u32 h_fstyle,
                                 u32 g_fstyle)
{
  ...
  p_ctrl = ctrl_get_child_by_id(cont, IDC_COMM_CTRL);
  nbox_set_font_style(p_ctrl, n_fstyle, h_fstyle, g_fstyle);
}

static RET_CODE on_longitude_change(control_t *p_ctrl,
                                    u16 msg,
                                    u32 para1,
                                    u32 para2)
{
  ...
  if(nbox_is_on_edit(p_ctrl))
  {
    // pass the msg to base proc
    ret = ERR_NOFEATURE;
  }
  else
  {
    if(nbox_get_postfix(p_ctrl) == IDS_CHAR_W)
    {
      nbox_set_postfix_by_strid(p_ctrl, IDS_CHAR_E);
    }
    else
    {
      nbox_set_postfix_by_strid(p_ctrl, IDS_CHAR_W);
    }
    ctrl_paint_ctrl(p_ctrl, TRUE);
  }
...
{
  control_t *p_ctrl =
    _comm_ctrl_create(COMM_NUMEDIT, parent, ctrl_id, x, y, lw,
                      rw);

  MT_ASSERT(p_ctrl != NULL);

  ui_comm_numedit_set_param(p_ctrl,
                            NBOX_NUMTYPE_DEC | NBOX_ITEM_POSTFIX_TYPE_STRID,
                            0, 18000, 5, 0);

  ui_comm_numedit_set_decimal_places(p_ctrl, 2);
  ui_comm_ctrl_set_proc(p_ctrl, ui_comm_longitude_proc);
...
{
  ui_comm_numedit_set_postfix(p_ctrl,
                              IS_W_LONGITUDE(
                                longitude) ? IDS_CHAR_W : IDS_CHAR_E);

  LONGITUDE_NORMALIZE(longitude);
  ui_comm_numedit_set_num(p_ctrl, longitude);
}


u16 ui_comm_longitude_edit_get_num(control_t *p_ctrl)
{
  u16 longitude = (u16)ui_comm_numedit_get_num(p_ctrl);

  SET_LONGITUDE(longitude, ui_comm_numedit_get_postfix(p_ctrl) == IDS_CHAR_W);

  return longitude;
}

void ui_comm_longitude_edit_set_font(control_t *cont,
                                 u32 n_fstyle,
                                 u32 h_fstyle,
                                 u32 g_fstyle)
{
  ui_comm_numedit_set_font(cont, n_fstyle, h_fstyle, g_fstyle);
}

static RET_CODE on_latitude_change(control_t *p_ctrl,
                                   u16 msg,
                                   u32 para1,
                                   u32 para2)
{
  ...
  {
    if(nbox_get_postfix(p_ctrl) == IDS_CHAR_N)
    {
      nbox_set_postfix_by_strid(p_ctrl, IDS_CHAR_S);
    }
    else
    {
      nbox_set_postfix_by_strid(p_ctrl, IDS_CHAR_N);
    }
    ctrl_paint_ctrl(p_ctrl, TRUE);
  }
                                        ...
                                        u16 rw)
{
  ...
  ui_comm_numedit_set_decimal_places(p_ctrl, 2);
  ui_comm_ctrl_set_proc(p_ctrl, ui_comm_latitude_proc);
...
void ui_comm_latitude_edit_set_num(control_t *p_ctrl, u16 latitude)
{
  ui_comm_numedit_set_postfix(p_ctrl,
                              IS_S_LATITUDE(
                                latitude) ? IDS_CHAR_S : IDS_CHAR_N);

  LONGITUDE_NORMALIZE(latitude);
  ui_comm_numedit_set_num(p_ctrl, latitude);
}


u16 ui_comm_latitude_edit_get_num(control_t *p_ctrl)
{
  u16 latitude = (u16)ui_comm_numedit_get_num(p_ctrl);

  SET_LATITUDE(latitude, ui_comm_numedit_get_postfix(p_ctrl) == IDS_CHAR_S);
...
u32 ui_comm_pwdedit_get_realValue(control_t *p_ctrl)
{
  control_t *ebox;
  u16 buf[SS_PWD_MAX_LENGTH + 1];
  u16 *str = NULL;
  u8 leng;
  ...
  leng = uni_strlen(str);
  memcpy(buf, str, leng * sizeof(u16));
  buf[leng+1] = '\0';

  OS_PRINTF("[debug] str[%s] leng[%d]",buf, leng);
                              ...
                              u16 percent_h)
{
  ...
  text_set_content_by_ascstr(percent, "0%");
                                   ...
                                   u32 para2)
{
  ...
  if((msg&MSG_NUMBER) == MSG_NUMBER) //switch to number key process
  {
  ...
  else      //cbox is active
  {
    switch(msg)
    {
      case MSG_SELECT:
        return ERR_NOFEATURE;
      case MSG_INCREASE:
        if((IDC_COMM_EXT_SEL == ctrl_get_ctrl_id(p_active_child)) && cbox_droplist_is_opened(p_ctrl_sel))
          break;
        tmp_msg = MSG_FOCUS_RIGHT;
...
BEGIN_MSGPROC(ui_comm_longitude_proc, ui_comm_num_proc)
ON_COMMAND(MSG_FOCUS_LEFT, on_longitude_change)
ON_COMMAND(MSG_FOCUS_RIGHT, on_longitude_change)
END_MSGPROC(ui_comm_longitude_proc, ui_comm_num_proc)

BEGIN_MSGPROC(ui_comm_latitude_proc, ui_comm_num_proc)
ON_COMMAND(MSG_FOCUS_LEFT, on_latitude_change)
ON_COMMAND(MSG_FOCUS_RIGHT, on_latitude_change)
END_MSGPROC(ui_comm_latitude_proc, ui_comm_num_proc)
