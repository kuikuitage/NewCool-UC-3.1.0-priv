...
static BOOL ui_vdoplayer_enter_seek_mode(void)
{
    osd_set_t osd_set;

    if (g_pVdoPlayer->play_mode == VDO_PLAY_MODE_LIVE)
    {
        return FALSE;
    }
...
static BOOL ui_vdoplayer_exit_seek_mode(void)
{
...
static BOOL ui_vdoplayer_enter_pause_mode(void)
{
...
static BOOL ui_vdoplayer_exit_pause_mode(void)
{
    mul_fp_play_state_t _state;
    ...
    bmap_set_content_by_id(g_pVdoPlayer->hLargePlayIcon,  IM_XPLAY_CTRL_PLAY);
...
///////////////////////////////////////////////////////////////////
    ...
    g_pVdoPlayer->play_mode = p_param->play_mode;
...
///////////////////////////////////////////////////////////////////
    ...
    text_set_font_style(p_subCtrl, FSI_WHITE_22, FSI_WHITE_22, FSI_WHITE_22);
    text_set_content_type(p_subCtrl, TEXT_STRTYPE_UNICODE);
    if(NULL == g_pVdoPlayer->name)
    {
        text_set_content_by_ascstr(p_subCtrl, temp_name);
    }
    else
    {
        if(uni_strlen(g_pVdoPlayer->name))
        {
...
/////////////////////////////////////////////////////////////////////////////
    ...
    {
        ui_video_c_play_by_url(g_pVdoPlayer->pp_urlList[0], 0, g_pVdoPlayer->play_mode);
    }
    else
    {
        ui_video_c_play_by_url_ex(g_pVdoPlayer->pp_urlList, g_pVdoPlayer->url_cnt, 0, g_pVdoPlayer->play_mode);
    }
...
static RET_CODE ui_vdoplayer_on_seek(control_t *p_cont, u16 msg, u32 para1, u32 para2)
{
...
static RET_CODE ui_vdoplayer_on_show_goto(control_t *p_cont, u16 msg, u32 para1, u32 para2)
{
    ...
    else if (cur_state == MUL_PLAY_STATE_PAUSE)
    {
        manage_open_menu(ROOT_ID_VIDEO_GOTO, g_pVdoPlayer->play_time, ROOT_ID_VIDEO_PLAYER);
    }
...
{
    net_conn_stats_t net_connt_stats = ui_get_net_connect_status();

    if((msg == MSG_INTERNET_PLUG_OUT) || (msg == MSG_USB_INTERNET_PLUG_OUT))
    {
        if(net_connt_stats.is_usb_eth_conn || net_connt_stats.is_usb_eth_conn)
        {
            return SUCCESS;
        }
    }
...
BEGIN_MSGPROC(ui_vdoplayer_cont_proc, ui_comm_root_proc)
    ...
    ON_COMMAND(MSG_EXIT, ui_vdoplayer_on_cancel)
    ON_COMMAND(MSG_PAUSE, ui_vdoplayer_on_pause_resume)
    ON_COMMAND(MSG_SEEK, ui_vdoplayer_on_seek)
    ...
    ON_COMMAND(MSG_USB_INTERNET_PLUG_OUT, ui_vdoplayer_on_user_exit)
    ON_COMMAND(MSG_EXIT_ALL, ui_vdoplayer_on_user_exit)
