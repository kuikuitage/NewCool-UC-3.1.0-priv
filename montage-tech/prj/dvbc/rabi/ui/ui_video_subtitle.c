...
#define UI_SUBT_INVALID_ID (0xFF) 
...
#define UI_SUBT_TITLE_Y 10
#define UI_SUBT_TITLE_W 280
#define UI_SUBT_TITLE_H 40

#define UI_SUBT_LIST_X UI_SUBT_TITLE_X
#define UI_SUBT_LIST_Y (UI_SUBT_TITLE_Y + UI_SUBT_TITLE_H)
#define UI_SUBT_LIST_W UI_SUBT_TITLE_W
#define UI_SUBT_LIST_H  UI_SUBT_MAIN_H - UI_SUBT_TITLE_Y -UI_SUBT_TITLE_H -20
...
#define UI_SUBT_LIST_ITEM_V_GAP  4         
#define UI_SUBT_LIST_ITEM_H_GAP  4

#define UI_SUBT_SBAR_X (UI_SUBT_LIST_X + UI_SUBT_LIST_W)
#define UI_SUBT_SBAR_Y (UI_SUBT_LIST_Y + UI_SUBT_LIST_MID_T)
#define UI_SUBT_SBAR_W 6
#define UI_SUBT_SBAR_H (UI_SUBT_LIST_H  - 2*UI_SUBT_LIST_MID_T)
...
#define UI_SUBT_MAIN_X 100
#define UI_SUBT_MAIN_Y 50
#define UI_SUBT_MAIN_W 326
#define UI_SUBT_MAIN_H 330
...
static RET_CODE _ui_video_subt_list_update(control_t* p_ctrl, u16 start, u16 size, u32 context)
{
    ...
    u16 subt_lang[UI_SUBT_STR_LEN] = {0};       
    ...
    for(i = 0; i < cnt_i; i++)
    { 
        sprintf(asc_str, "[%02d] ", i + 1);
        list_set_field_content_by_ascstr(p_ctrl, i, 1, asc_str);

        ui_video_c_get_insert_sub_by_idx(i, subt_lang, UI_SUBT_STR_LEN);
    ...
    list_set_field_content_by_ascstr(p_ctrl, cnt_i + cnt_o, 1, "");
...
static void _ui_video_subt_create_list(control_t *p_cont, u16 cnt)
{
    ...
    ctrl_set_rstyle(p_list, RSI_FLIST_LIST, RSI_FLIST_LIST, RSI_FLIST_LIST);
    ctrl_set_keymap(p_list, _ui_video_subt_keymap);
    ctrl_set_proc(p_list, _ui_video_subt_proc);

    ctrl_set_mrect(p_list, UI_SUBT_LIST_MID_L, UI_SUBT_LIST_MID_T,
		UI_SUBT_LIST_MID_W+UI_SUBT_LIST_MID_L, UI_SUBT_LIST_MID_H+UI_SUBT_LIST_MID_T);
	list_set_item_interval(p_list, UI_SUBT_LIST_ITEM_V_GAP);                 
...
RET_CODE _ui_video_get_glyph(glyph_input_t *p_input, glyph_output_t *p_output)
{
  ...
  BOOL ret_boo = FALSE;
  handle_t p_rsc_hdl = get_rsc_handle();
...
static void _ui_video_subt_set_text_encode(void)
{
    ...
    ui_video_c_set_charset(lang_set.text_encode_video);
}
static void _ui_video_subt_show_by_focus(u16 focus)
{
    ...
    if(focus < cnt_i)
    {
        ui_video_c_set_playing_insert_sub_id(focus);
        ui_video_c_set_insert_sub_id_by_idx(focus);
        _ui_video_subt_set_attr(TRUE, NULL);
    }
    else
    {
        ui_video_c_set_playing_insert_sub_id(UI_SUBT_INVALID_ID);
...
void ui_video_subt_bg_init(void)
{
  ...
  for(i = 0; i < cnt_i; i++)
  {
    ...
    if((0 == STRCMPI(pp_lang_b[index], asc_str) )
      || (0 == STRCMPI(pp_lang_t[index], asc_str)))
    {
...
static RET_CODE _ui_video_subt_select(control_t *p_ctrl, u16 msg, u32 para1, u32 para2)
{
    ...
    playing_sub_id = ui_video_c_get_playing_insert_sub_id();
    if(focus != playing_sub_id)
    {
