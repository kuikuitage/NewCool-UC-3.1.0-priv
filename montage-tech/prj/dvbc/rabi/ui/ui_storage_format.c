...
RET_CODE storage_format_ntfs_proc(control_t * p_ctrl,
  u16 msg, u32 para1, u32 para2);
...
static void storage_format_set_detail(control_t * p_cont)
{
  ...
  {
    p_ctrl[i] = ctrl_get_child_by_id(p_cont, IDC_STORAGE_FORMAT_VOLUME+i);
  }
 ...
 ui_comm_static_set_content_by_unistr(p_ctrl[0],p_partition[focus].name); 
  ...
  {
    ui_comm_static_set_content_by_ascstr(p_ctrl[4], "FAT16");
  }
  else if(p_dev_info.fs_type == VFS_FAT32)
  {
    ui_comm_static_set_content_by_ascstr(p_ctrl[4], "FAT32");
  }
  else if(p_dev_info.fs_type == VFS_NTFS)
  {
    ui_comm_static_set_content_by_ascstr(p_ctrl[4], "NTFS");
  }
  else
  {
    ui_comm_static_set_content_by_ascstr(p_ctrl[4], "UNKNOW");
  }
...
RET_CODE open_storage_format(u32 para1, u32 para2)
{
  ...
  { IDS_PARTITION, IDS_TOTAL, IDS_FREE, IDS_USED,
  ...
  p_cont =ui_comm_right_root_create(ROOT_ID_STORAGE_FORMAT, ROOT_ID_XSYS_SET, OBJ_ATTR_ACTIVE, 0);
  ...
  ctrl_set_rstyle(p_hdd_btn,RSI_COMM_CONT_SH,RSI_SELECT_F,RSI_COMM_CONT_GRAY);
  ...
  ctrl_set_rstyle(p_dlg_cont, RSI_POPUP_BG, RSI_POPUP_BG, RSI_POPUP_BG);
  ...
  p_fat = ctrl_create_ctrl(CTRL_TEXT, IDC_STORAGE_FORMAT_CONFIRM_FAT,
                           ...
                           p_dlg_cont, 0);
  ctrl_set_rstyle(p_fat, RSI_ITEM_4_SH, RSI_ITEM_4_HL, RSI_ITEM_4_SH);
 ...
 /* //ntfs btn //cancel ntfs
  ...
  p_cancel= ctrl_create_ctrl(CTRL_TEXT, IDC_STORAGE_FORMAT_CONFIRM_CANCEL,
                             ...
                             p_dlg_cont, 0);
  ctrl_set_rstyle(p_cancel, RSI_ITEM_4_SH, RSI_ITEM_4_HL, RSI_ITEM_4_SH);
  ...
  u32 para1, u32 para2)
{
  ...
  if(usb_dev_type == HP_WIFI || usb_dev_type == HP_MODEM)
  {
