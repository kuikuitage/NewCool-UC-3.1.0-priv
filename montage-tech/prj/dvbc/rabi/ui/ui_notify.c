  ...
  else
  {
    ...
    if ((p_cont = fw_find_root_by_id(ROOT_ID_NOTIFY)) != NULL)//already open
    {
      ...
      else
      {
        ...
        fw_tmr_reset(ROOT_ID_NOTIFY, MSG_EXIT, NOTIFY_AUTOCLOSE_MS);
      }
    }
    else
    {
...
RET_CODE open_notify(u32 para1, u32 para2)
{
  ...
  p_cont = fw_create_mainwin(ROOT_ID_NOTIFY,
                                  g_notify_rc.left, g_notify_rc.top, 
  ...
  fw_tmr_create(ROOT_ID_NOTIFY, MSG_EXIT, NOTIFY_AUTOCLOSE_MS, FALSE);
#ifdef ENABLE_CA
