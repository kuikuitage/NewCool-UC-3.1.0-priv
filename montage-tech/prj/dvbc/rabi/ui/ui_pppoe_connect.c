...
static void create_pppoe_items(control_t *p_menu)
{
  ...
  for(i = 0;i < PPPOE_ITEM_CNT;i++)
  {
    ...
    {
      case 0:
        p_ctrl[i] = ui_comm_t9_v2_edit_create(p_menu, IDC_PPPOE_USR + i,PPPOE_EBOX_X,y,
      ...
      case 1:
        ...
        break;
      case 2:
        p_ctrl[i] = ui_comm_select_create(p_menu, IDC_PPPOE_USR + i,
                                    PPPOE_EBOX_X,y,
...
static void disconnect_pppoe(int ppp_fd)
{
  ...
  pppoe_connt_stats.is_usb_eth_conn = FALSE;
...
static void ui_lwip_init_pppoe(ethernet_device_t *eth_dev)
{
  ...
  if(pppoe_connt_stats.is_eth_insert)
  {
...
static void get_ctrl_strings(u16 ctrl_idc, u8 *asc)
{
  ...
  p_ctrl = ui_comm_root_get_ctrl(ROOT_ID_PPPOE_CONNECT, ctrl_idc);
...
static RET_CODE on_pppoe_connect(control_t *p_cont, u16 msg, u32 para1, u32 para2)
{
  ...
  pppSetAuth(PPPAUTHTYPE_ANY, usr_asc, pwd_asc);
  ...
  if(ppp_fd == 0)
  {
   ...
   // sys_status_save();
    ...
    {
      pppoe_connt_stats.is_usb_eth_conn = TRUE;
    }
  }
  else
  {
...
{
  is_conn_pppoe = FALSE;
  if(pppoe_connt_stats.is_eth_conn)
  {
    pppoe_connt_stats.is_eth_conn = FALSE;
    ui_set_net_connect_status(pppoe_connt_stats);
  }
...
static RET_CODE on_pppoe_usb_net_cable_plug_out(control_t *p_ctrl, u16 msg,
                           u32 para1, u32 para2)
{
  is_conn_pppoe = FALSE;
  if(pppoe_connt_stats.is_usb_eth_conn)
  {
    pppoe_connt_stats.is_usb_eth_conn = FALSE;
    ui_set_net_connect_status(pppoe_connt_stats);
  }
  OS_PRINTF("###########at pppoe ui, the usb net cable plug out########\n");
...
BEGIN_MSGPROC(pppoe_cont_proc, cont_class_proc)
  ...
  ON_COMMAND(MSG_USB_INTERNET_PLUG_OUT, on_pppoe_usb_net_cable_plug_out)
