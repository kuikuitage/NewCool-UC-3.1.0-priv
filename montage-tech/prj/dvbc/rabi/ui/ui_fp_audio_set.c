...
static RET_CODE on_fp_audio_set_comm_select(control_t *p_ctrl, u16 msg, u32 para1, u32 para2)
{
  ...
  {
    case IDC_FP_CHANNEL:
      //ui_set_audio_channel();
      ret = ui_video_c_change_audio_track(tmp);
...
RET_CODE open_fp_audio_set(u32 para1, u32 para2)
{
  ...
  p_txt = ctrl_create_ctrl(CTRL_TEXT, IDC_FP_TITLE,
                           ...
                           p_cont, 0);
  text_set_align_type(p_txt, STL_CENTER | STL_BOTTOM);
  ...
  // fill info
  ui_comm_select_set_focus(p_ctrl[0], fp_prog.audio_ch_num);
  ui_comm_select_set_focus(p_ctrl[1], fp_prog.audio_type);
