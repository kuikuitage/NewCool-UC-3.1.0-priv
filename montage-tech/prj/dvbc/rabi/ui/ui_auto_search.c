...
{
  IDC_INVALID = 0,
  IDC_AUTO_SEARCH_FREQ,
  IDC_AUTO_SEARCH_SYM,
  IDC_AUTO_SEARCH_DEMOD,
  IDC_AUTO_SEARCH_START,
};
...
static RET_CODE auto_search_cont_proc(control_t *cont, u16 msg, u32 para1,
                                            u32 para2);

static RET_CODE auto_search_cbox_proc(control_t *p_cbox, u16 msg, u32 para1,
                                            u32 para2);
static RET_CODE auto_search_nbox_proc(control_t *p_cbox, u16 msg, u32 para1,
                                            u32 para2);
...
static void auto_search_set_transpond(control_t *p_cont)
{
  dvbs_tp_node_t tp = {0};
  control_t *p_ctrl;
  u32 freq;
  ...
  p_ctrl = ctrl_get_child_by_id(p_cont, IDC_AUTO_SEARCH_FREQ);
  freq = ui_comm_numedit_get_num(p_ctrl);

  p_ctrl = ctrl_get_child_by_id(p_cont, IDC_AUTO_SEARCH_SYM);
  sym = ui_comm_numedit_get_num(p_ctrl);

  p_ctrl = ctrl_get_child_by_id(p_cont, IDC_AUTO_SEARCH_DEMOD);
...
static RET_CODE start_auto_scan(control_t *cont, u16 msg, u32 para1, u32 para2)
{
  ...
  p_ctrl = ctrl_get_child_by_id(cont, IDC_AUTO_SEARCH_FREQ);
  freq = ui_comm_numedit_get_num(p_ctrl);

  p_ctrl = ctrl_get_child_by_id(cont, IDC_AUTO_SEARCH_SYM);
  sym = ui_comm_numedit_get_num(p_ctrl);

  p_ctrl = ctrl_get_child_by_id(cont, IDC_AUTO_SEARCH_DEMOD);
  demod = (u8)ui_comm_select_get_focus(p_ctrl);
...
{
  control_t *p_cont, *p_ctrl[AUTO_SEARCH_ITEM_CNT];
  u8 i = 0;
  u16 stxt [AUTO_SEARCH_ITEM_CNT] = {IDS_FREQUECY, IDS_SYMBOL, IDS_MODULATION, IDS_SEARCH};
  u16 y;
  dvbc_lock_t tp = {0};
  sys_status_get_main_tp1(&tp);

  p_cont = ui_comm_root_create_with_signbar(ROOT_ID_AUTO_SEARCH, 0,
                                  COMM_BG_X, COMM_BG_Y,
                                  COMM_BG_W, COMM_BG_H,
                                  IM_ARROW1_L, IDS_AUTOL_SEARCH, FALSE);
  ...
  for (i = 0; i < AUTO_SEARCH_ITEM_CNT; i++)
  {
    switch (i)
    {
      case 0:
      case 1:
        p_ctrl[i] = ui_comm_numedit_create(p_cont, (u8)(IDC_AUTO_SEARCH_FREQ + i),
          AUTO_SEARCH_ITEM_X, y, AUTO_SEARCH_ITEM_LW, AUTO_SEARCH_ITEM_RW);
        ui_comm_numedit_set_static_txt(p_ctrl[i], stxt[i]);
        ui_comm_ctrl_set_proc(p_ctrl[i], auto_search_nbox_proc);
        if(i == 0)
        {
        ...
        break;
      case 2:
        p_ctrl[i] = ui_comm_select_create(p_cont, (u8)(IDC_AUTO_SEARCH_FREQ + i),
          AUTO_SEARCH_ITEM_X, y, AUTO_SEARCH_ITEM_LW, AUTO_SEARCH_ITEM_RW);
        ui_comm_select_set_static_txt(p_ctrl[i], stxt[i]);
        ui_comm_ctrl_set_proc(p_ctrl[i], auto_search_cbox_proc);
        ...
        break;
      case 3:
        p_ctrl[i] = ctrl_create_ctrl(CTRL_TEXT, (u8)(IDC_AUTO_SEARCH_FREQ + i), 
    ...
    ctrl_set_related_id(p_ctrl[i],
                        0,                                     /* left */
                        (u8)((i - 1 +
                              AUTO_SEARCH_ITEM_CNT) %
                             AUTO_SEARCH_ITEM_CNT + 1),            /* up */
                        0,                                     /* right */
...
//combobox
static RET_CODE on_auto_search_cbox_changed(control_t *p_cbox, u16 msg, u32 para1,
                             	   u32 para2)
{
  cbox_class_proc(p_cbox, msg, para1, para2);
  auto_search_set_transpond(ctrl_get_parent(ctrl_get_parent(p_cbox)));
...
static RET_CODE on_auto_search_change_freq_symbol(control_t *p_ctrl, u16 msg, u32 para1, u32 para2)
{
  ...
  auto_search_set_transpond(p_ctrl->p_parent->p_parent);
  return SUCCESS;
}
...
static RET_CODE on_auto_search_destroy(control_t *cont, u16 msg, u32 para1,
                            u32 para2)
{
...
BEGIN_MSGPROC(auto_search_cont_proc, ui_comm_root_proc)
  ON_COMMAND(MSG_DESTROY, on_auto_search_destroy)
END_MSGPROC(auto_search_cont_proc, ui_comm_root_proc)

BEGIN_MSGPROC(auto_search_cbox_proc, cbox_class_proc)
	ON_COMMAND(MSG_CHANGED, on_auto_search_cbox_changed)
END_MSGPROC(auto_search_cbox_proc, cbox_class_proc)

BEGIN_MSGPROC(auto_search_nbox_proc, ui_comm_num_proc)
  ON_COMMAND(MSG_NUMBER, on_auto_search_change_freq_symbol)
END_MSGPROC(auto_search_nbox_proc, ui_comm_num_proc)
