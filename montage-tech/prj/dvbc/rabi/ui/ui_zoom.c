...
static rect_t g_tvdst_fullrc = {0};
...
static void fill_zoom_info(control_t *cont)
{
  ...
  sprintf(buf, "ZOOM %dX", g_zoom_x[g_zoom_curn]);
  text_set_content_by_ascstr(ctrl, buf);
  ctrl_paint_ctrl(ctrl, TRUE);

  ctrl = ctrl_get_child_by_id(cont, IDC_ARROW_L);
  enable_icon(ctrl, rc_icon.left > ZOOM_BOUND_X);

  ctrl = ctrl_get_child_by_id(cont, IDC_ARROW_T);
  enable_icon(ctrl, rc_icon.top > ZOOM_BOUND_Y);

  ctrl = ctrl_get_child_by_id(cont, IDC_ARROW_R);
  enable_icon(ctrl, rc_icon.right < ZOOM_BOUND_X + ZOOM_BOUND_W);

  ctrl = ctrl_get_child_by_id(cont, IDC_ARROW_B);
  enable_icon(ctrl, rc_icon.bottom < ZOOM_BOUND_Y + ZOOM_BOUND_H);
...
//  g_tvsrc_fullrc.right = 1280;
//  g_tvsrc_fullrc.bottom = 720;
  g_tvdst_fullrc.right = 1280;
  g_tvdst_fullrc.bottom = 720;
#if 0
  disp_get_tv_sys(g_display_dev, DISP_CHANNEL_SD, &disp_sys);

  g_tvdst_fullrc.right = 1280;
  if((disp_sys == VID_SYS_PAL)
    || (disp_sys == VID_SYS_PAL_N))
  {
    g_tvdst_fullrc.bottom = 720;
  }
  else
  ...
  text_set_content_by_ascstr(p_ctrl, "ZOOM 1X");
  ...
  ctrl_set_rstyle(p_ctrl, RSI_ZOOM_BOUND,
                  RSI_ZOOM_BOUND, RSI_ZOOM_BOUND);
  bmap_set_content_by_id(p_ctrl, IM_ZOOM);
  ...
  bmap_set_content_by_id(p_ctrl, IM_ZOOM_ICON_T);
  ...
  p_ctrl = ctrl_create_ctrl(CTRL_BMAP, IDC_ARROW_R,
                            ...
                            p_cont, 0);
  ctrl_set_sts(p_ctrl, OBJ_STS_HIDE);
  bmap_set_content_by_id(p_ctrl, IM_ZOOM_ICON_R);
  ...
  p_ctrl = ctrl_create_ctrl(CTRL_BMAP, IDC_ARROW_B,
                            ...
                            p_cont, 0);
  ctrl_set_sts(p_ctrl, OBJ_STS_HIDE);
  bmap_set_content_by_id(p_ctrl, IM_ZOOM_ICON_D);
  ...
  p_ctrl = ctrl_create_ctrl(CTRL_BMAP, IDC_ARROW_L,
                            ...
                            p_cont, 0);
  ctrl_set_sts(p_ctrl, OBJ_STS_HIDE);
  bmap_set_content_by_id(p_ctrl, IM_ZOOM_ICON_L);
