...
static RET_CODE on_prog_detail_close(control_t *p_cont, u16 msg, u32 para1,
									 u32 para2)
{
...
RET_CODE open_prog_detail(u32 para1, u32 para2)
{
  ...
  dvbs_prog_node_t pg;
  #ifndef IMPL_NEW_EPG
  ext_evt_desc_uni_t *p_evt_desc = NULL;
  #else
  ...
  p_detail = ctrl_create_ctrl(CTRL_TEXT, IDC_PDETAIL_DTL,
   PDETAIL_DETAIL_X,PDETAIL_DETAIL_Y,PDETAIL_DETAIL_W,PDETAIL_DETAIL_H,
    p_cont, 0);
  ...
  ctrl_set_mrect(p_detail, PDETAIL_DETAIL_MIDX, PDETAIL_DETAIL_MIDY,
  892,152 );
  ...
  {
    uni_strncpy(p_ext_detail, p_evt_node->p_sht_txt, PDETAIL_MAX_LENTH);
  ...
  for(i = 0; i < p_evt_node->ext_desc_num; i++)
  {
    ...
    {
      uni_strcat(p_ext_detail, p_evt_desc->item[j].item_name, PDETAIL_MAX_LENTH);
      uni_strcat(p_ext_detail, uni_str2, PDETAIL_MAX_LENTH);        
      uni_strcat(p_ext_detail, p_evt_desc->item[j].item_cont, PDETAIL_MAX_LENTH);
      uni_strcat(p_ext_detail, uni_str1, PDETAIL_MAX_LENTH);
    }
  ...
  {
    uni_strcat(p_ext_detail, p_evt_node->p_ext_txt, PDETAIL_MAX_LENTH);
  }
