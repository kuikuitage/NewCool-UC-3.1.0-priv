...
static url_type_t g_url_type = NUMBER;
static rename_param_t upgrade_by_net_url_param = { NULL,32,KB_INPUT_TYPE_SENTENCE};
static rename_param_t upgrade_by_net_usr_param = { NULL,32,KB_INPUT_TYPE_SENTENCE};
static rename_param_t upgrade_by_net_pwd_param = { NULL,32,KB_INPUT_TYPE_SENTENCE};
/*************************************************************
    ************* ********create UI  func***********************
*************************************************************/
...
static void create_net_upg_items(control_t *p_menu)
{
  ...
  for (i = 0; i < UPGRADE_BY_NET_ITEM_CNT; i++)
  {
    ...
    switch (i)
    {
      ...
      case 2:
        ...
        ui_comm_ipedit_set_address(p_num_url, &ip_address);
        ...
        ctrl_set_sts(p_num_url, OBJ_STS_SHOW);
        ...
        ui_comm_t9_v2_edit_set_content_by_ascstr(p_ctrl[i], "www.set-box-tech.com");
...
/*************************************************************
    ************* *******url item proc func********************
*************************************************************/
void edit_upg_by_net_url_update(void)
{
...
/*************************************************************
    ************* *******user item proc func********************
*************************************************************/
void edit_upg_by_net_usr_update(void)
{
  control_t *p_edit_cont;
...
/*************************************************************
    ************* *******pwd item proc func********************
*************************************************************/
void edit_upg_by_net_pwd_update(void)
{
  control_t *p_edit_cont;
...
static RET_CODE on_upg_by_net_start(control_t *p_ctrl, u16 msg, u32 para1, u32 para2)
{
  ...
  ip_address_t ip_address = {0, 0, 0, 0};
#if 1
  u8 usr_asc_temp[32] = {'t', 0};
  u8 pwd_asc_temp[32] = {'1', '2', '3', 0};
#ifdef WIN32
ip_address_t ip_address_temp = {192, 168, 8, 80};
#else
ip_address_t ip_address_temp = {192, 168, 130, 2};
#endif
#endif
  u8 path[256] = {0};
  switch(g_protocol)
