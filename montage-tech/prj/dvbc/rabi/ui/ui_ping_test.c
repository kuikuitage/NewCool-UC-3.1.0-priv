...
RET_CODE open_ping_test(u32 para1, u32 para2)
{
  ...
  u8 i = 0;
  p_menu = ui_comm_right_root_create(ROOT_ID_PING_TEST, ROOT_ID_XSYS_SET, OBJ_ATTR_ACTIVE, 0);
     ...
     switch(i)
      {
        case 0:
          ...
          ui_comm_t9_v2_edit_set_content_by_ascstr(p_ctrl[i], "www.baidu.com");
  ...
  ctrl_set_rstyle(p_dlg_cont, RSI_PBACK, RSI_PBACK, RSI_PBACK);
...
static void do_cmd_to_ping_test(ethernet_device_t * p_dev, s32 ping_count, ip_address_t *addr, u8 *url_str)
{
  ...
  net_svc_para.ping_para.count = ping_count;
...
static void get_url_strings(u8 *asc)
{
  ...
  p_ctrl = ui_comm_right_root_get_ctrl(ROOT_ID_PING_TEST, IDC_IP_ADDR);
  ...
  u32 para1, u32 para2)
{
  ...
  ping_count = ui_comm_numedit_get_num(p_times);
 ...
 // text_set_content_by_dec(p_ctrl, (s32)result.len);
  ...
  if(!eth_connt_stats.is_eth_conn &&
     !eth_connt_stats.is_usb_eth_conn &&
     !eth_connt_stats.is_wifi_conn &&
     !eth_connt_stats.is_gprs_conn &&
     !eth_connt_stats.is_3g_conn)
  {
  ...
  u32 para1, u32 para2)
{
  ...
  DEBUG(MAIN, INFO, "update ping test reesult, ok is %d\n", para1);
  p_dlg_container = ctrl_get_child_by_id(p_sub->p_parent, IDC_DLG_CONT);
  p_times = ctrl_get_child_by_id(p_sub->p_parent, IDC_TIMES);
...
RET_CODE edit_ping_test_url_update(u16* p_unistr)
{
  ...
  p_edit_cont = ui_comm_right_root_get_ctrl(ROOT_ID_PING_TEST, IDC_IP_ADDR);
