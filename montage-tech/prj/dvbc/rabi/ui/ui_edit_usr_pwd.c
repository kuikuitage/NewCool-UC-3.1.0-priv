...
extern RET_CODE pnp_svc_mount(u16 *p_url);
...
RET_CODE open_edit_usr_pwd(u32 para1, u32 para2)
{
  ...
  sys_status_get_ip_account(g_index, ip_account);
  sys_status_get_ip_passwd(g_index, ip_passwd);
  ...
  for(i = 0;i < USR_PWD_ITEM_CNT;i++)
  {
    switch(i)
      {
      case 0:
        p_ctrl[i] = ui_comm_t9edit_create(p_cont, IDC_EDIT_USR+i,
                                    EDIT_USR_PWD_EBOX_X,
                                    (EDIT_USR_PWD_EBOX_Y+i*(EDIT_USR_PWD_EBOX_H+EDIT_USR_PWD_ITEM_VGAP)),
   ...
   ebox_set_content_by_ascstr(p_edit_box, ip_passwd);
...
static RET_CODE on_edit_cancel(control_t *p_cont, u16 msg, u32 para1, u32 para2)
{
  ...
  BOOL  ui_get_ip_service_with_path(char *ipaddress,char *service)
  {
    ...
    memcpy(ip_address_with_path_temp,ip_address_with_path,64);
      ...
      str_uni2asc( ipaddress, ipaddress_temp);
       str_uni2asc(service, service_temp);
       return TRUE;
     }
     else
     {
...
RET_CODE edit_usr_box_update(u16 *p_unistr)
{
 ...
 u8 test[32];
  ...
  sys_status_set_ip_account(g_index, test);
  return SUCCESS;
}
...
RET_CODE edit_pwd_box_update(u16 *p_unistr)
{
  ...
  ctrl_paint_ctrl(p_edit_cont, TRUE);
  str_uni2asc(test, p_unistr);
  sys_status_set_ip_passwd(g_index, test);
  return SUCCESS;
}
