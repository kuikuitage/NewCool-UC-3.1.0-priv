 ...
 */
static control_t * _ui_video_goto_creat_cont(u32 parent_root_id)
{
  control_t *p_cont = NULL;
  
  p_cont = fw_create_mainwin(ROOT_ID_VIDEO_GOTO, 
                                VIDEO_GOTO_CONT_X, VIDEO_GOTO_CONT_Y, 
                                VIDEO_GOTO_CONT_W, VIDEO_GOTO_CONT_H,
                                (u8) parent_root_id, 0,
                                OBJ_ATTR_ACTIVE, STL_EX_WHOLE_HL);
...
static control_t * _ui_video_goto_creat_text(control_t *p_cont)
{
  ...
  text_set_font_style(p_txt, FSI_WHITE, FSI_BLACK, FSI_GRAY);
...
RET_CODE open_video_goto(u32 para1, u32 para2)
{
  ...
  /*Create Menu*/
  p_cont = _ui_video_goto_creat_cont(para2);
  MT_ASSERT(p_cont != NULL);

  /*Create text*/
  p_txt = _ui_video_goto_creat_text(p_cont);
...
#if ENABLE_NETWORK
      else if(NULL != fw_find_root_by_id(ROOT_ID_VIDEO_PLAYER))
      {
        play_sec = ui_video_c_get_play_time();
      }
#endif
      
      //ui_video_c_seek(jump_sec - play_sec);
                            ...
                            u32 para1, u32 para2)
{
  ...
  {
    tbox_get_time(p_ctrl, &time);

    if(is_invalid_time(&time))
    {
      tbox_set_time(p_ctrl, &time);
      ctrl_paint_ctrl(p_ctrl, TRUE);
      return SUCCESS;
    }          
  }
  
  _video_goto_update(&time);
  if(NULL != fw_find_root_by_id(ROOT_ID_FILEPLAY_BAR))
  {
    fw_notify_parent(ROOT_ID_VIDEO_GOTO, NOTIFY_T_MSG, FALSE,
                   MSG_UP_FILEBAR, 0, 0);
  }
  if(NULL != fw_find_root_by_id(ROOT_ID_VIDEO_PLAYER))
  {
    fw_notify_parent(ROOT_ID_VIDEO_GOTO, NOTIFY_T_MSG, FALSE,
                   MSG_GOTO_DONE, 0, 0);
  }
