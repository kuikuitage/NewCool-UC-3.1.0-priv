...
BOOL have_logic_number()
{
  ...
  if(lcn_flag == FALSE)
  {
    OS_PRINTF("have not logic number\n");
    return FALSE;
  }
  else
  {
    OS_PRINTF("have logic number\n");
    return TRUE;
  }
}
...
u16 get_max_logic_number(void)
{
  ...
  for(i=0; i<pg_count; i++)
  {
    ...
    if(prog.logical_num)
    {
      if(temp < prog.logical_num)
        temp = prog.logical_num;
    }
  }
...
void parse_nit_india(handle_t handle, dvb_section_t *p_sec)
{
  ...
  while(network_desc_len > 0)
  {
    ...
    {
      u8  bqt_data_length = 0;
      u8  bqt_data_offset = 0;
