...
static BOOL ft_get_char(void *p_private,
  u16 char_code, rsc_fstyle_t *p_fstyle, rsc_char_info_t *p_info)
{
  ...
  u8 *p_font = NULL;
  ft_private_t *p_ft_private = (ft_private_t*)p_private;

  MT_ASSERT(p_private != NULL);
  if((p_fstyle->width > p_ft_private->ft_prv2.max_width)
  				||(p_fstyle->height> p_ft_private->ft_prv2.max_height))
  {
	p_vf = &(p_ft_private->ft_prv1);
  }
  else
  {
	p_vf = &(p_ft_private->ft_prv2);
  }
  p_priv = (void*)p_vf;
...
static void vf_ft_cache_init(ft_priv_t *p_priv, vf_ft_cfg_t *p_cfg, handle_t rsc_handle)
{
	u32 i = 0;
	RET_CODE ret = SUCCESS;
#ifndef WIN32
...
void vf_ft_attach(handle_t rsc_handle, vf_ft_config_t *p_cfg)
{
  rsc_main_t *p_rsc_info = NULL;
  ft_private_t *p_priv = NULL;

  p_rsc_info = (rsc_main_t *)rsc_handle;
  MT_ASSERT(p_rsc_info != NULL);
  
  p_priv = (ft_private_t *)mmi_alloc_buf(sizeof(ft_private_t));
  MT_ASSERT(p_priv != NULL);
  memset(p_priv, 0, sizeof(ft_private_t));
  
  p_rsc_info->p_priv = p_priv;

	vf_ft_cache_init(&(p_priv->ft_prv1), &(p_cfg->vf_ft1), rsc_handle);
	vf_ft_cache_init(&(p_priv->ft_prv2), &(p_cfg->vf_ft2), rsc_handle);
  //attach function.
...
static void vf_ft_cache_deinit(ft_priv_t *p_priv)
{
	u32 i = 0;
#ifndef WIN32
	chip_ic_t chip_ic = hal_get_chip_ic_id();
...
void vf_ft_detach(handle_t res_handle)
{
  ...
  p_private = (ft_private_t *)p_res_info->p_priv;
  vf_ft_cache_deinit(&(p_private->ft_prv1));
  vf_ft_cache_deinit(&(p_private->ft_prv2));
  if(NULL!=p_private)
  {
  	mmi_free_buf((void *)p_private);
  	p_private = NULL;
  }
}
