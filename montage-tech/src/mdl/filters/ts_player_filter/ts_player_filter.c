...
static void update_play_time(handle_t _this)
{
  ...
  if (vdec_info.pts)
  {
    ...
    {
      int tmp_play_time = (play_time / 1000) * 1000;
      int start_play_time = (mtos_ticks_get() / 100 - p_priv->start_time) * 1000 ;
      msg.content = PLAYER_MSG_PLAY_TIME_UPDATE;
      if((tmp_play_time > start_play_time) && (tmp_play_time < start_play_time + 10000))
      {
          if(start_play_time >= p_priv->last_play_time + 1)
          {
              p_priv->last_play_time = start_play_time;
          }
          else
          {
              return;
...
static u32 push_down(ts_player_filter_private_t *p_priv, media_sample_t *p_in)
{
  ...
  else
  {
    ...
    //  p_priv->seek_end_check ++;
     // return 0;
        p_priv->ts_info.fragment_num= 1;
        p_priv->ts_info.fragment_offset[0] = 0;
        p_priv->ts_info.fragment_size[0] = p_in->payload;
    }
  }
...
static RET_CODE on_start(handle_t _this)
{
  ...
  p_ifilter->set_active_enter(p_priv->p_this, update_play_time);
#endif
  p_priv->start_time = mtos_ticks_get() / 100;
  return SUCCESS;
}
