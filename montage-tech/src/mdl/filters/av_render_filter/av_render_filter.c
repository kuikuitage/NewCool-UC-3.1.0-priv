  ...
  BOOL b_need_resume, u32 is_trick_no_audio)
{
  ...
  if(p_priv->cfg.encrypt_flag == 1)
  {      
      if (p_priv->video_chain != 0xffff)
      {
          ret = dmx_descrambler_onoff(p_dmx_dev, p_priv->video_chain, FALSE);
          ret = dmx_descrambler_onoff(p_dmx_dev, p_priv->video_chain, TRUE);
          if (ret == SUCCESS)
          {             
              ret = dmx_descrambler_set_odd_keys(p_dmx_dev, 
                                                 p_priv->video_chain, 
                                                 p_priv->cfg.video_key_odd, 
                                                 p_priv->cfg.video_key_odd_len);
              ret = dmx_descrambler_set_even_keys(p_dmx_dev, 
                                                  p_priv->video_chain, 
                                                  p_priv->cfg.video_key_even, 
                                                  p_priv->cfg.video_key_even_len);
          }
      }
      if (p_priv->audio_chain != 0xffff)
      {
          ret = dmx_descrambler_onoff(p_dmx_dev, p_priv->audio_chain, FALSE);
          ret = dmx_descrambler_onoff(p_dmx_dev, p_priv->audio_chain, TRUE);
          if (ret == SUCCESS)
          {
              ret = dmx_descrambler_set_odd_keys(p_dmx_dev, 
                                                 p_priv->audio_chain, 
                                                 p_priv->cfg.audio_key_odd, 
                                                 p_priv->cfg.audio_key_odd_len);
              ret = dmx_descrambler_set_even_keys(p_dmx_dev, 
                                                  p_priv->audio_chain, 
                                                  p_priv->cfg.video_key_even, 
                                                  p_priv->cfg.video_key_even_len);
