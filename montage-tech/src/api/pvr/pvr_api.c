...
RET_CODE mul_pvr_play_start_chn(u32 chanid)
{
  ...
  if(p_ctrl->play_attr.is_clear_stream == FALSE)
  {
      av_config.encrypt_flag = 1;
      av_config.audio_key_odd_len  = p_ctrl->play_attr.key_cfg.audio_key_odd_len;
      memcpy(av_config.audio_key_even, 
             p_ctrl->play_attr.key_cfg.audio_key_even, 
             av_config.audio_key_odd_len);
      av_config.audio_key_odd_len  = p_ctrl->play_attr.key_cfg.audio_key_odd_len;
      memcpy(av_config.audio_key_odd, 
             p_ctrl->play_attr.key_cfg.audio_key_odd, 
             av_config.audio_key_odd_len);
      av_config.video_key_even_len = p_ctrl->play_attr.key_cfg.video_key_even_len;
      memcpy(av_config.video_key_even, 
             p_ctrl->play_attr.key_cfg.video_key_even, 
             av_config.video_key_even_len);
      av_config.video_key_odd_len = p_ctrl->play_attr.key_cfg.video_key_odd_len;
      memcpy(av_config.video_key_odd, 
             p_ctrl->play_attr.key_cfg.video_key_odd, 
             av_config.video_key_odd_len);
  }
  else
  {
      av_config.encrypt_flag = 0;
  }
  icmd.cmd = AV_RENDER_CFG;
  icmd.lpara = (u32)(&av_config);
  p_av_render_filter->do_command(p_av_render_filter, &icmd);
