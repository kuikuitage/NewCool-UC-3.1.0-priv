...
#define DATA_SIZE (200 * (KBYTES))  //128  // panhui  TRCA need 200
...
//#define CAS_DEBUG
#ifdef CAS_DEBUG
#define CAS_PRINTF	OS_PRINTF("%s, line=%d  ", __FUNCTION__, __LINE__); OS_PRINTF
#else
#define CAS_PRINTF(...)	do{}while(0)
#endif
...
static RET_CODE cas_notify_callback(u32 slot, u32 event, u32 param, u32 cam_id, u32 context)
{
  ...
  CAS_PRINTF("callback evt %d  slot %d,index %d\n", event, slot,p_this->evt_index);
...
static RET_CODE cas_events_get(handle_t handle, ap_cas_evt_t *p_event)
{
  ...
  if (ret == TRUE)
  {
    ...
    CAS_PRINTF("get evt %d slot %d. index %d\n", p_event->event, p_event->slot_id, index);
    mtos_sem_give(&p_this->evt_mutex);
    return SUCCESS;
  }
  mtos_sem_give(&p_this->evt_mutex);
...
static void pat_request(handle_t handle)
{
  ...
  CAS_PRINTF("CAS: pat request. pgid %d\n", p_this->pgid);
}
...
static void pmt_request(handle_t * handle, u16 pid, u16 s_id)
{
  ...
  CAS_PRINTF("CAS: request pmt, pid %d, sid %d\n", pid, s_id);
...
static void nit_request(handle_t handle)
{
  ...
  CAS_PRINTF("CAS: nit request. pgid %d\n", p_this->pgid);
}
...
static void cat_request(handle_t handle, u16 sid)
{
  ...
  CAS_PRINTF("CAS: cat request. pgid %d, sid %d\n", p_this->pgid, sid);
}
  ...
  os_msg_t *p_msg)
{
  ...
  {
    //CAS_PRINTF("CAS: p_this->svc_id %d, p_msg->para2 %d\n", p_this->svc_id[i], p_msg->para2);
...
static void on_cas_play_info(handle_t handle, cas_sid_t *p_info)
{
  ...
  CAS_PRINTF("CAS: set s id %d, tp_freq %d\n", p_this->svc_id[0], p_this->cur_tp_freq);
}
...
static void on_cas_set_sid(handle_t handle, cas_sid_t *p_info)
{
  ...
  CAS_PRINTF("CAS: set s id %d, tp_freq %d, pmt_id %d\n",
...
static void on_dvb_msg(handle_t handle, os_msg_t *p_msg)
{
  ...
  u8 i = 0;
  u16 ecm_num = MAX_ECM_DESC_NUM;
  //CAS_PRINTF("CAS: curn %d, repeat %d\n", p_this->repeat_times, repeat_times);
  //if((p_this->pgid != pgid) || (repeat_times != p_this->repeat_times))
  //{
    //CAS_PRINTF("CAS: p_this->pgid %d, pgid %d, drop msg 0x%x, extend_data 0x%x\n",
  ...
  {
    case DVB_PAT_FOUND:
      //CAS_PRINTF("CAS: pat found, ts_id = %d\n", p_msg->para1);
      if((p_this->pgid != pgid) || (repeat_times != p_this->repeat_times))
      {
        CAS_PRINTF("repeat req pat\n");
        pat_request(handle);
        p_this->pmt_ticks = mtos_ticks_get();
        return;
      }
    ...
    }
    {
      ...
      {
        CAS_PRINTF("pmt version change org %d, new %d\n", p_this->pmt_version, p_pmt->version);
        p_this->pmt_version = p_pmt->version;
    ...
    case DVB_TABLE_TIMED_OUT:
      CAS_PRINTF("CAS : time out table %d\n",p_msg->para1);
      on_timeout(handle, p_msg->para1);
      break;
    case DVB_NIT_FOUND:
      //CAS_PRINTF("CAS: nit found, len %d\n", p_msg->para2);
      if (p_this->p_policy->test_ca_performance != NULL)
      {
        p_this->p_policy->test_ca_performance(CAS_NIT_DATA_STEP, mtos_ticks_get());
      }
...
static void cas_process_msg(handle_t handle, os_msg_t *p_msg)
{
  ...
  switch(p_msg->content)
  {
    case M_SVC_DATA_RESET:
      CAS_PRINTF("cas get monitor info %d\n", p_msg->para1);
    ...
    case CAS_CMD_OPERATOR_INFO_GET:
      ...
      CAS_PRINTF("get mem addr 0x%x, LINE=%d,size %d\n", temp_addr,
    ...
    case CAS_CMD_ACLIST_INFO_GET:
      ...
      CAS_PRINTF("get mem addr 0x%x, LINE=%d\n", temp_addr,__LINE__);
      cam_id = (p_msg->para2 & 0xFF);
      slot = ((p_msg->para2 >> 16) & 0xFF);
      cas_param.cam_id = cam_id;
    ...
    case CAS_CMD_MON_CHILD_STATUS_GET:
      ...
      CHECK_FAIL_RET_VOID(temp_addr != 0);
      memset((void *)temp_addr, 0x0, CAS_MON_CHILD_SIZE);              
      CAS_PRINTF("get mem addr 0x%x, LINE=%d, sz %d\n", 
    ...
    case CAS_CMD_PAIRE_SET:
      cam_id = (p_msg->para2 & 0xFF);
      slot = ((p_msg->para2 >> 16) & 0xFF);
      cas_param.cam_id = cam_id;
      cas_param.p_param = (void *)p_msg->para1;
      ret = cas_io_ctrl(slot, CAS_IOCMD_PATERNER_STA_SET, &cas_param);
      cas_send_evt_to_ui(CAS_EVT_PAIRED_SET, (u32)ret, temp_addr);
      break;
    ...
    case CAS_CMD_REQUEST_MASK_BUFFER:
      CAS_PRINTF("CAS_CMD_REQUEST_MASK_BUFFER\n");
      cam_id = (p_msg->para2 & 0xFF);
      cas_param.cam_id = cam_id;
      slot = ((p_msg->para2 >> 16) & 0xFF);
    ...
    case CAS_CMD_REQUEST_UPDATE_BUFFER:
      CAS_PRINTF("CAS_CMD_REQUEST_UPDATE_BUFFER\n");
      cam_id = (p_msg->para2 & 0xFF);
      cas_param.cam_id = cam_id;
      slot = ((p_msg->para2 >> 16) & 0xFF);
  ...
  switch(p_this->sm)
  {
    ...
    case CAS_SM_WAITTING_LOCK:
      ...
      else
      {
        ...
        {
            //CAS_PRINTF("CAS: tunner info match\n\n");
    ...
    case CAS_SM_MONITOR:
      ...
      {
        CAS_PRINTF("TRACE: NO response for pmt request, clear the flag\n");
        p_this->pmt_req_flag = FALSE;
        p_this->pat_req_flag = FALSE;
        p_this->cat_req_flag = FALSE;
      }
 ...
 if(SUCCESS == cas_events_get(p_this, &event))
{
    ...
    switch(cas_event)
    {
      ...
      case CAS_C_REQU_NIT_SECITON:
        CAS_PRINTF("CAS : get req nit cmd\n");
      ...
      case CAS_C_HIDE_NEW_FINGER:
        CAS_PRINTF("cas hide new finger  LINE=%d\n", __LINE__);
      ...
      case CAS_E_POP_MESSAGE:
        ...
        else
        {
          ...
          CAS_PRINTF("CAS_E_POP_MESSAGE: no msg in buffer now, return directly\n");
          return;
        }
        break;        
	  ...
	  case CAS_C_HIDE_FORCEOSD:
        CAS_PRINTF("cas hide force osd  LINE=%d\n", __LINE__);
      ...
      case CAS_C_HIDE_OSD_TEXT:       
        CAS_PRINTF("cas hide super osd text LINE=%d\n", __LINE__);       
     ...
     case CAS_C_CARD_NEED_UPDATE:
      ...
      case CAS_C_IEXIGENT_PROG_STOP:
        CAS_PRINTF("CAS_C_IEXIGENT_PROG_STOP, LINE=%d\n", __LINE__);
...
case CAS_C_STB_URGENT_BROADCAST:
      ...
      case CAS_C_STB_NOTIFICATION:
        ...
        evt.data2 = event.param;
        break;
	  case CAS_E_VOD_AUTH_INVALID:
	    evt.id = CAS_EVT_VOD_AUTH_INVALID;
        evt.data1 = (event.slot_id << 16) | cas_param.cam_id;
        evt.data2 = event.param;
		CAS_PRINTF("[debug] send msg CAS_EVT_NO_NETWORK_AUTHORIZATION \n");
        break;
	  case CAS_C_ADS_MESSAGE:
		evt.id = CAS_EVT_ADS_MESSAGE;
		evt.data1 = (event.slot_id << 16) | cas_param.cam_id;
		evt.data2 = event.param;
		CAS_PRINTF("[debug] send msg CAS_EVT_NO_NETWORK_AUTHORIZATION \n");
		break;
        case CAS_UPGRADEFREQ:
           evt.id = CAS_EVT_UPGRADE_FREQ;
           evt.data1 = (event.slot_id << 16) | cas_param.cam_id;
           evt.data2 = event.param;
           CAS_PRINTF("[debug] send msg CAS_EVT_UPGRADE_FREQ \n");
           break;	
      default:
        evt.id = CAS_EVT_NOTIFY;
        evt.data1 = (event.slot_id << 16) | cas_param.cam_id;
    ...
    if(CAS_EVT_NOTIFY==evt.id)
		CAS_PRINTF("CAS_S_ADPT_CARD_REMOVE:%d, cas_event:%d\n", 
					CAS_S_ADPT_CARD_REMOVE, cas_event);
    ap_frm_send_evt_to_ui(APP_CA, &evt);
...
static void init(handle_t handle)
{
  ...
  if (p_this->p_policy->config_ca_data_mem != NULL)
  {
    ...
    CAS_PRINTF("start mem addr 0x%x\n", p_this->mem_start);
    p_this->mem_end = p_this->mem_start + size;
    CAS_PRINTF("end mem addr 0x%x\n", p_this->mem_end);
  }
  else
  {
