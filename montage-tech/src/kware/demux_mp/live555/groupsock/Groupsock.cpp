...
Boolean OutputSocket::writedata(netAddressBits address, Port port, u_int8_t ttl,
...
Boolean Groupsock::output(UsageEnvironment& env, u_int8_t ttlToSend,
	...
	do {
		...
		for (destRecord* dests = fDests; dests != NULL; dests = dests->fNext) {
			if (!writedata(dests->fGroupEId.groupAddress().s_addr, dests->fPort, ttlToSend,
						buffer, bufferSize)) {
				writeSuccess = False;
				break;
			}
