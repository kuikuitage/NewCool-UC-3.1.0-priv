...
::noteIncomingPacket(u_int32_t SSRC, u_int16_t seqNum,
	...
	if (stats->numPacketsReceivedSinceLastReset() == 0) {
		LIV555_DEBUG("[%s] ----stats->numPacketsReceivedSinceLastReset() == 0 \n",__func__);
...
::noteIncomingPacket(u_int16_t seqNum, u_int32_t rtpTimestamp,
	...
	++fTotNumPacketsReceived;
	//OS_PRINTF("[%s] ----fTotNumPacketsReceived:%d, fNumPacketsReceivedSinceLastReset:%d\n",__func__,fTotNumPacketsReceived,fNumPacketsReceivedSinceLastReset);
...
void RTPReceptionStats::noteIncomingSR(u_int32_t ntpTimestampMSW,
	...
	gettimeofday_replace(&fLastReceivedSR_time, NULL);
#else
	gettimeofday_replace(&fLastReceivedSR_time, NULL);
#endif
	...
	OS_PRINTF("[%s] fHasBeenSynchronized = True!\n",__func__);
}
