...
public:
  ...
  static void Cfprintf(FILE* fid, CuePoint* cuePoint); // used for debugging; it's static to allow for "cuePoint == NULL"
...
{
  CuePoint::Cfprintf(fid, fCuePoints);
}
#else
void MatroskaFile::printCuePoints(void* fid)
{
  CuePoint::Cfprintf((FILE*)fid, fCuePoints);
}
#endif
...
void CuePoint::Cfprintf(FILE* fid, CuePoint* cuePoint) {
  if (cuePoint != NULL) {
    fprintf(fid, "[");  //fixed me ::fprintf  will lead release compile error on iris core, so changed it to fprintf
    Cfprintf(fid, cuePoint->left());

    fprintf(fid, ",%.1f{%d},", cuePoint->fCueTime, cuePoint->fBalance);

    Cfprintf(fid, cuePoint->right());
    fprintf(fid, "]");
  }
}
