...
unsigned RTSPClient::sendDescribeCommand(responseHandler* responseHandler, Authenticator* authenticator) {
	...
	unsigned ret = sendRequest(new RequestRecord(++fCSeq, "DESCRIBE", responseHandler));
	LIV555_DEBUG("[%s]------------end \n",__func__);
	return ret;
}
...
int RTSPClient::openConnection() {
	...
	do {
		...
		if (!parseRTSPURL(envir(), fBaseURL, username, password, destAddress, urlPortNum, &urlSuffix)) {
			OS_PRINTF("[%s]------------parseRTSPURL error, break!\n",__func__);
			break;
		}
		...
		if (fInputSocketNum < 0) {
			OS_PRINTF("[%s]------------fInputSocketNum < 0, break!\n",__func__);
			break;
		}

		// Connect to the remote endpoint:
		fServerAddress = *(netAddressBits*)(destAddress.data());
		OS_PRINTF("[%s]------------fInputSocketNum=%d, destPortNum=%d \n",__func__,fInputSocketNum,destPortNum);
		int connectResult = connectToServer(fInputSocketNum, destPortNum);
		if (connectResult < 0) {
			OS_PRINTF("[%s]------------connectResult < 0, break!\n",__func__);
			break;
		}
			...
			OS_PRINTF("[%s] add incomingDataHandler to process rtsp response !!!!\n",__func__);
		}
		OS_PRINTF("[%s]------------connectResult = %d!\n",__func__,connectResult);
...
int RTSPClient::connectToServer(int socketNum, portNumBits remotePortNum) {
	...
	if (connect(socketNum, (struct sockaddr*) &remoteName, sizeof remoteName) != 0) {
		...
		OS_PRINTF("[%s]------------err = %d, EINPROGRESS: %d, EWOULDBLOCK:%d\n",__func__,err,EINPROGRESS,EWOULDBLOCK); 
		if (err == EINPROGRESS || err == EWOULDBLOCK) {
			OS_PRINTF("[%s]------------err = %d, return 0\n",__func__,err); 
			...
			OS_PRINTF("[%s] add connectionHandler to process connect for rtspclient!!!!\n",__func__);
	...
	OS_PRINTF("[%s]------------fVerbosityLevel = %d\n",__func__,fVerbosityLevel); 
...
unsigned RTSPClient::sendRequest(RequestRecord* request) {
	...
	do {
		...
		if (!fRequestsAwaitingConnection.isEmpty()) {
			...
			LIV555_DEBUG("[%s]------------call openConnection!\n",__func__);
			int connectResult = openConnection();
			if (connectResult < 0) {
				OS_PRINTF("[%s]------------an error occurred, connectResult=%d!\n",__func__,connectResult);
		...
		OS_PRINTF("[%s]------------connectionIsPending = %d\n",__func__,connectionIsPending);

		if (connectionIsPending) {
			fRequestsAwaitingConnection.enqueue(request);
			return request->cseq();
		}
		LIV555_DEBUG("[%s]------------out of if (connectionIsPending)!\n",__func__);
		...
		OS_PRINTF("[%s]------------start to construct and send the command!\n",__func__);
		...
		if (strcmp(request->commandName(), "DESCRIBE") == 0) {
			...
			OS_PRINTF("[%s]------------start to construct a Transport header!\n",__func__);
			...
			if (strcmp(request->commandName(), "GET") == 0) {
				...
				gettimeofday_replace(&seedData.timestamp, NULL);
#else
				gettimeofday_replace(&seedData.timestamp, NULL);
#endif
			...
			if (request->session() != NULL) {
				...
				OS_PRINTF("[%s]------------else Media-level operation  \n",__func__);
			...
			OS_PRINTF("[%s]------------start to compare the commandName with PLAY  \n",__func__);
			...
			OS_PRINTF("[%s]------------out of judging request->commandName()  \n",__func__);
		}

		LIV555_DEBUG("[%s]------------call createAuthenticatorString  \n",__func__);
		...
		OS_PRINTF("[%s]------------ if()  tunneling RTSP-over-HTTP\n",__func__);
		...
		LIV555_DEBUG("[%s]------------call send(fOutputSocketNum, cmd, strlen(cmd)  \n",__func__);
		...
		LIV555_DEBUG("[%s]------------call fRequestsAwaitingResponse.enqueue(request)  \n",__func__);
		...
		delete[] cmd;
		OS_PRINTF("[%s]------------delete cmd : %s \n",__func__,cmd);

		unsigned ret_sendRequest = request->cseq();

		OS_PRINTF("[%s]------------return : %d \n",__func__,ret_sendRequest);
		return ret_sendRequest;
	} while (0);
	LIV555_DEBUG("[%s]------------out of while(0)  \n",__func__);
...
char* RTSPClient::describeURL(char const* url, Authenticator* authenticator,
	...
	LIV555_DEBUG("[%s]------------fResultCode = %d!\n",__func__,fResultCode);
