...
#else
void MP3StreamState::assignStream(UFILE* fid, unsigned fileSize)
#endif
{
  fFid = fid;
 ...
 gettimeofday_replace(&fNextFramePresentationTime, NULL);
#else
  gettimeofday_replace(&fNextFramePresentationTime, NULL);
#endif
}
...
void MP3StreamState::writeGetCmd(char const* hostName,
  ...
  if (fFidIsReallyASocket) {
    ...
    sprintf(tmp_data, getCmdFmt, fileName, hostName, portNum);
    ret = ufs_write(fFid, (void *)tmp_data, sizeof tmp_data, NULL);
    ufs_flush(fFid);
#endif
  }
}
...
unsigned MP3StreamState::readFromStream(unsigned char* buf,
  ...
  if (fFidIsReallyASocket) {
    ...
    u32 read_size;
    int ret;
    ret = ufs_read(fFid, (void *)buf, numChars, &read_size);
#endif
  }
}
