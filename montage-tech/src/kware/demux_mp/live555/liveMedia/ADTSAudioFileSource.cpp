...
ADTSAudioFileSource::createNew(UsageEnvironment& env, char const* fileName) {
  ...
  do {
    ...
    u32 read_size;
    int ret;
    ret = ufs_read(fid, (void *)fixedHeader, sizeof fixedHeader, &read_size);
    if ((read_size != sizeof fixedHeader) || (ret != FR_OK)) break;
#endif
...
#else
ADTSAudioFileSource
::ADTSAudioFileSource(UsageEnvironment& env, UFILE* fid, u_int8_t profile,
		      u_int8_t samplingFrequencyIndex, u_int8_t channelConfiguration)
  : FramedFileSource(env, fid)
#endif
{
...
void ADTSAudioFileSource::doGetNextFrame() {
  ...
  u32 read_size;
  int ret;
  ret = ufs_read(fFid, (void *)headers, sizeof headers, &read_size);
  if ((read_size != sizeof headers) || (ret != FR_OK))
#endif
  {
...
#else
  ret = ufs_read(fFid, (void *)fTo, numBytesToRead, &read_size);
  int numBytesRead = read_size;
#endif
