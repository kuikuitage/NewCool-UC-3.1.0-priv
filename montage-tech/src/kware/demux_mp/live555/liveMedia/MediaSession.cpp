...
Boolean MediaSubsession::initiate(int useSpecialRTPoffset) {
	...
	do {
		...
		if (fClientPortNum != 0) {
			...
			while (1) {
				...
				OS_PRINTF("[%s] fClientPortNum:%d\n",__func__,fRTPSocket->socketNum());
				...
				if (fRTCPSocket != NULL && fRTCPSocket->socketNum() >= 0) {
					...
					success = True;
					OS_PRINTF("[%s] rtcpPortNum:%d\n",__func__,fRTCPSocket->socketNum());
					break;
				} else {
		...
		OS_PRINTF("[%s] fBandwidth:%ld\n",__func__,fBandwidth);
		//OS_PRINTF("[%s] fRTCPSocket:%ld\n",__func__,fRTCPSocket);
	...
	Medium::closeMedium(fRTCPInstance); fRTCPInstance = NULL;
	Medium::closeMedium(fReadSource); fReadSource = fRTPSource = NULL;
	fClientPortNum = 0;
	return False;
}

void MediaSubsession::deInitiate() {
	Medium::closeMedium(fRTCPInstance);
	fRTCPInstance = NULL;

	Medium::closeMedium(fReadSource); // this is assumed to also close fRTPSource
