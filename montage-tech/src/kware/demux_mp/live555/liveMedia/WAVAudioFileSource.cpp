...
#else
static u8 readbyte(UFILE *fid)
{
  u8 onebyte;
  ufs_read(fid, (void *)&onebyte, 1, NULL);
  return onebyte;
}
#define nextc readbyte(fid)
#endif

#ifdef __LINUX__
static Boolean get4Bytes(FILE* fid, unsigned& result)
#else
static Boolean get4Bytes(UFILE* fid, unsigned& result)
#endif
{ // little-endian
  int c0, c1, c2, c3;
...
#else
static Boolean get2Bytes(UFILE* fid, unsigned short& result)
#endif
{//little-endian
  int c0, c1;
...
#else
static Boolean skipBytes(UFILE* fid, int num)
#endif
{
  while (num-- > 0) {
...
#else
WAVAudioFileSource::WAVAudioFileSource(UsageEnvironment& env, UFILE* fid)
  : AudioInputDevice(env, 0, 0, 0, 0)/* set the real parameters later */,
    fFid(fid), fFidIsSeekable(False), fLastPlayTime(0), fHaveStartedReading(False), fWAVHeaderSize(0), fFileSize(0),
    fScaleFactor(1), fLimitNumBytesToStream(False), fNumBytesToStream(0), fAudioFormat(WA_UNKNOWN)
#endif
...
void WAVAudioFileSource::doReadFromFile() {
  ...
  u32 read_size;
  int ret;
  ret = ufs_read(fFid, (void *)fTo, bytesToRead, &read_size);
#endif
   }
#ifdef __LINUX__
    else {
