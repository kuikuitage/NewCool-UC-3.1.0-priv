...
BasicTaskScheduler* BasicTaskScheduler::createNew() {
	...
	BasicTaskScheduler* i = new BasicTaskScheduler();
	LIV555_DEBUG("[%s]------------end!\n",__func__);
	return i;
}
...
void BasicTaskScheduler::SingleStep(unsigned maxDelayTime) {
	...
	if (selectResult < 0) {
...
#else
		OS_PRINTF("[%s] ERROR ERROR Select !!!!!\n",__func__);
		...
		while ((handler = iter.next()) != NULL) {
			...
			if ((resultConditionSet&handler->conditionSet) != 0 && handler->handlerProc != NULL) {
				...
				LIV555_DEBUG("[%s] handlerProc: 0x%x, s:%d\n",__func__, (*handler->handlerProc), sock);
				break;
			}
	...
	void BasicTaskScheduler
		::setBackgroundHandling(int socketNum, int conditionSet, BackgroundHandlerProc* handlerProc, void* clientData) {
			...
			OS_PRINTF("[%s]socketNum[%d]\n",__func__,socketNum);
			OS_PRINTF("[%s]handlerProc[0x%x]\n",__func__,handlerProc);
