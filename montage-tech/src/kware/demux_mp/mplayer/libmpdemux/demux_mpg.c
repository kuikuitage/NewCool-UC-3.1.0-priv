    ...
    if (mpg_d) {
        ...
        if (demuxer->seekable
            ...
            if(!mpg_d->has_valid_timestamps)
            {
                ...
                while(!demuxer->video->eof && get_pts_times++ < 20)
                {
                    ds_fill_buffer(demuxer->video);
                ...
                while(mpg_d->last_pts != -1.0 && loop_size_step < 3)
                {
                    demux_flush(demuxer);
                    mpg_d->last_pts = -1.0;
        ...
        if (id >= 0x1E0 && id <= 0x1EF) {
            ...
            if (ds->sh) {
                ...
                if (!sh->sh.extradata_parsed) {
                    ...
                    sh->sh.extra_data = (unsigned char *)malloc(vol_len + 1);
                    memcpy(sh->sh.extra_data, &(buf[start_i - 3]), vol_len);
                } else if (sh->sh.extradata_size) {
                    ...
                    if (i < dp->len && (sequence_header_flag == 0)) {
                        ...
                        memcpy(new_pkt->buffer + i - 3, sh->sh.extra_data, sh->sh.extradata_size);
                        memcpy(new_pkt->buffer + i - 3 + sh->sh.extradata_size, p_tmp + i - 3, dp->len - i + 3);
        ...
        case DEMUXER_CTRL_SWITCH_AUDIO:
            if (!(mpg_d && mpg_d->num_a_streams > 1 && demuxer->audio && demuxer->audio->sh)) {
                ...
                if (*((int *)arg) < 0) {
                    ...
                    for (i = 0; i < mpg_d->num_a_streams; i++)
                        if (*((int *)arg) == mpg_d->a_stream_ids[i]) {
                            break;
                        }
