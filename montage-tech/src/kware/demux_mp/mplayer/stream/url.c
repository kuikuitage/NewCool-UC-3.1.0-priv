...
int mthttp_url_is_protocol(const URL_t *url, const char *proto) {
  ...
  if (proxy) {
    URL_t *tmp = mthttp_url_new(url->file + 1);
...
void mthttp_url_set_protocol(URL_t *url, const char *proto) {
  ...
  #ifndef __LINUX__
  mthttp_free(url->protocol);
  url->protocol = mtstrdup(proto);
  #else
  free(url->protocol);
...
URL_t *mthttp_url_redirect(URL_t **url, const char *redir) {
  ...
  if (!strchr(redir, '/') || *redir == '/') {
    char *tmp;
    #ifndef __LINUX__
    newurl = buffer = mthttp_malloc(strlen(oldurl) + strlen(redir) + 1);
    #else
...
URL_t* mthttp_url_new(const char* url) {
	...
	if( url==NULL ) return NULL;
        ...
        #ifndef __LINUX__
        escfilename=mthttp_malloc(strlen(url)*3+1);
        #else
	...
	#ifndef __LINUX__
	Curl = mthttp_calloc(1, sizeof(*Curl));
	#else
	...
	#ifndef __LINUX__
	Curl->protocol = mthttp_malloc(pos1+1);
	#else
	...
	if( ptr2!=NULL ) {
		...
		int len = ptr2-ptr1;
		#ifndef __LINUX__
		Curl->username = mthttp_malloc(len+1);
		#else
		...
		if( ptr3!=NULL && ptr3<ptr2 ) {
			...
			#ifndef __LINUX__
			Curl->password = mthttp_malloc(len2+1);
			#else
	...
	#ifndef __LINUX__
	Curl->hostname = mthttp_malloc(pos2-pos1+1);
	#else
	    ...
	    ptr2 = strstr(ptr1, "\?");
            if( ptr2!=NULL ) {
            		...
            		if( strlen(ptr2)>1 ) {
            			...
            			file_name = mtos_malloc(file_name_len);
            			if(file_name)
            			{
						    //add / for special url
            			    snprintf(file_name, file_name_len, "/%s", ptr2);
            			    Curl->file = mtstrdup(file_name);
            			}
            			else
            			    Curl->file = mtstrdup(ptr2);
            			if(file_name)
...
void
mthttp_url_free(URL_t* url) {
	if(!url) return;
	#ifndef __LINUX__
	mthttp_free(url->url);
	mthttp_free(url->protocol);
	mthttp_free(url->hostname);
	mthttp_free(url->file);
	mthttp_free(url->username);
	mthttp_free(url->password);
	mthttp_free(url->noauth_url);
	mthttp_free(url);
	#else
	free(url->url);
	free(url->protocol);
...
mthttp_url_escape_string(char *outbuf, const char *inbuf) {
	...
	while(i < len) {
		...
		if(j < len) {
		#ifndef __LINUX__
			if(!tmp) tmp = mthttp_malloc(len+1);
		#else
			...
			in = (char*)inbuf+i;
        #ifndef __LINUX__
		if(!unesc) unesc = mthttp_malloc(len+1);
		#else
