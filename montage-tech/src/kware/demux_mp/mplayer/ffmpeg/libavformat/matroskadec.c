...
#ifndef __LINUX__
 //peacer add 20140709
 if(((unsigned int)(p->buf)) <= 0x80000000)
 {
          mtos_printk("[%s] ===ERROR ERROR !!!!!!!\n",__func__);
	   mtos_printk("[%s] ==p->buf[0x%x]!!!!!\n",__func__,p->buf);
	   return 0;
 }
 #endif
...
static int matroska_check_cues(MatroskaDemuxContext *matroska) 
{
    ...
    /* seek to cues*/
    av_log(0,0,"\n seekhead[%d].id=0x%llx, pos=0x%llx \n",i,seekhead[i].id,seekhead[i].pos);
    offset = seekhead[i].pos + matroska->segment_start;
    av_log(0,0,"offset=0x%llx \n",offset);
    if (avio_seek(matroska->ctx->pb, offset, SEEK_SET) == offset) 
    {
        cues_id = avio_rb32(matroska->ctx->pb);
        av_log(0,0,"cues_id=0x%x \n",cues_id);
        if(cues_id == MATROSKA_ID_CUES)
        {
            //matroska->seek_enable = 1;
            matroska->ctx->pb->seekable = 1;
        }
        else
        {
            //matroska->seek_enable = 0;
            matroska->ctx->pb->seekable = 0;
            ret = -1;
         }
        
        av_log(0,0,"matroska->seek_enable=%d \n",matroska->ctx->pb->seekable);
    }
                                ...
                                int64_t pos, int is_keyframe)
{
    ...
    else if (st->codec->codec_id == AV_CODEC_ID_VP8)
    {
        ...
        {
            //add 32 ivf headers
            bytestream_put_le32(&buf, AV_RL32("DKIF"));
            bytestream_put_le16(&buf, 0);// version
            bytestream_put_le16(&buf, 32); // header length
            bytestream_put_le32(&buf, AV_RL32("VP80"));
            bytestream_put_le16(&buf, st->codec->width);
            bytestream_put_le16(&buf, st->codec->height);
            bytestream_put_le32(&buf, (int)av_q2d(st->avg_frame_rate));//frame rate
            bytestream_put_le32(&buf, st->time_base.den);//time scale
            int total_frames = ((int)(matroska->duration * av_q2d(st->avg_frame_rate)) /1000);
            bytestream_put_le32(&buf, total_frames);//number of frames in file
            bytestream_put_le32(&buf, 0);//unused

            //OS_PRINTF("[%s] video width[%d] height[%d] duration[%d] ms, fps[%d] time_base.num[%d] total frames[%u]\n",__func__,
            //    st->codec->width, st->codec->height,(int)matroska->duration, (int)av_q2d(st->avg_frame_rate), st->time_base.den, total_frames);

            vp8_add_ivf_header = 0;
        }

        //av_log(matroska->ctx, AV_LOG_DEBUG,"VP8 pkt_size[%d] timecode[%llu]\n",
        //    pkt_size, timecode);
        bytestream_put_le32(&buf, pkt_size);//for write to file.
        bytestream_put_le64(&buf, timecode);
    }
...
static int matroska_parse_cluster_incremental(MatroskaDemuxContext *matroska)
{
    ...
    if (!res &&
        ...
        if (blocks[i].bin.size > 0 && blocks[i].bin.data) {
            ...
            av_free(blocks[i].bin.data);
            blocks[i].bin.data = NULL;
