...
static int flv_read_packet(AVFormatContext *s, AVPacket *pkt)
{
 ...
 for(;;avio_skip(s->pb, 4)){ /* pkt size is repeated at end. skip it */
    ...
    dts = avio_rb24(s->pb);
    dts |= avio_r8(s->pb) << 24;
     //av_log(s, AV_LOG_DEBUG, "type:%d, size:%d, dts:%"PRId64"\n", type, size, dts);
    ...
    if (type == FLV_TAG_TYPE_AUDIO) {
        ...
        //yliu
        static int flv_err_cnt = 0;
        flv_err_cnt++;
        
        av_log(s, AV_LOG_DEBUG, "skipping flv packet: type %d, size %d, flags %d\n", type, size, flags);
        if(flv_err_cnt>5)
            {
             av_log(s, AV_LOG_DEBUG, "skipping too much just set to EOF\n");
             return AVERROR_EOF;
            }
    skip:
...
retry_duration:
        ...
        if (st->codec->codec_id == AV_CODEC_ID_H264 || st->codec->codec_id == AV_CODEC_ID_MPEG4) {
            ...
            if (cts < 0) { // dts are wrong
                ...
                av_log(s, AV_LOG_WARNING, "negative cts, previous timestamps might be wrong\n");
            	 ...
            	 flv->wrong_dts = 0;//add by doreen for rtmp loading long time, 14-03-06
            if (flv->wrong_dts){
                av_log(s, AV_LOG_DEBUG, "@@ wrong_dts[%d] dts[0x%llx] cts[%d] pts[0x%llx], set dts = AV_NOPTS_VALUE\n",flv->wrong_dts,dts,cts,pts);
                dts = AV_NOPTS_VALUE;
            }
        }
        if (type == 0 && (!st->codec->extradata || st->codec->codec_id == AV_CODEC_ID_AAC)) {
            if (st->codec->extradata) {
                if ((ret = flv_queue_extradata(flv, s->pb, stream_type, size)) < 0)
                    return ret;
                //ret = AVERROR(EAGAIN); delete by ybc
                goto leave;
            }
    ...
    pkt->stream_index = st->index;
    //av_log(s, AV_LOG_DEBUG, "\n[%s] -- pkt->dts[0x%llx], pkt->pts[0x%llx], stream_index[%d]\n",__func__, pkt->dts, pkt->pts,pkt->stream_index);
