...
static int mp3_read_probe(AVProbeData *p)
{
	...
	//doreen modify 1 to 10
    else if(max_frames>=10) return 1;
    else                   return 0;
...
static int mp3_parse_vbr_tags(AVFormatContext *s, AVStream *st, int64_t base)
{
   ...
   // OS_PRINTF("[%s] -----start!\n",__func__);
    ...
    if (avpriv_mpegaudio_decode_header(&c, v) == 0){
        ...
        st->codec->channels = c.nb_channels;
        st->codec->sample_rate = c.sample_rate;
        st->codec->bit_rate =c.bit_rate;
        }
    if(c.layer != 3)
        return -1;
    ...
    //st->codec->frame_size=spf;//@@@@@@@@
    //OS_PRINTF("[%s] spf: %d",__func__,spf);
    if(frames)
...
AVInputFormat ff_mp3_demuxer = {
    .name           = "mp3",
    .long_name      = "MPEG audio layer 2/3",//NULL_IF_CONFIG_SMALL("MPEG audio layer 2/3"),
