...
static int f4fbox_parse_asrt(AVIOContext *in, int64_t data_size, void *opague)
{
    ...
    for(i = 0; i < segment_run_entry_count; i++) {
        ...
        //av_log(NULL, AV_LOG_DEBUG, "f4fbox segment i[%d] entry->fragments_per_segment[%d]\n",
        //    i, entry->fragments_per_segment);
    }
...
static int f4fbox_parse_afrt(AVIOContext *in, int64_t data_size, void *opague)
{
    ...
    for(i = 0; i < fragment_run_entry_count; i++) {
        ...
        //av_log(NULL, AV_LOG_DEBUG, "f4fbox fragment_run_entry_count[%d] current entry->first_fragment[%d] fragment_duration[%u]\n",
        //    fragment_run_entry_count, entry->first_fragment, entry->fragment_duration);
        if(entry->fragment_duration == 0) {
            entry->discontinuity_indicator = avio_r8(in);
        }
    }

    av_log(NULL, AV_LOG_DEBUG, "f4fbox parent->nb_fragment_run_table_boxes:%d \n",
            parent->nb_fragment_run_table_boxes);
