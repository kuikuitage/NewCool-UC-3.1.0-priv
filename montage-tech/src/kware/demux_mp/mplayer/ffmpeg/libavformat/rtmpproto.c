...
static int rtmp_handshake(URLContext *s, RTMPContext *rt)
{
    ...
    //av_log(s, AV_LOG_DEBUG, "rt->stream->flags = %d\n",rt->stream->flags);
    ...
    if ((ret = ffurl_read_complete(rt->stream, serverdata,
                                   RTMP_HANDSHAKE_PACKET_SIZE + 1)) < 0) {
        av_log(s, AV_LOG_ERROR, "Cannot read RTMP handshake response, ret=%d\n",ret);
#ifdef __LINUX__
	printf("Cannot read RTMP handshake response, ret=%d\n",ret);
#endif
        return ret;
    }
...
static int handle_invoke_error(URLContext *s, RTMPPacket *pkt)
{
    ...
    if ((ret = find_tracked_method(s, pkt, 9, &tracked_method)) < 0)
        ...
        if (tracked_method && (!strcmp(tracked_method, "_checkbw")      ||
            ...
            ret = 0;
            av_log(s, level, "Gracefully ignore Adobe-specific historical artifact errors.  tracked_method=%d\n", tracked_method);
        } else
            ret = -1;
...
static int get_packet(URLContext *s, int for_header)
{
    ...
    //OS_PRINTF("[%s] ---------  %d!\n", __FUNCTION__,__LINE__);
        ...
        if (ret < 0) {//serious error in current packet
            av_log(s, AV_LOG_DEBUG, "rtmp_parse_result() ret:%d, serious error in current packet! \n",ret);
...
static int rtmp_open(URLContext *s, const char *uri, int flags)
{
    ...
    if (!rt->playpath) {
        ...
        //doreen add for getting the right playpath when rtmp url with -p and -y, 2014-3-14
        OS_PRINTF("[%s] ----1-----playpath:%s \n", __FUNCTION__,rt->playpath);
        p_pageurl = strstr(rt->playpath," -p ");
        if(p_pageurl){
		*p_pageurl = '\0';
		OS_PRINTF("[%s] ----2-----playpath:%s \n", __FUNCTION__,rt->playpath);
        }
    ...
    if (ret < 0){
        av_log(s, AV_LOG_DEBUG, "get_packet erro, go fail!  ret = %d\n", ret);
        goto fail;
    ...
    s->is_streamed     = 1;
    rt->stream->rw_timeout = 1000000*2;
     printf("%s    %d  \n",__func__,__LINE__);
...
fail:
    av_dict_free(&opts);
    rtmp_close(s);
    OS_PRINTF("[%s] ---------called rtmp_close(), ret=%d!\n", __FUNCTION__,ret);
    return ret;
}
extern while_cnt;
static int rtmp_read(URLContext *s, uint8_t *buf, int size)
{
    ...
    //OS_PRINTF("[%s] ----------%d: ticks[%dms]  rtmp_read 1 size[%d]\n", __func__,while_cnt,cur_ticks*10,size);
    ...
    while (size > 0) {
        ...
        if (data_left > 0) {
            ...
            //OS_PRINTF("[%s] ---------ret 2, data_left %d!\n", __FUNCTION__,data_left);
            return data_left;
        }
        //OS_PRINTF("[%s] ---------while loop: size=%d!\n", __FUNCTION__,size);
        //cur_ticks = mtos_ticks_get();
        //OS_PRINTF("[%s] ----------%d: ticks[%dms]  rtmp_read get_packet size[%d]\n", __func__,while_cnt,cur_ticks*10,size);
        if ((ret = get_packet(s, 0)) < 0){
           OS_PRINTF("[%s] ---------get_packet error, ret %d!\n", __FUNCTION__,ret);
           return ret;
       }
        //OS_PRINTF("[%s] --------- get_packet ret %d!\n", __FUNCTION__,ret);
    }
    
    //cur_ticks = mtos_ticks_get();
    //OS_PRINTF("[%s] ----------%d: ticks[%dms]  rtmp_read 2\n", __func__,while_cnt,cur_ticks*10);
