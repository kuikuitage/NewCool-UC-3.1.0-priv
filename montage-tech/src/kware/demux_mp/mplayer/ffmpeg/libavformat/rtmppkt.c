                        ...
                        int chunk_size, RTMPPacket *prev_pkt)
{
    ...
    int ret;
        ...
        ret = ffurl_read(h, &hdr, 1);
    
    if(ret ==0){
    		OS_PRINTF("[%s] ----ffurl_read 1 B for hdr, ret 0 \n", __FUNCTION__);
     		return ret;
     	}
                                 ...
                                 RTMPPacket *prev_pkt, uint8_t hdr)
{
    ...
    while (data_size > 0) {
        ...
        int real_read = ffurl_read_complete(h, p->data + offset, toread);
        if ( real_read != toread) {
            OS_PRINTF("[%s] to_read[%d] real_read[%d] return -5\n", __FUNCTION__,toread,real_read);
        ...
        if (data_size > 0) {
            ...
            if (t != (0xC0 + channel_id)){
            	  OS_PRINTF("[%s] line:%d, t=%d, channel_id=%d\n", __FUNCTION__,__LINE__,t,channel_id);
                return -1;
            }
        }
    }
