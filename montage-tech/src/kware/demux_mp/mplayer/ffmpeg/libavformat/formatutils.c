...
AVInputFormat *av_probe_input_format3(AVProbeData *pd, int is_opened, int *score_ret)
{
	...
	int score, nodat = 0, score_max=0;
      ...
      if(lpd.buf == NULL){//peacer add

              mtos_printk("[%s] ===ERROR ERRORlpd->buf[0X%x] \n",__func__,lpd.buf);
			  return NULL;
	  }
...
AVInputFormat *av_probe_input_format2(AVProbeData *pd, int is_opened, int *score_max)
{
	...
	av_log(NULL, AV_LOG_INFO, "[%s] --score_ret[%d], score_max[%d], fmt[0x%x]\n",__func__,score_ret, *score_max, score_max);
		...
		unsigned int offset, unsigned int max_probe_size)
{
	...
	if (!*fmt) {
		av_free(buf);
		av_log(logctx, AV_LOG_ERROR, "[%s] -- return AVERROR_INVALIDDATA\n", __func__);
    ...
    for(;;){
		...
		if (pktl) {
			*pkt = pktl->pkt;
			...
			else // linda zhu add, when decoding mpegts multicast stream and probed stream failed, st->request_probe=1, although new packet will be add_to_pktbuf but the first packet will never be popped until end is set in probe_codec when "s->raw_packet_buffer_remaining_size <= 0". 
			{
                while(pktl->next)
                {
                    pktl=pktl->next;
		...
		pkt->size = 0;
		av_init_packet(pkt);
		//OS_PRINTF("[%s]---- call s->iformat->read_packet(), 0x%x\n", __FUNCTION__,s->iformat->read_packet);
		ret= s->iformat->read_packet(s, pkt);
		//OS_PRINTF("%s ret of read_packet: %d\n", __FUNCTION__, ret);
		if (ret < 0) {
			//OS_PRINTF("[%s]---- called s->iformat->read_packet(), 0x%x,ret[%d] < 0\n", __FUNCTION__,s->iformat->read_packet,ret);
		...
		if(!pktl && st->request_probe <= 0){
			//if(ret < 0){
				//OS_PRINTF("[%s] pktl=0x%x, st->request_probe=%d, return: %d\n", __FUNCTION__, pktl, st->request_probe, ret);
			//}
			return ret;
		}
		add_to_pktbuf(&s->raw_packet_buffer, pkt, &s->raw_packet_buffer_end);
		s->raw_packet_buffer_remaining_size -= pkt->size;
		//OS_PRINTF("[%s] s->raw_packet_buffer_remaining_size=%d, pkt->size = %d\n", __FUNCTION__, s->raw_packet_buffer_remaining_size, pkt->size);
		...
		AVCodecParserContext *pc, AVPacket *pkt)
{
	...
	//av_log(NULL, AV_LOG_ERROR, " [%s]--start: pkt->first_dts[0x%llx], st->cur_dts[0x%llx], pkt->dts[0x%llx], pkt->pts[0x%llx], \n",__func__,st->first_dts,st->cur_dts,pkt->dts, pkt->pts);
	...
	//av_log(s, AV_LOG_DEBUG, "---pkt->duration=%d\n",pkt->duration); 
	if(pkt->duration != 0 && (s->packet_buffer || s->parse_queue)){
		int64_t first_dts_bak;
        	first_dts_bak = st->first_dts;
		update_initial_durations(s, st, pkt->stream_index, pkt->duration);
		if(st->first_dts != first_dts_bak)
			av_log(s, AV_LOG_DEBUG, "---update_initial_durations() , st->first_dts=[0x%llx -> 0x%llx], codec_type[%d]\n",first_dts_bak,st->first_dts, st->codec->codec_type); 
	...
	if((delay==0 || (delay==1 && pc)) && st->codec->codec_id != AV_CODEC_ID_H264){
		if (presentation_delayed) {
			...
			if(st->first_dts != first_dts_bak)
            			av_log(s, AV_LOG_ERROR, "-1--update_initial_timestamps() st->first_dts=[0x%llx -> 0x%llx], codec_type[%d]\n",first_dts_bak,st->first_dts, st->codec->codec_type);
			...
			update_initial_timestamps(s, pkt->stream_index, pkt->pts,
					pkt->pts);
			if(st->first_dts != first_dts_bak)                                      
				av_log(s, AV_LOG_ERROR, "-2--update_initial_timestamps() st->first_dts=[0x%llx -> 0x%llx], codec_type[%d]\n",first_dts_bak,st->first_dts, st->codec->codec_type); 					
			if (pkt->pts == AV_NOPTS_VALUE)
				pkt->pts = st->cur_dts;
			pkt->dts = pkt->pts;
			if (pkt->pts != AV_NOPTS_VALUE)
	...
	if(st->codec->codec_id == AV_CODEC_ID_H264){ // we skipped it above so we try here
		...
		//av_log(NULL, AV_LOG_ERROR, " [%s]--@@up for H264: pkt->first_dts[0x%llx -> 0x%llx], st->cur_dts[%d], pkt->dts[0x%llx], pkt->pts[0x%llx], \n",__func__,first_dts_bak,st->first_dts,st->cur_dts,pkt->dts, pkt->pts);
		if(st->first_dts != first_dts_bak)
        		av_log(NULL, AV_LOG_ERROR, " [%s]-3--update_initial_timestamps() pkt->first_dts[0x%llx -> 0x%llx] \n",__func__,first_dts_bak,st->first_dts);
	}
...
static int read_frame_internal(AVFormatContext *s, AVPacket *pkt)
{
	...
	while (!got_packet && !s->parse_queue) {
		...
		if (ret < 0) {
			//OS_PRINTF("[%s]-- called ff_read_packet() ret: %d\n",__func__,ret);
		...
		if (st->need_parsing && !st->parser && !(s->flags & AVFMT_FLAG_NOPARSE)&&io_isnetworkstream==0) {
		...
		if (!st->need_parsing || !st->parser||(get_seek_state()==0)) {
			/* no parsing needed: we just output the packet as is */
			*pkt = cur_pkt;
			//av_log(s, AV_LOG_DEBUG, "---1---st->first_dts=%llx\n",st->first_dts); 
			compute_pkt_fields(s, st, NULL, pkt);
			//av_log(s, AV_LOG_DEBUG, "---2--st->first_dts=%llx\n",st->first_dts); 
			...
			if ((ret = parse_packet(s, &cur_pkt, cur_pkt.stream_index)) < 0){
				//OS_PRINTF("[%s]-- called parse_packet() ret: %d\n",__func__,ret);
				return ret;
				}
		} else {
			/* free packet */
	...
	for (;;) {
		...
		if (ret < 0) {
			if (pktl && ret != AVERROR(EAGAIN)) {
				...
				//OS_PRINTF("[%s]--2 called read_frame_internal, return %d\n",__func__,ret);		
				return ret;
		...
		if (av_dup_packet(add_to_pktbuf(&s->packet_buffer, pkt,
			...
			//OS_PRINTF("[%s]--3 no memory , return -12\n",__func__);		
			return AVERROR(ENOMEM);
	}
	}
...
static int try_decode_frame(AVStream *st, AVPacket *avpkt, AVDictionary **options)
{
	...
	AVPacket pkt = *avpkt;
       ...
       //av_log(NULL, AV_LOG_ERROR, "[%s]---start\n",__func__);
	...
	if (!avcodec_is_open(st->codec) && !st->info->found_decoder) {
		...
		if (!codec) {
			...
			ret = -1;
                        //av_log(NULL, AV_LOG_ERROR, "[%s]---fail 1\n",__func__);
			goto fail;
		}
		...
		if (ret < 0) {
			st->info->found_decoder = -1;
                        //av_log(NULL, AV_LOG_ERROR, "[%s]---fail 2\n",__func__);
			goto fail;
		}
		st->info->found_decoder = 1;
	...
	if (st->info->found_decoder < 0) {
		ret = -1;
                //av_log(NULL, AV_LOG_ERROR, "[%s]---fail 3\n",__func__);
		goto fail;
	}
    ...
    if(!pkt.data && !got_picture){
                //av_log(NULL, AV_LOG_ERROR, "[%s]---fail 4\n",__func__);
		ret = -1;
    }
...
fail:
	avcodec_free_frame(&frame);
        ...
        //av_log(NULL, AV_LOG_ERROR, "[%s]---end,ret=%d\n",__func__,ret);
	return ret;
}
...
int avformat_find_stream_info(AVFormatContext *ic, AVDictionary **options)
{
	...
	int flush_codecs = ic->probesize > 0;
        int ret1 = -1;
        //av_log(ic, AV_LOG_ERROR, "---avformat_find_stream_info start, file name:%s \n",ic->filename);
	if(ic->pb)
	...
	for(i=0;i<ic->nb_streams;i++) {
		...
		if (!st->parser && !(ic->flags & AVFMT_FLAG_NOPARSE)&&io_isnetworkstream==0) {
			st->parser = av_parser_init(st->codec->codec_id);
			if(st->parser){
        ...
        //av_log(ic, AV_LOG_ERROR, "---11111111\n"); 
	...
	for(;;) {
		...
		for(i=0;i<ic->nb_streams;i++) {
			...
			//av_log(ic, AV_LOG_ERROR, "---i=%d,ic->nb_streams=%d,st->first_dts=%llx\n",i,ic->nb_streams,st->first_dts); 
	            //doreen modified for demuxering some urls failed, 13-11-04
#if 0            
             if(!strncmp(ic->filename, "mp:rtmp://", 10) &&
                strstr(ic->iformat->name, "mpegts") == NULL ){
                  if(st->codec->codec_type != AVMEDIA_TYPE_VIDEO){
		            if (!has_codec_parameters(st,&errmsg1)){
		            	  //av_log(ic, AV_LOG_ERROR, "---if 1,break\n"); 
		            	  //av_log(ic, AV_LOG_WARNING, "stream %d : %s\n", i, errmsg1);
		                break;
		            }
	            }
             }
...
#if 0            
		     //doreen modified for too much loading time when playing rtmp live urls, 13-10-24
		    // if(strncmp(ic->filename, "mp:rtmp://", 10))
		    if(0)
                {
				if(   tb_unreliable(st->codec) && !(st->r_frame_rate.num && st->avg_frame_rate.num)
		               ...
		               && st->codec->codec_type == AVMEDIA_TYPE_VIDEO){
				  //av_log(ic, AV_LOG_ERROR, "---if 2,break,[%d,%d]\n",st->info->duration_count,fps_analyze_framecount); 
					break;
		            }
	            }
#endif            
            if(st->parser && st->parser->parser->split && !st->codec->extradata){
		              //av_log(ic, AV_LOG_ERROR, "---if 3,break\n"); 
				break;
            }
#if 1            
			if (st->first_dts == AV_NOPTS_VALUE &&
				...
				//av_log(ic, AV_LOG_ERROR, "---if 4,codec_type:%d, break\n",st->codec->codec_type); 
					break;
				}
		...
		if (st->first_dts == AV_NOPTS_VALUE){
			if(st->codec->codec_type == AVMEDIA_TYPE_VIDEO){
				av_log(ic, AV_LOG_ERROR, "---if 4,codec_type:%d, ",st->codec->codec_type); 
				if(count < 300){
					av_log(ic, AV_LOG_ERROR, " break\n");
					break;
				}
		...
		if (i == ic->nb_streams) {
			...
			if (!(ic->ctx_flags & AVFMTCTX_NOHEADER)) {
				...
				ret = count;
                av_log(ic, AV_LOG_DEBUG, "All info found, count=%d\n",count);
				flush_codecs = 0;
				break;
			}
		}
		...
		if (ret == AVERROR(EAGAIN)){
			av_log(ic, AV_LOG_DEBUG, "read_frame_internal ret : %d, continue\n",ret);
			continue;
		}

		if (ret < 0) {
			/* EOF or error*/
			av_log(ic, AV_LOG_DEBUG, "read_frame_internal ret : %d, break\n",ret);
			break;
		}
		...
		if (st->codec_info_nb_frames>1) {
			...
			if (st->avg_frame_rate.num > 0)
				t = FFMAX(t, av_rescale_q(st->codec_info_nb_frames, av_inv_q(st->avg_frame_rate), AV_TIME_BASE_Q));
                    if (io_isnetworkstream && ff_mpegts_pmt.use_ff_mpegts && t==0
...
#if FF_API_R_FRAME_RATE
		{
            ...
            //if(st->codec->codec_type == AVMEDIA_TYPE_VIDEO)
	     	//av_log(ic, AV_LOG_WARNING, "---compare dts[0x%llx, 0x%llx]\n", pkt->dts,last);
...
#endif
		  //if(st->codec->codec_type == AVMEDIA_TYPE_VIDEO)
		  	//av_log(NULL, AV_LOG_ERROR, "----st->info->duration_count++: %d\n", st->info->duration_count);
				st->info->duration_count++;
		...
		count++;
        //av_log(ic, AV_LOG_ERROR, "---for count=%d, i=%d\n",count,i);

	}
    //av_log(ic, AV_LOG_ERROR, "---2222222222,flush_codecs=%d\n",flush_codecs);
	...
	if (flush_codecs) {
		...
		for(i=0;i<ic->nb_streams;i++) {
	     ...
	     ret1 = has_codec_parameters(st, &errmsg);
	     //av_log(ic, AV_LOG_ERROR, "---has_codec_parameters ret: %d\n",ret1);  
            if (!ret1) {
				char buf[256];
    ...
    //av_log(ic, AV_LOG_ERROR, "---333333333333\n");
    ...
    //av_log(ic, AV_LOG_ERROR, "---444444444444444\n");
...
void av_hex_dump(void *f, const uint8_t *buf, int size)
{
...
//yliu rm
#if 0
void av_pkt_dump2(void *f, AVPacket *pkt, int dump_payload, AVStream *st)
{
