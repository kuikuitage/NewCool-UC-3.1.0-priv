...
static void add_pid_to_pmt(MpegTSContext *ts, unsigned int programid, unsigned int pid)
{
    ...
    if(!ff_mpegts_pmt.stream_ok)
    {
    
        if(ff_mpegts_pmt.use_ff_mpegts && ff_mpegts_pmt.vpid == pid)
        {
            ff_mpegts_pmt.programid = programid;
        }
        if(ff_mpegts_pmt.programid != -1 && ff_mpegts_pmt.vpid != pid && ff_mpegts_pmt.programid == programid)
        {
            if(!ff_mpegts_pmt.nb_auds)
                ff_mpegts_pmt.aprgs[ff_mpegts_pmt.nb_auds++] =pid;
            else
            {
                for(i=0;i<ff_mpegts_pmt.nb_auds; i++)
                {
                     if(ff_mpegts_pmt.aprgs[i] ==pid)
                        break;
                }
                if(i == ff_mpegts_pmt.nb_auds)
                {
                    ff_mpegts_pmt.aprgs[ff_mpegts_pmt.nb_auds++] =pid;
                    //OS_PRINTF("[%s]pid[%d],nb-auds[%d]\n",__func__,pid,ff_mpegts_pmt.nb_auds);
...
void search_id_in_pmt()
{
    int i,j;
    int programid = -1;
    struct Program *p = NULL;
    ff_mpegts_pmt.video_id = -1;
    ff_mpegts_pmt.audio_id = -1;
    //OS_PRINTF("[%s]nb_auds[%d],use_ff_mpegts[%d]\n",__func__,ff_mpegts_pmt.nb_auds,ff_mpegts_pmt.use_ff_mpegts);
    if(!ff_mpegts_pmt.ts || !ff_mpegts_pmt.use_ff_mpegts)
        return;
    MpegTSContext *ts = (MpegTSContext *)ff_mpegts_pmt.ts;
    if(ff_mpegts_pmt.programid != -1 && ff_mpegts_pmt.nb_auds)
    {
        AVStream *st;
        for(j=0; j<ts->stream->nb_streams; j++) {
            st=ts->stream->streams[j];
         //OS_PRINTF("[%s]nb_streams[%d],stid[%d]\n",__func__,ts->stream->nb_streams,st->id);
           if(ff_mpegts_pmt.vpid== st->id)
                ff_mpegts_pmt.video_id = j;
        }
        for(i=0; i<ff_mpegts_pmt.nb_auds; i++){
            for(j=0; j<ts->stream->nb_streams; j++) {
                st=ts->stream->streams[j];
                if(ff_mpegts_pmt.aprgs[i] == st->id)
                {
                    if(st->codec->codec_type == AVMEDIA_TYPE_AUDIO)
                    {
                        if(ff_mpegts_pmt.audio_id == -1)
                        {
                            if(ff_mpegts_pmt.apid == 0 || ff_mpegts_pmt.apid == ff_mpegts_pmt.aprgs[i])
                            {
                                ff_mpegts_pmt.audio_id= j;
                                ff_mpegts_pmt.apid= ff_mpegts_pmt.aprgs[i];

                                //avoid to select the min audio pid, add by libin
                                if(ff_mpegts_pmt.apid == ff_mpegts_pmt.aprgs[i])
                                    break;
                            }
                        }
                        else if(ff_mpegts_pmt.aprgs[i] < ff_mpegts_pmt.apid)
                        {
                            ff_mpegts_pmt.audio_id= j;
                            ff_mpegts_pmt.apid= ff_mpegts_pmt.aprgs[i];
                        }
                               ...
                               const uint8_t *buf, int buf_size, int is_start)
{
    ...
    if (tss->section_h_size != -1 && tss->section_index >= tss->section_h_size) {
        ...
        if (crc_valid){
          //  OS_PRINTF("[%s] call tss->section_cb:0x%x\n", __FUNCTION__,tss->section_cb);//peacer del
...

{
    ...
    if (!sec->section_buf) {
        av_free(filter);
        //OS_PRINTF("[%s] call av_free line:%d\n", __FUNCTION__,__LINE__);
        return NULL;
    }
    return filter;
}
...
static void mpegts_close_filter(MpegTSContext *ts, MpegTSFilter *filter)
{
    ...
    if (filter->type == MPEGTS_SECTION){
        ...
        //OS_PRINTF("[%s] MPEGTS_SECTION call av_freep(&filter->u.section_filter.section_buf)\n", __FUNCTION__);
        }
    else if (filter->type == MPEGTS_PES) {
        PESContext *pes = filter->u.pes_filter.opaque;
        av_freep(&pes->buffer);
      // OS_PRINTF("[%s] MPEGTS_PES call av_freep(&pes->buffer)\n", __FUNCTION__);
    ...
    av_free(filter);
    //OS_PRINTF("[%s] call av_free line:%d,pid=%d\n", __FUNCTION__,__LINE__,pid);
    ts->pids[pid] = NULL;
}
                                  ...
                                  uint32_t stream_type, uint32_t prog_reg_desc)
{
    ...
    if ((prog_reg_desc == AV_RL32("HDMV") ||
        ...
        if (pes->stream_type == 0x83) {
            ...
            if (!sub_st) {
                av_free(sub_pes);
                //OS_PRINTF("[%s] call av_free(sub_pes), addr: 0x%x, size:%d, pid:%d\n", __FUNCTION__,sub_pes,sizeof(sub_pes),sub_pes->pid);
                return AVERROR(ENOMEM);
            }
    ...
    if(!parsing_first && pes->st->codec->codec_type == AVMEDIA_TYPE_VIDEO && ff_mpegts_pmt.vpid != 0
        && ff_mpegts_pmt.vpid == pes->pid){
        parsing_first = 1;
        ff_mpegts_pmt.vpid = pes->pid;
        ff_mpegts_pmt.use_ff_mpegts = 1;
        ff_mpegts_pmt.ts = pes->ts;
    }
    return 0;
}
...
static void new_pes_packet(PESContext *pes, AVPacket *pkt)
{
    ...
    if(pes->stream_type > 0 && pes->st->codec->codec_type == AVMEDIA_TYPE_VIDEO && !parsing_first
        && (ff_mpegts_pmt.vpid == 0 || ff_mpegts_pmt.vpid == pes->pid))
    {
        parsing_first = 1;
        ff_mpegts_pmt.vpid = pes->pid;
        ff_mpegts_pmt.use_ff_mpegts = 1;
        ff_mpegts_pmt.ts = pes->ts;
        //OS_PRINTF("[%s] pes->data_index:%d,pid[%d]\n",__func__,pes->data_index,pes->pid);
    }
    
    if((pes->total_size == MAX_PES_PAYLOAD) && (pes->data_index < MAX_PES_PAYLOAD / 2))
    {
        buffer = av_malloc(pes->data_index+FF_INPUT_BUFFER_PADDING_SIZE);
        if(!pes->buffer || !buffer || pes->data_index<=0)
        	OS_PRINTF("[%s] --buffer:0x%x  pes->buffer: 0x%x, pes->data_index:%d\n",__func__,buffer,pes->buffer,pes->data_index,pes->data_index);
	 if(buffer){
            memcpy(buffer,pes->buffer,pes->data_index);
            av_freep(&pes->buffer);
            pkt->data = buffer;
        }
        else
            ...
            if (pes->data_index == PES_START_SIZE) {
                ...
                if (pes->header[0] == 0x00 && pes->header[1] == 0x00 &&
                    ...
                    if (!pes->st) {
                        ...
                        if (!pes->st){
                        	OS_PRINTF("[%s] called avformat_new_stream(), return -12\n", __FUNCTION__);
                            return AVERROR(ENOMEM);
                        }
                        pes->st->id = pes->pid;
                    ...
                    //OS_PRINTF("[%s] case MPEGTS_HEADER av_malloc for pes->buffer,size: [%d], addr:0x%x,pid:%d\n", __FUNCTION__,pes->total_size+FF_INPUT_BUFFER_PADDING_SIZE,pes->buffer,pes->pid);
                    if (!pes->buffer){
                    	   OS_PRINTF("[%s] case MPEGTS_HEADER av_malloc for pes->buffer,size: %d, addr: 0x%x, pid:%d, return -12\n", __FUNCTION__,pes->total_size+FF_INPUT_BUFFER_PADDING_SIZE,pes->buffer,pes->pid);
                        return AVERROR(ENOMEM);
                        }
        ...
        case MPEGTS_PAYLOAD:
            if (buf_size > 0 && pes->buffer) {
                if (pes->data_index > 0 && pes->data_index+buf_size > pes->total_size) {
                    ...
                    pes->buffer = av_malloc(pes->total_size+FF_INPUT_BUFFER_PADDING_SIZE);
                    if (!pes->buffer){
                    	   OS_PRINTF("[%s] case MPEGTS_PAYLOAD av_malloc for pes->buffer,size: %d, return -12\n", __FUNCTION__,pes->total_size+FF_INPUT_BUFFER_PADDING_SIZE);
                        return AVERROR(ENOMEM);
                        }
                    ts->stop_parse = 1;
...
static PESContext *add_pes_stream(MpegTSContext *ts, int pid, int pcr_pid)
{
    ...
    if (!tss) {
        av_free(pes);
        //OS_PRINTF("[%s] call av_free(pes), addr: 0x%x, size:%d, pid:%d\n", __FUNCTION__,pes,sizeof(pes),pes->pid);
        return 0;
    }
    return pes;
}
...
static void pmt_cb(MpegTSFilter *filter, const uint8_t *section, int section_len)
{
    ...
    for(;;) {
        ...
        if (stream_type < 0){
            //OS_PRINTF("[%s] stream_type[%d] < 0,break\n", __FUNCTION__,stream_type);
            break;
            }
        pid = get16(&p, p_end);
        if (pid < 0){
            //OS_PRINTF("[%s] pid[%d] < 0,break\n", __FUNCTION__,pid);
            break;
        }
        pid &= 0x1fff;
        ...
        if (ts->pids[pid] && ts->pids[pid]->type == MPEGTS_PES) {
            ...
            if (ts->pids[pid]){
              //OS_PRINTF("[%s] call mpegts_close_filter,stream_type =%d,pid:%d, test_cnt: %d\n", __FUNCTION__,stream_type,pid, test_cnt);
        ...
        if (!st){
        	OS_PRINTF("[%s] st[0x%x] null,goto out\n", __FUNCTION__,st);
            goto out;
        }
        ...
        if (desc_list_len < 0){
            //OS_PRINTF("[%s] desc_list_len[%d] < 0,break\n", __FUNCTION__,desc_list_len);
            break;
        }
        desc_list_len &= 0xfff;
        desc_list_end = p + desc_list_len;
        if (desc_list_end > p_end){
            //OS_PRINTF("[%s] desc_list_end[%d] > p_end[%d],break\n", __FUNCTION__,desc_list_end,p_end);
            break;
        }
        for(;;) {
...
static void pat_cb(MpegTSFilter *filter, const uint8_t *section, int section_len)
{
    ...
    for(;;) {
        ...
        if (sid < 0){
			//peacer del 20140108
	     //OS_PRINTF("[%s] sid[%d] < 0, test_cnt:%d, break\n", __FUNCTION__,sid,test_cnt); 
            break;
        }
        pmt_pid = get16(&p, p_end);
        if (pmt_pid < 0){
            //OS_PRINTF("[%s] pmt_pid[%d] < 0, test_cnt:%d, break\n", __FUNCTION__,pmt_pid,test_cnt);
            break;
        }
        pmt_pid &= 0x1fff;

        //av_dlog(ts->stream, "sid=0x%x pid=0x%x\n", sid, pmt_pid);
        //av_log(ts->stream, "sid=0x%x pid=0x%x\n", sid, pmt_pid);
        ...
        if (sid == 0x0000) {
            ...
            if (ts->pids[pmt_pid]){
				//peacer del 20140108
            	// OS_PRINTF("[%s] call mpegts_close_filter,pid=%d, test_cnt=%d\n", __FUNCTION__,pmt_pid,test_cnt);
...
static int handle_packet(MpegTSContext *ts, const uint8_t *packet)
{
    ...
    tss = ts->pids[pid];
    //OS_PRINTF("[%s] pid: %d, tss->pid: %d\n", __FUNCTION__,pid,tss->pid);
    ...
    if (tss->type == MPEGTS_SECTION) {
        ...
        //reset vpid as default value, not special value....because no special video pid found in pmt tables before
        if(!ff_mpegts_pmt.use_ff_mpegts){
            ff_mpegts_pmt.vpid = 0;
            ff_mpegts_pmt.apid = 0;
        }
        // Note: The position here points actually behind the current packet.
        if ((ret = tss->u.pes_filter.pes_cb(tss, p, p_end - p, is_start,
                                            pos - ts->raw_packet_size)) < 0){
            //OS_PRINTF("[%s] tss->u.pes_filter.pes_cb: 0x%x, ret[%d] < 0, return\n", __FUNCTION__,tss->u.pes_filter.pes_cb,ret);
            return ret;
		}
    }
...
static int handle_packets(MpegTSContext *ts, int nb_packets)
{
    ...
    if (avio_tell(s->pb) != ts->last_pos) {
        ...
        //OS_PRINTF("[%s] Skipping after seek, pb[0x%llx], last_pos[0x%llx] \n", __FUNCTION__,avio_tell(s->pb),ts->last_pos);
        ...
        for (i = 0; i < NB_PID_MAX; i++) {
            if (ts->pids[i]) {
                if (ts->pids[i]->type == MPEGTS_PES) {
                   ...
                   av_freep(&pes->buffer);
                   //OS_PRINTF("[%s] call av_freep(&pes->buffer) i=%d, addr: 0x%x, size[%d]\n", __FUNCTION__,i, &pes->buffer,pes->total_size);
                   pes->data_index = 0;
    ...
    for(;;) {
        ...
        if (ret != 0){
            OS_PRINTF("[%s] called read_packet, ret[%d] != 0, break\n", __FUNCTION__,ret);
            break;
            }
        ...
        if (ret != 0){
        	OS_PRINTF("[%s] called handle_packet, ret[%d] != 0, break\n", __FUNCTION__,ret);
            break;
            }
    }
    ts->last_pos = avio_tell(s->pb);
    if(ret < 0){
    	OS_PRINTF("[%s] ret[%d] < 0, return\n", __FUNCTION__,ret);
    }
	return ret;
}
...
static int mpegts_read_header(AVFormatContext *s)
{
	...
	//OS_PRINTF("[%s] call mpegts_open_section_filter()\n", __FUNCTION__);
                              ...
                              AVPacket *pkt)
{
    ...
    if (ret < 0) {
    	 OS_PRINTF("[%s] ret[%d] < 0, call av_free_packet()\n", __FUNCTION__,ret);
