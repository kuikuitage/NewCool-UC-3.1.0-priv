...
static int tls_open(URLContext *h, const char *uri, int flags)
{
    ...
    if (!lwip_getaddrinfo(host, NULL, &hints, &ai)) {
        numerichost = 1;
    ...
    if (use_proxy) {
        ...
        memset(proxy_host, 0, 200);
        memset(proxy_auth, 0, 200);
        memset(dest, 0, 200);
        int proxy_port;
        mtos_printk("[%s] zx in use_proxy[%d] proxy_path[%s] \n",__func__, use_proxy, proxy_path);
