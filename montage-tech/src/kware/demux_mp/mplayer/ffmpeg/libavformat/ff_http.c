...
static int http_open_cnx(URLContext *h)
{
    ...
    if (proxy_path == NULL) {
        ...
        if (p_proxy = (HTTP_proxy_t *)mthttp_proxy_get()) {
            if (p_proxy->username)
                proxy_path = mthttp_mp_asprintf("http://%s:%s@%s:%d/",
                                       p_proxy->username,
                                       p_proxy->password ? p_proxy->password : "",
                                       p_proxy->hostname, p_proxy->port);
            else
                proxy_path = mthttp_mp_asprintf("http://%s:%d/",
                                       p_proxy->hostname, p_proxy->port);
        }
    }
...
static int http_buf_read(URLContext *h, uint8_t *buf, int size)
{
    ...
    if (len > 0) {
        ...
        //yliu add: for content length not right
           if(s->filesize>0&&len>0&&((s->off+len)>s->filesize))
            {
            int over_size = (s->off+len)-s->filesize;
            int j=0;
            char *over_buf = buf+len-over_size;
            for(j = 0;j<over_size;j++)
                {
                if((*over_buf)!='\r'&&(*over_buf)!='\n')
                    break;
                over_buf++;
...
static int64_t http_seek1(URLContext *h, int64_t off, int whence)
{
    HTTPContext *s = h->priv_data;
