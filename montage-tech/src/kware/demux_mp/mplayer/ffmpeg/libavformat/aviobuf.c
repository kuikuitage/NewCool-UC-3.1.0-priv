...
{
    int64_t ret;
   // av_log(s, AV_LOG_PANIC, "\n  %s %d\n", __func__,__LINE__);
    ret = avio_seek(s, offset, SEEK_CUR);
   // av_log(s, AV_LOG_PANIC, "\n  %s %d\n", __func__,__LINE__);
    return ret;
}
...
int avio_read(AVIOContext *s, unsigned char *buf, int size)
{
    ...
    //OS_PRINTF("[%s]-------start! s->read_packet: 0x%x\n",__func__,s->read_packet);
    size1 = size;
    while (size > 0) {
    ...
    if (size1 == size) {
        if (s->error){
		OS_PRINTF("[%s]------- return s->error[%d]\n",__func__,s->error);
        	return s->error;
        }
        if (url_feof(s)){
		OS_PRINTF("[%s]------- url_feof(s), return AVERROR_EOF[%d]\n",__func__,AVERROR_EOF);
        	return AVERROR_EOF;
        }
    }
    return size1 - size;
...
int avio_openm(AVIOContext **s, const char *filename, int flags)
{
    return avio_open2m(s, filename, flags, NULL, NULL);
}
...
int avio_open2m(AVIOContext **s, const char *filename, int flags,
               const AVIOInterruptCB *int_cb, AVDictionary **options)
{
    URLContext *h;
    int err;
    //printf("[%s] line[%d],use_mem_size[%d],libmem_size[%d]\n",__func__,__LINE__,use_mem_size,libmem_size);
	...
	//yliu add:for redirect url
      
        	if(strncmp(filename,"http://",7) == 0)//yliu add
	{

       //mtos_printk("[%s] --1 filename:%s\n",__func__, filename);
     HTTPContext *ss = h->priv_data;
       memset(filename,0,MAX_URL_SIZE);
      
     memcpy(filename,ss->location,strlen(ss->location));
     //mtos_printk("[%s] --2 filename:%s\n",__func__, filename);
    }
