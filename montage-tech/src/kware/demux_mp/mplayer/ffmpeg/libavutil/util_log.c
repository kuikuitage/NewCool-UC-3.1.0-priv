...
#else
#define set_color(x)  )  OS_PRINTF("\033[%d;3%dm", (color[x] >> 4) & 15, color[x] & 15)
#define set_256color(x)  OS_PRINTF("\033[48;5;%dm\033[38;5;%dm", (color[x] >> 16) & 0xff, (color[x] >> 8) & 0xff)
#define reset_color()  OS_PRINTF("\033[0m")
#endif
#endif
...
#undef fprintf
    if (print_prefix && (flags & AV_LOG_SKIP_REPEATED) && !strcmp(line, prev)){
        ...
        if (is_atty == 1)
            ...
            #else
            OS_PRINTF("    Last message repeated %d times\r", count);
            #endif
        return;
    }
    if (count > 0) {
        ...
        #else
        OS_PRINTF("    Last message repeated %d times\n", count);
        #endif
        count = 0;
    }
    strcpy(prev, line);
