		...
		const char *hostname, unsigned int port)
{
	...
	printf_debug("[%s:%s:%d] yiyuan test \n", __FILE__,__func__,__LINE__);
...
void ne_close_connection(ne_session *sess)
{
	if (sess->connected) 
	{
		...
		//OS_PRINTF("[%s]Closing connection!!\n",__func__);
		...
		//OS_PRINTF("[%s]Connection closed!!!\n",__func__);

	} else {
		NE_DEBUG(NE_DBG_SOCKET, "(Not closing closed connection!).\n");
		OS_PRINTF("[%s](Not closing closed connection!)!!!\n",__func__);
	}
	sess->connected = 0;
}
...
int  ne_ssl_trust_keypair(ne_session *sess, const char *cert,
                           const char *key,const char *pw)
{
#ifdef NE_HAVE_SSL
	if (sess->ssl_context) {
		
		ne_ssl_context_set_userdata_pw(sess->ssl_context, pw) ;		
		return ne_ssl_context_keypair(sess->ssl_context, cert,key);
	}
#endif
	return   10 ;
}
