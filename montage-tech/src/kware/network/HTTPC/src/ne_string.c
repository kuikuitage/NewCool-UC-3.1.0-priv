...
__attribute__((weak)) void ne_print_download_info(char *url, int isPost, const char * extraHeaders, const char * body)
{
    mtos_printk("[%s] test , just return false !\n", __func__);
}
...
//	OS_PRINTF("[%s] ==start start \n",__func__);
...
#else
	if(buf < 0x80000000 || buf >= 0x90000000)
	{
		OS_PRINTF("[%s][ERROR] buf[0x%x]==end do noting\n",__func__,buf);
		return;
	}


	if(buf->data > 0x80000000 && buf->data <= 0x90000000)
	{
		ne_free(buf->data);
		buf->data = NULL;
	}
	...
	{
		OS_PRINTF("[%s] do noting11  buf->data[0x%x]\n",__func__,buf->data);
		//buf->data = NULL;
	}
