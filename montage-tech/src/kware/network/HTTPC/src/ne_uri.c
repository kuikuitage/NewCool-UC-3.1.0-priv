	...
	{  
		if (isalnum(*src) || *src == '-' || *src == '_' || *src == '.' || *src == '~')   
			...
			*dest = '+';  
            ...
            {      
			*dest = '%', *(dest+1) = to_hex(*src >> 4), *(dest+2) = to_hex(*src & 15);  
                    return 3;
            }
	} 
      else
  ...
  path:
    ...
    if (*p != '\0') {
        ...
        while (1) {
            ...
            else if(*p < 0)  ///yiyuan change this 20140821 for aljabeer search function
			{
			        
				int len  = 0 ;
				len = url_encode(p,reform+reformindex);
                          reformindex += len;
                          p++;
			}
			else {
                if (*p =='\0' || *p == '#' )
                     ...
                     * to deal with it.
                     */
                int l = ne_uri_urlencode(*p, reform+reformindex);///just copy it 
                reformindex += l;
                p++;
            }
        }        
