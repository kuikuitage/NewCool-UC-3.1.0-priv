...
void ne_ssl_context_set_userdata_pw(ne_ssl_context *ctx, const char *pw)
{
	if(pw)
	{
		printf_debug("[%s:%s:%d] ne_ssl_context_set_userdata_pw  pw= %s  ! \n", __FILE__,__func__,__LINE__,pw);
		SSL_CTX_set_default_passwd_cb_userdata(ctx->ctx, pw);
	}
}
...
//	SSL_CTX_set_default_passwd_cb_userdata(ctx->ctx, "yiyuan");
	if(key)
	    ret = SSL_CTX_use_PrivateKey_file(ctx->ctx, key, SSL_FILETYPE_PEM);
	printf_debug("[%s:%s:%d] SSL_CTX_check_private_key  ret = %d  ! \n", __FILE__,__func__,__LINE__,ret);
    if (ret == 1) {
		if(cert)
	        ret = SSL_CTX_use_certificate_file(ctx->ctx, cert, SSL_FILETYPE_PEM);
		printf_debug("[%s:%s:%d] SSL_CTX_use_certificate_file  ret = %d  ! \n", __FILE__,__func__,__LINE__,ret);	
    }

	 if (!SSL_CTX_check_private_key(ctx->ctx)) 
	 {
	 	printf_debug("[%s:%s:%d] SSL_CTX_check_private_key  test   fail ! \n", __FILE__,__func__,__LINE__);
	 }
	 else
	 {
	 	printf_debug("[%s:%s:%d] SSL_CTX_check_private_key  test   ok  ! \n", __FILE__,__func__,__LINE__);
	 }
