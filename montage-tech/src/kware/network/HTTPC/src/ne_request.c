...
static int aborted(ne_request *req, const char *doing, ssize_t code)
{
	...
	NE_DEBUG(NE_DBG_HTTP, "Aborted request (%" NE_FMT_SSIZE_T "): %s\n",
			code, doing);
      mtos_printk( "Aborted request (%" NE_FMT_SSIZE_T "): %s\n",
			code, doing);
	switch(code) {
		case NE_SOCK_CLOSED:
			if (sess->use_proxy) {
				ne_set_error(sess, _("%s: connection was closed by proxy server"),
						doing);
                          mtos_printk("%s: connection was closed by proxy server",doing);
			} else {
				ne_set_error(sess, _("%s: connection was closed by server"),
						doing);
                          mtos_printk("%s: connection was closed by server",doing);
			}
			break;
		case NE_SOCK_TIMEOUT:
			ne_set_error(sess, _("%s: connection timed out"), doing);
                   mtos_printk("%s: connection timed out",doing);
			ret = NE_TIMEOUT;
			break;
		case NE_SOCK_ERROR:
		case NE_SOCK_RESET:
		case NE_SOCK_TRUNC:
			ne_set_error(sess, "%s: %s", doing, ne_sock_error(sess->socket));
                   mtos_printk("%s: %s",doing, ne_sock_error(sess->socket));
			break;
		case 0:
...
#else
		newoff = ne_lseek(req->body.file.fd, req->body.file.offset, 0);
#endif
...
static int send_request(ne_request *req, const ne_buffer *request)
{
	...
	OS_PRINTF("[%s]Sending request-line and headers:\n",__func__);
...
static int read_response_headers(ne_request *req) 
{
	...
	static int lookup_host(ne_session *sess, struct host_info *info)
	{
		...
		if (ne_addr_result(info->address)) {
			...
			ne_set_error(sess, _("Could not resolve hostname `%s': %s"), 
                   ...
                   mtos_printk("[%s] Could not resolve hostname : %s\n",__func__,info->hostname);
	...
	int ne_begin_request(ne_request *req)
	{
		...
		if (!req->flags[NE_REQFLAG_IDEMPOTENT] && req->session->persisted
			...
			mtos_printk("[%s][ERROR] req: Closing connection for non-idempotent!!\n",__func__);
		 ...
		 * retried: */
		free_response_headers(req);
		printf_debug("[%s] 666666666...\n",__func__);
		/* Read the headers */
		ret = read_response_headers(req);
		if (ret) 
			return ret;
		printf_debug("[%s] rrrrrrrrr...\n",__func__);
	...
	static int do_connect(ne_session *sess, struct host_info *host, const char *err)
	{
		...
		if (ret) {
			...
			mtos_printk("[%s] [ERROR] return NE_CONNECT!!  err:%s\n",__func__,err);
			return NE_CONNECT;
		}
	...
	static int open_connection(ne_session *sess) 
	{
		...
		if (host->address == NULL) {
			//OS_PRINTF("[%s] error 1111111111 \n",__func__);
			ret = lookup_host(sess, host);
			if (ret) return ret;
		}    
		//OS_PRINTF("[%s] start do connect!! \n",__func__);
		...
		if (ret != NE_OK) 
		{
			mtos_printk("[%s]11 fail to do connect: ret:%d \n",__func__,ret);
			return ret;
		}

#ifdef NE_HAVE_SSL
		//OS_PRINTF("[%s] 33333333333 \n",__func__);
		...
		if (sess->use_ssl && !sess->in_connect) {
			...
			if (ret == NE_OK) 
			{
				...
				if (ret != NE_OK)
				{
					mtos_printk("[%s]ssl fail to do connect: ret:%d \n",__func__,ret);
					ne_close_connection(sess);
				}
			}
		}
