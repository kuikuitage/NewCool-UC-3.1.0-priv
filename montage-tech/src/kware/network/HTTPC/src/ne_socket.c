...
#include <socks.h>
#endif
//#include "..\..\..\..\src\drv\ethernet\ethernet_priv.h"
#endif
#ifdef NE_USE_POLL
#include <sys/poll.h>
...
#else
	startT.tv_sec = mtos_ticks_get()  / 100;
#endif
	do {
#ifdef __LINUX__
		gettimeofday(&nowT, NULL);
#else
		nowT.tv_sec = mtos_ticks_get()  / 100;
#endif
...
#else
static os_sem_t * ne_cached_lock = NULL;
//static BOOL sem_ret = mtos_sem_create(&ne_cached_lock, TRUE);
#endif
static void ne_add_cached_host(const char * hostname, void * addr) 
{
...
#else
	time_t now = mtos_ticks_get();
#endif
	int i;
#ifdef __LINUX__
	pthread_mutex_lock(&ne_cached_lock);
#else
	if(ne_cached_lock == NULL) 
	{
		ne_cached_lock = mtos_malloc(sizeof(os_sem_t));
		mtos_sem_create(ne_cached_lock, TRUE);
	}
	mtos_sem_take(ne_cached_lock,0);
#endif
	...
	ne_cached_hosts[i].hostname = ne_strdup(hostname);//peacer fix bug for using wrong strdup 20130807
...
ne_sock_addr *ne_addr_resolve(const char *hostname, int flags)
{
	...
	if (laddr == INADDR_NONE) {
		...
		memset(buf, 0, sizeof(buf));
            // OS_PRINTF( "[%s] gethostbyname_r start...hostname : %s\n", __func__,hostname);
		gethostbyname_r(hostname, &h, buf, sizeof(buf), &hp, &errn);

             printf_debug( "[%s] gethostbyname_r end...hostname : %s\n", __func__,hostname);
		...
		const struct sockaddr *sa, size_t salen)
{
	...
	if (sock->cotimeout) {
...
#else
		val = 0;
		ioctlsocket(fd, FIONREAD, &val);
		if (ioctlsocket(fd, FIONBIO,  &val) == -1) {
			set_strerror(sock, errno);
			return NE_SOCK_ERROR;
		}
#endif
		...
		/* Reset to old flags: */
#ifdef __LINUX__  
		if (fcntl(fd, F_SETFL, flags) == -1) {//F_SETFL 4
			set_strerror(sock, errno);
			ret = NE_SOCK_ERROR;
		}
#else
		...
		const ne_inet_addr *addr, unsigned int port)
{
	...
	///yiyuan add this for bug: 52829/52519, 20141203
	//while in https , the socket can not connect to server , it may block the UI 
	struct timeval to;
	to.tv_sec = 1*1000 * 2 ; ///2 S 
...
int ne_sock_connect_ssl(ne_socket *sock, ne_ssl_context *ctx, void *userdata)
{
	...
	SSL *ssl;
#ifdef  __LINUX__
//	printf("remove function  seed_ssl_prng , yiyuan add 2014 12 05 ,for this function return error , while without this , it download https well  ! \n");
#else
	if (seed_ssl_prng()) {
	...
	printf_debug("[%s:%s:%d] ssl test \n", __FILE__,__func__,__LINE__);
	
#ifdef HAVE_OPENSSL
#if 0 ///yiyuan remove this 20140905 just for run https download 
	...
	ret = SSL_connect(ssl);
		printf_debug("[%s:%s:%d] ssl test  ret = %d \n", __FILE__,__func__,__LINE__,ret);
	if (ret != 1) {
		error_ossl(sock, ret);
		SSL_free(ssl);
		sock->ssl = NULL;
