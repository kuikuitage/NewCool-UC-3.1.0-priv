...
/*!
  IRDA HuiZhou Decoder State
  */
typedef enum
{
  E_HUIZHOU_STA_IDLE = 0,
  E_HUIZHOU_STA_START,
  E_HUIZHOU_STA_KEY,
  E_HUIZHOU_STA_FINISH
} irda_pro_huizhou_status_t;

/*!
  IRDA HuiZhou Decoder Signal
  */
typedef enum
{
  E_HUIZHOU_SIG_START = 0,
  E_HUIZHOU_SIG_BIT0,
  E_HUIZHOU_SIG_BIT1,
  E_HUIZHOU_SIG_INVALID
}  irda_pro_huizhou_sig_t;

/*
0 -->         ontime  50us ~ 450us                         9 ~ 84
                 period  339+50us ~ 681+450us           73 ~ 213

1 -->         ontime  300us ~ 700us                       56 ~ 132
                 period  900+300us ~ 1210+700us        226 ~ 360
			 
start -->    ontime 4800us ~ 5200us                      905 ~ 981
                 period 4800+4800us ~ 5200+5200us    1811 ~ 1962

*/
static u16  huizhou_sigtbl[12] = {905, 981, 1811, 1962, 9, 84, 73, 213, 56, 132, 226, 360};
#ifdef IRDA_DEBUG
char * huizhou_sig_strs[4] = {
"E_HUIZHOU_SIG_START","E_HUIZHOU_SIG_BIT0","E_HUIZHOU_SIG_BIT1",
"E_HUIZHOU_SIG_INVALID"
};

char * huizhou_sta_strs[4] = {
"E_HUIZHOU_STA_IDLE","E_HUIZHOU_STA_START",
"E_HUIZHOU_STA_KEY","E_HUIZHOU_STA_FINISH"
};
#endif
...
static u8 irda_concerto_huizhou_decoder_parse_sig(u16 ontime, u16 period, u16 * sigtbl)
{
  if((ontime >= sigtbl[0]) && (ontime <= sigtbl[1]) &&
     (period >= sigtbl[2]) && (period <= sigtbl[3]))
  {
...
inline static void irda_concerto_huizhou_reset_decoder(irda_dec_result_t *p_result)
{
  p_result->state = E_HUIZHOU_STA_IDLE;
  p_result->counter = 0;
  p_result->r.huizhou.kcode = 0;
}

/*IRDA huizhou Decoder */
static void irda_concerto_huizhou_decoder(irda_dec_para_t *p_para, irda_dec_result_t *p_result)
{
  u8 sig = irda_concerto_huizhou_decoder_parse_sig(p_para->ontime, p_para->period, p_result->p_sigtbl);
  IRDA_PRINT("irda_decoder sig=%s, state=%s\n", huizhou_sig_strs[sig], huizhou_sta_strs[p_result->state]);
  if(p_result->state == E_HUIZHOU_STA_IDLE)
  {
    if(sig == E_HUIZHOU_SIG_START)
    {
      p_result->counter = 0;
      p_result->state = E_HUIZHOU_STA_START;
    }
    else
    {
      p_result->counter = 0;
  ...
  else if(p_result->state == E_HUIZHOU_STA_START)
  {
    if(sig == E_HUIZHOU_SIG_BIT0 || sig == E_HUIZHOU_SIG_BIT1)
    {
      p_result->state = E_HUIZHOU_STA_KEY;
      p_result->counter = 0;
      if(sig == E_HUIZHOU_SIG_BIT0)
      {
        p_result->r.huizhou.kcode &= ~(1 << p_result->counter);
      }
      else
      {
        p_result->r.huizhou.kcode |= (1 << p_result->counter);
    ...
    {
      irda_concerto_huizhou_reset_decoder(p_result);
    }
  }  
  else if(p_result->state == E_HUIZHOU_STA_KEY)
  {
    if(sig == E_HUIZHOU_SIG_BIT0 || sig == E_HUIZHOU_SIG_BIT1)
    {
      p_result->counter ++;
      if(p_result->counter < 12)
      {
        ...
        if(p_result->counter == 11)
        {
          p_result->state = E_HUIZHOU_STA_FINISH;
          p_result->counter = 0;
        }
      }
    }
...
static RET_CODE irda_concerto_decoder(irda_dec_para_t *p_para, irda_dec_result_t *p_result)
{
  ...
  switch(p_para->protocol)
  {
    ...
    case IRDA_SW_PANA:
      ...
      return SUCCESS;
    case IRDA_SW_HUIZHOU:
        //mtos_printk("\n %s,%d",__FUNCTION__,__LINE__);
        irda_concerto_huizhou_decoder(p_para, p_result);
        return SUCCESS;
    default:
      return ERR_NOFEATURE;
    }
...
static RET_CODE irda_concerto_set_wfilt(irda_priv_t *p_ir)
{
  ...
  for(i = 0; i < channel_num; i++)
  {
    ...
    else if (irda_protocol == IRDA_SW_HUIZHOU)
     { /* IrDA Huizhou protocol */
       ...
       for(j = 0; j < channel_len; j++)
       {
         ...
         {
           irda_concerto_wave_wfn_set(j + wfilt_add_len,
             huizhou_sigtbl[8], huizhou_sigtbl[9], huizhou_sigtbl[10], huizhou_sigtbl[11]);
         }
         else // Timing of Pana bit 0 code
         {
           irda_concerto_wave_wfn_set(j + wfilt_add_len,
             huizhou_sigtbl[4], huizhou_sigtbl[5], huizhou_sigtbl[6], huizhou_sigtbl[7]);
         }
       }
...
static void irda_concerto_int_handler(void)
{
  ...
  else /* software decoding */
  {
    ...
    if(!is_ir_intialized)
    {
      ...
      else if (p_irda->irda_protocol == IRDA_SW_HUIZHOU)
      {
        dec_result.state = E_HUIZHOU_STA_IDLE;
        dec_result.p_sigtbl = huizhou_sigtbl;
      }
    }
	...
	OS_PRINTF("\n kcode:%x,ucode:%x",kcode,ucode);
        break;
              ...
              (dec_result.state == E_HUIZHOU_STA_FINISH))
     {
       OS_PRINTF("\n %s,%d",__FUNCTION__,__LINE__);
       kcode = dec_result.r.huizhou.kcode & 0xff;
       ucode = (dec_result.r.huizhou.kcode >> 8) & 0xf;
       OS_PRINTF("\n kcode:%x",kcode);
       break;
     }
     ...
     else if (p_irda->irda_protocol == IRDA_SW_PANA)
    {
     ...
     {
       if (dec_result.state != E_HUIZHOU_STA_FINISH)
         return;
       else
         dec_result.state = E_HUIZHOU_STA_IDLE;
     }
...
//mtos_printk("\n %s,%d",__FUNCTION__,__LINE__);
   ...
   else if (p_irda->irda_protocol == IRDA_SW_PANA)
  {
    ...
    OS_PRINTF("\n %s,%d",__FUNCTION__,__LINE__);
    OS_PRINTF("   user code: 0x%x\n", ucode);
    OS_PRINTF("   key code: 0x%x\n", irda_code);
  }
  else if (p_irda->irda_protocol == IRDA_SW_HUIZHOU)
   {
     irda_code = kcode;
