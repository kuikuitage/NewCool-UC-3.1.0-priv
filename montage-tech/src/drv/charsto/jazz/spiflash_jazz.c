...
typedef struct spi_flash 
{
	...
	s32 (*erase_sector)(struct spi_flash *p_flash, u32 offset);
...
static inline s32 spi_flash_erase_sector(spi_flash_t *p_flash, u32 offset)
{
    if (p_flash->erase_sector != NULL)
        return p_flash->erase_sector(p_flash, offset);
	else
		return -1;
}
...
s32 spi_flash_cmd_erase_sector(spi_flash_t *p_flash, u8 erase_cmd,
      u32 offset)
{
  s32 ret = 0;
  u8 cmd[4];

  if (offset % (1<<SECTOR_SHIFT)) {
    SF_DEBUG_PRINTF("SF: Erase offset/length not multiple of erase size %s %d %s\n",
            __FUNCTION__, __LINE__, __FILE__);
    return -1;
  }
    ...
    u32 len, void *p_buf)
{
  ...
  cmd[0] = CMD_OTP_WR;
  spi_flash_addr(offset, cmd);
  SF_DEBUG_PRINTF("prepare to do otp write offset:%x cmd:%x len:%x\n",offset,cmd[0],len);
  ret = spi_flash_cmd_write_enable(p_flash);
  if (ret < 0) {
    return ret;
  }
  ...
  spi_flash_cmd_wait_ready(p_flash, SPI_FLASH_PROG_TIMEOUT);
  SF_DEBUG_PRINTF("finish doing otp wr offset:%x ret:%x\n",offset,ret);
  return ret;
}
...
s32 spi_flash_cmd_otp_erase(spi_flash_t *p_flash, u32 offset)
{
  ...
  cmd[0] = CMD_OTP_ER;
  spi_flash_addr(offset, cmd);
  SF_DEBUG_PRINTF("prepare to do otp erase offset:%x cmd:%x\n",offset,cmd[0]);
  ret = spi_flash_cmd_write_enable(p_flash);
  if (ret < 0) {
    return ret;
  }
  ...
  spi_flash_cmd_wait_ready(p_flash, SPI_FLASH_PROG_TIMEOUT);
  SF_DEBUG_PRINTF("finish doing otp erase offset:%x ret:%x\n",offset,ret);
  return ret;
}
...
static s32 winbond_erase_sector(spi_flash_t *p_flash, u32 offset)
{
  return spi_flash_cmd_erase_sector(p_flash, CMD_W25_SE, offset);
}
...
//  p_flash = mtos_malloc(sizeof(*p_flash));
  ...
  page_size = 1 << p_params->l2_page_size;
	...
	p_flash->erase_sector= winbond_erase_sector;
...
static s32 eon_erase_sector(spi_flash_t *p_flash, u32 offset)
{
  return spi_flash_cmd_erase_sector(p_flash, CMD_EN25Q128_SE, offset);
}
...
spi_flash_t *spi_flash_probe_eon(spi_bus_t *p_spi, u8 *p_idcode)
{
 ...
 // p_flash = mtos_malloc(sizeof(*p_flash));
  ...
  p_flash->erase_sector = eon_erase_sector;
  p_flash->read = spi_flash_cmd_read_fast;
  p_flash->power_down = spi_flash_cmd_power_down;
  p_flash->power_up = spi_flash_cmd_power_up;
  p_flash->page_size = p_params->page_size;
...
static s32 sst_erase_sector(spi_flash_t *p_flash, u32 offset)
{
  return spi_flash_cmd_erase_sector(p_flash, CMD_SST_SE, offset);
}
...
spi_flash_probe_sst(spi_bus_t *p_spi, u8 *p_idcode)
{
  ...
  p_flash->erase_sector = sst_erase_sector;
  p_flash->read = spi_flash_cmd_read_fast;
...
static s32 macronix_erase_sector(spi_flash_t *p_flash, u32 offset)
{
  return spi_flash_cmd_erase_sector(p_flash, CMD_MX25XX_SE, offset);
}
...
spi_flash_t *spi_flash_probe_macronix(spi_bus_t *p_spi, u8 *p_idcode)
{
 ...
 // p_flash = mtos_malloc(sizeof(*p_flash));
  ...
  p_flash->erase_sector = macronix_erase_sector;
  p_flash->power_down = spi_flash_cmd_power_down;
  p_flash->power_up = spi_flash_cmd_power_up;
...
static s32 esmt_erase_sector(spi_flash_t *p_flash, u32 offset)
{
    return spi_flash_cmd_erase_sector(p_flash, CMD_ESMT_SE, offset);
}
...
spi_flash_t *spi_flash_probe_esmt(spi_bus_t *p_spi, u8 *p_idcode)
{
    ...
    p_flash->erase_sector = esmt_erase_sector;
    p_flash->read = spi_flash_cmd_read_fast;
    p_flash->power_down = spi_flash_cmd_power_down;
    p_flash->power_up = spi_flash_cmd_power_up;
    p_flash->page_size = p_params->page_size;
...
static s32 gigadevice_erase_sector(spi_flash_t *p_flash, u32 offset)
{
    return spi_flash_cmd_erase_sector(p_flash, CMD_GIGADEVICE_SE, offset);
}
...
spi_flash_t *spi_flash_probe_gigadevice(spi_bus_t *p_spi, u8 *p_idcode)
{
    ...
    p_flash->erase_sector = gigadevice_erase_sector;
    p_flash->read = spi_flash_cmd_read_fast;
    p_flash->power_down = spi_flash_cmd_power_down;
    p_flash->power_up = spi_flash_cmd_power_up;
    p_flash->page_size = p_params->page_size;
...
#define CMD_ISSI_SE		    0x20	/* sector Erase */
...
static s32 issi_erase_sector(spi_flash_t *p_flash, u32 offset)
{
    return spi_flash_cmd_erase_sector(p_flash, CMD_ISSI_SE, offset);
}
...
spi_flash_t *spi_flash_probe_issi(spi_bus_t *p_spi, u8 *p_idcode)
{
	...
	flash->erase_sector= issi_erase_sector;	
...
static RET_CODE spiflash_jazz_protect_set(spi_jazz_priv_t *p_priv, u32 param)
{
    ...
    switch(m_id)
    {
        case FLASH_MF_GD:
            ...
            if(pro_cmd == 0xFFFF)
            {
                SF_DEBUG_PRINTF("GD protect type err!\n");
                return ERR_FAILURE;
            }
        ...
        case FLASH_MF_WINB:
            ...
            if(pro_cmd == 0xFFFF)
            {
                SF_DEBUG_PRINTF("winbond protect type err!\n");
                return ERR_FAILURE;
            }
            //spi_jazz_set_sections_prot_by_mode(p_priv, prt_type);
            status[0] = pro_cmd & 0xFF;
        ...
        default:
            ...
            {
                SF_DEBUG_PRINTF("The flash type not in list can't use block sector function\n");
                return ERR_FAILURE;
            }
            return SUCCESS;
    }
...
S25FL004K/S25FL008K/S25FL016K can support the read unique id command.*/
  ...
  spi_flash_read_common(p_priv->p_flash,cmd_buf,5,p_buf, 8);
}
...
static s32 flash_get_unique_id(spi_jazz_priv_t *p_priv, u8 *p_buf, u32 len)
{
  ...
  SF_DEBUG_PRINTF("flash_get_unique_id m_id[%x]\n",m_id);
	switch(m_id)
	{
	case FLASH_MF_KH:
...
static RET_CODE flash_read_status_reg(lld_charsto_t *p_spiflash_set,  u32 *status)
{
  ...
  if(spi_read(p_spi_bus, cmd_buf, 1, NULL, data_buf, 1) != SUCCESS)
  {
    OS_PRINTF("%s %d %s\n", __FUNCTION__, __LINE__, __FILE__);
    return ERR_FAILURE;       
  }
...
static RET_CODE flash_write_enable(lld_charsto_t * p_spiflash_set)
{
  ...
  if(spi_write(p_spi_bus, cmd_buf, 1, NULL, NULL, 0) != SUCCESS)
  {
    OS_PRINTF("%s %d %s\n", __FUNCTION__, __LINE__, __FILE__);
    return ERR_FAILURE;
  }
...
static RET_CODE spiflash_jazz_erase_sector(lld_charsto_t *p_lld, u32 addr)
{
  ...
  if (spi_flash_erase_sector(p_priv->p_flash,addr) == -1)
    return ERR_FAILURE;
  else
    return SUCCESS;
}
    ...
    switch(cmd)
    {
        ...
        */
        case CHARSTO_IOCTRL_ERASE_MIN_SECTOR:
            return spiflash_jazz_erase_sector(p_lld, param);
            break;
