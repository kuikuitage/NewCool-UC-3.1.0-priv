...
const UINT8 MxLWareDrvVersion[] = {2, 1, 22, 1};
...
static MXL_STATUS MxL_GetDeviceInfo(PMXL_DEV_INFO_T DevInfoPtr)
{
  ...
  MxL_DLL_DEBUG0("Device Id = 0x%d, Device Version = 0x%x\n", DevInfoPtr->DevId, DevInfoPtr->DevVer);
...
static MXL_STATUS MxL_ConfigDemodSymbolRate(PMXL_SYMBOL_RATE_T SymbolRatePtr) 
{
  ...
  MxL_DLL_DEBUG0("MXL_DEMOD_SYMBOL_RATE_CFG - SymbolType=%d, SymbolRate=%d, SymbolRate256=%d, OobSymbolRate=%d\n",
...
--| DATE CREATED  : 6/19/2009
--|
--| DESCRIPTION   : This function configures AGC mode
--|
--| RETURN VALUE  : True or False
--|
--|---------------------------------------------------------------------------*/
...
--| 
--| AUTHOR        : Sunghoon Park, Mahee, Dong 
--|
--| DATE CREATED  : 02/01/2010, 08/03/2012, 11/18/2013 
--|
...
static MXL_STATUS MxL_ConfigTunerChanDependent(PMXL_CHAN_DEPENDENT_T ParamPtr)
{
  ...
  if (ParamPtr->ChanDependentCfg == MXL_ENABLE)
  {
    ...
    /* Change 0x009F and 0x0084 setting according to frequency point */
    /* Read DN_RF_RSSI_AMP_GAIN value  */
    status |= Ctrl_ReadRegister(ParamPtr->I2cSlaveAddr, DN_RF_RSSI_AMP_GAIN_REG, &tmpData);
    tmpData &= 0xFFF9;  // set Bit<2:1> = 0 

    if (ParamPtr->RfFreqHz < 333000000)
    {
      tmpData |= 0x06; 
      control = 0x15;    /* DIG_RSSITHLO and DIG_RSSITHHI value */ 
    }
    else 
    {
      tmpData |= 0x00; 
      control = 0x37;    /* DIG_RSSITHLO and DIG_RSSITHHI value */      
    }
    status |= Ctrl_WriteRegister(ParamPtr->I2cSlaveAddr, DN_RF_RSSI_AMP_GAIN_REG, tmpData);
    status |= Ctrl_WriteRegister(ParamPtr->I2cSlaveAddr, DIG_RSSITH_REG, control);
  }
  else
  {
    status |= Ctrl_WriteRegister(ParamPtr->I2cSlaveAddr, XTAL_EN_VCO_BIASBST, tmpData);
...
static MXL_STATUS MxL_ConfigTunerChanTuneInt(PMXL_RF_TUNE_CFG_T TuneParamPtr, MXL_BOOL clearIqFlip)
{
  ...
  status |= Ctrl_WriteRegister(TuneParamPtr->I2cSlaveAddr, 0xb8, 0x00);

  if (TuneParamPtr->BandWidth == 6) bandwidth = 0x49;
  else if (TuneParamPtr->BandWidth == 8) bandwidth = 0x6F;
  else if (TuneParamPtr->BandWidth == 7) bandwidth = 0x5A;
  ...
  if (status == MXL_TRUE)
  {
    ...
    if (MxL_CheckIfFreqIsInLUT(TuneParamPtr->Frequency / 1000000) == MXL_TRUE)
    {
      UINT16 lock;

      MXL_DelayMs(1);
      status |= Ctrl_ReadRegister(TuneParamPtr->I2cSlaveAddr, RF_LOCK_STATUS_REG, &lock);
      
      if ((lock & RF_SYN_RDY) != RF_SYN_RDY)
      {
        status |= Ctrl_WriteRegister(TuneParamPtr->I2cSlaveAddr, 0xb8, 0xf4);
...
static MXL_STATUS MxL_ConfigTunerChanTuneInt(PMXL_RF_TUNE_CFG_T TuneParamPtr, MXL_BOOL clearIqFlip)
{
  ...
  MxL_DLL_DEBUG0("MXL_TUNER_CHAN_TUNE_CFG - Freq = %fHz, BW = %dMHz\n", TuneParamPtr->Frequency, TuneParamPtr->BandWidth);    
  ...
  if (status == MXL_TRUE)
  {
    ...
    if (MxL_CheckIfFreqIsInLUT((UINT32) (TuneParamPtr->Frequency / 1000000.0)) == MXL_TRUE)
    {
      UINT16 lock;
...
static MXL_BOOL MxL_WaitForLockStatus(UINT8 I2cSlaveAddr, MXL_CMD_TYPE_E lockType, UINT16 timeoutMs)
{
  ...
  } while ((status == MXL_TRUE) && (lockStatus.Status != MXL_LOCKED) && (GetSystemTimeMs() < (startTime + timeoutMs)));
  MxL_DLL_DEBUG0("lock:%d (%dms)\n", lockStatus.Status, (GetSystemTimeMs() - startTime));
...
/*------------------------------------------------------------------------------
--| FUNCTION NAME : MxL_ConfigTunerExtendedChanTune - MXL_TUNER_EXTENDED_CHAN_TUNE_CFG
--| 
--| AUTHOR        : Mariusz Murawski 
--|
--| DATE CREATED  : 5/16/2012
    ...
    if ((regularTuneParams.Frequency == lastTunedFrequency) || (MxL_ConfigTunerChanTuneInt(&regularTuneParams, MXL_ENABLE) == MXL_TRUE))
    {
      ...
      if (MxL_WaitForLockStatus(TuneParamPtr->I2cSlaveAddr, MXL_DEMOD_QAM_LOCK_REQ, TuneParamPtr->qamLockTimeoutMs) == MXL_LOCKED)
      {
        ...
        else
        {
          ...
          } else done = MXL_TRUE; /* otherwise - game over*/
        }
...
/*------------------------------------------------------------------------------
--| FUNCTION NAME : MxL_GetDemodIntSnr - MXL_DEMOD_SNR_REQ
--| 
--| AUTHOR        : Sunghoon Park
--|
--| DATE CREATED  : 3/02/2010
--|
--| DESCRIPTION   : This function returns SNR(Signal to Noise Ratio) and
--|                 supports only int calculation
--|
--| RETURN VALUE  : True or False
--|
--|---------------------------------------------------------------------------*/
...
MXL_STATUS MxLWare_API_ConfigTuner(MXL_CMD_TYPE_E CmdType, void *ParamPtr)
{
  ...
  switch (CmdType)
  {
    ...
    default:
	  MxL_DLL_DEBUG0("MxLWare_API_ConfigTuner : WRONG COMMAND (CmdType =%d)\n", CmdType);
      status = MXL_FALSE;
      break;
  }
